...
class Mconsignee extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "mconsigneemodel");
    ...
    public function addedit() {
        ...
        if ($VarHashedId <> '' && base64_decode(urldecode($VarHashedId))) {
            ...
            $ArrResults = $this->mconsigneemodel->fnGetInfo('', '', $VarId);
            $ArrData['ArrBasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]['id'];
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addeditconsignee', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarContactName <> '') {
            ...
            $ArrResult = $this->mconsigneemodel->saveInfo($ArrUpdateData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manageconsignee() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'mconsignee/manageconsignee/';
            $config['total_rows'] = $this->mconsigneemodel->fnCount($VarMobileNo, $VarContatname, $VarEmailId, $VarStatus, $VarAfilter);
            ...
            $ArrFabricTypeList = $this->mconsigneemodel->fnList($VarMobileNo, $VarContatname, $VarEmailId, $VarStatus, $VarAfilter, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'manageconsignee');
        }
    }
    function changemStatus() {
        ...
        if ($VarActDeactOption <> '' && $VarCid <> '') {
            if ($this->mconsigneemodel->fnChangeComStat($VarCid, $VarActDeactOption))
