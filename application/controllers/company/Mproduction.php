...
class mproduction extends CI_Controller {
    private $limit = 10;
    public $companyid;
    public $userid;
    public $mysqldatetime;
    public $productionusertblname;
    public $usertblname;
    ...
    public function __construct() {
        ...
        $this->productionusertblname     = 'kn_productionusers';
        $this->mysqldatetime = date('Y-m-d H:i:s');
    }

    public function addeditproductionuser() {
...
if ($VarCompanyId <> '') {
    ...
    public function updateProductionUser() {
        $ArrUpdateData                  = array();
        ...
        $ArrUpdateData['usertype']      = '10';
        $ArrUpdateData['profilepermission'] = '10';
        if($ArrUpdateData['username']<>'') {
            ...
            $ArrResult                                          = $this->saveProductionUser($ArrUpdateData,$ArrOData);
        } else {
            $ArrResult['errcode']							    = '-1';
    ...
    public function saveProductionUser($ArrUpdateData,$ArrOData) {
        ...
        if($VarId=="") {
            ...
            if(empty($ArrCheckExist)) {
                ...
                $ArrOData['usercode'] = 'PRODUCTION_'.$VarUserId;
                $this->db->insert($this->productionusertblname,$ArrOData);
                $ArrResult['errcode']						    = 1;
                ...
                if($this->db->update(KN_USERS,$ArrUpdateData,array('id'=>$VarId))) {
                    $this->db->update($this->productionusertblname,$ArrOData,array('userid'=>$VarId));
                    $ArrResult['errcode']					    = 1;
                    $ArrResult['msg']							= 'Successfully updated';
                    $ArrResult['id']							= $VarId;
    ...
    public function manageproductionusers() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url']    = base_url().CNFCOMPANY.'mproduction/manageproductionusers/';
            ...
            $data['pagination'] = $this->pagination->create_linkswithajax('Productionusers');
            $i                  = 0;
            $ArrFnlList         = array();
            ...
            $this->load->view('production/manageproductionusers');
        }
    }
    ...
    public function fnCount($VarName = '', $VarEmailId='',$VarCompanyId='', $VarStatus = '',$VarAfilter='') {
        ...
        $VarWhere = '';
        if (@$ArrWhere[0] <> '') {
            $VarWhere = implode(" and ", $ArrWhere). " AND usertype = 10";
        }

        $VarSql = "SELECT count(1) as trec  FROM " . $this->productionusertblname. " AS c INNER JOIN " . KN_USERS . " AS u ON c.userid = u.id WHERE " . $VarWhere;
    ...
    function fnList($VarName = '', $VarEmailId='',$VarCompanyId='', $VarStatus = '', $VarLimit = 10, $offset = 0, $VarSortBy, $VarSortOrder,$VarAfilter='') {
        ...
        $VarSqlLab = "SELECT u.id,u.contactname,u.mobile,u.username,u.datecreated,u.dateupdated,u.status,u.updatedby FROM " . $this->productionusertblname. " 
    ...
    public function changemStatus() {
        ...
        if($VarActDeactOption <> '' && $VarCid <> '') {
            ...
            if ($this->db->update($this->productionusertblname, array('status' => $VarActDeactOption))) {
    ...
    public function deletedata() {
        ...
        if($VarId >= 1) {
            ...
            if($this->db->update($this->productionusertblname,$ArrUpdateData,array('userid'=>$VarId))) {
