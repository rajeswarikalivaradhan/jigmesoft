...
class Membellishmentvendor extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "membellishmentvendormodel");
    ...
    public function addedit() {
        ...
        if ($VarHashedId <> '' && base64_decode(urldecode($VarHashedId))) {
            ...
            $ArrResults = $this->membellishmentvendormodel->fnGetInfo('', '', $VarId);
            $ArrData['ArrBasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]['id'];
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addeditembellishmentvendor', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarVendorname <> '') {
            ...
            $ArrResult = $this->membellishmentvendormodel->saveInfo($ArrUpdateData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manageembellishmentvendor() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'membellishmentvendor/manageembellishmentvendor/';
            $config['total_rows'] = $this->membellishmentvendormodel->fnCount($VarVendor, $VarContatname, $VarEmailId, $VarStatus, $VarAfilter);
            ...
            $ArrFabricTypeList = $this->membellishmentvendormodel->fnList($VarVendor, $VarContatname, $VarEmailId, $VarStatus, $VarAfilter, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'manageembellishmentvendor');
        }
    }
    function changemStatus() {
        ...
        if ($VarActDeactOption <> '' && $VarCid <> '') {
            if ($this->membellishmentvendormodel->fnChangeComStat($VarCid, $VarActDeactOption))
