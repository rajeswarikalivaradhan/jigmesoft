...
class Mfabricuser extends CI_Controller {
    ...
    public function addedit() {
        ...
        $this->load->view('fabric/addeditfabricuser', $ArrData);
    }
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url']    = base_url().CNFCOMPANY.'mfabricuser/manage/';
            ...
            $data['pagination'] = $this->pagination->create_linkswithajax('FabricUser');
            $i                  = 0;
            $ArrFnlList         = array();
            ...
            $this->load->view('fabric/managefabricusers',array('ArrDesignation' => $ArrDesignation,
                'ArrStatus' => unserialize(ARRSTATUS)));
        }
    }
    ...
    public function fabricreceivedlist() {
        $ArrUserLoggedInfo = fnGetUserLoggedInfo('1');
        ...
        if ($VarFrom == 1) {
            ...
            $VarURLSegment = 3;
            $this->load->library('pagination');
            $config['base_url']    = base_url() . 'fabricuser/Fabricreceivedlist/';
            $VarStatus          = 1;
            ...
            $this->load->view('fabric/reqreceivedlist', $ArrData);
        }
    }
    public function fnReceivedCountSamUser($VarWip = '', $VarIsrIortype = '', $VarCoffDateFrom = '', $VarCoffDateTo = '', $VarRequirement = '', $VarMerchant = '', $VarBB = '',
        ...
        $ArrWhere[] = "c.companyid = " . $userInfo['companyid'] . " AND c.cadqueueno = '0' AND c.mgmtcurrentstatus IN(2,4) AND c.requestlisttypeid = '4'";
        if (@$ArrWhere[0] <> '') {
...
AS oe ON c.orderid = oe.id WHERE " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder . " limit " . $VarLimitInfo;
    ...
    /*    public function fabricindentlist_ajaxdata() {
            $ArrList = $this->msamplerequestmodel->samIndentListDataTables();
            //echo '<pre>'; print_r($ArrList); die('die');
            $data = array();
            $ArrRequestTypes = unserialize(ARRREQUESTTYPE);
            $ArrStatus = unserialize(ARRSTATUS);
            foreach ($ArrList as $Obj) {
                $row = array();
                $row[] = '<input type="checkbox" class="allcbox" id="'.$Obj->requestrefid.'">';
                $row[] = $Obj->isriorcode;
                ...
                $row[] = $Obj->indentrefno;
                $row[] = $Obj->requestdt;
                $row[] = $Obj->indentcutoff;
                $row[] = $ArrRequestTypes[$Obj->approvaltype];
                $row[] = $Obj->contactname;
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->msamplerequestmodel->count_all(),
                "recordsFiltered" => $this->msamplerequestmodel->count_filtered(),
                "data" => $data,
            );
    ...
    public function indentlist() {
        ...
        if($VarFrom == 1) {
            ...
            $ArrList = $this->indentsmodel->fabIndentlistDatatables();
            $data = array();
            $ArrStatus = unserialize(ARRSTATUS);
            $ArrRequestType        = unserialize(ARRREQUESTTYPE);
            foreach ($ArrList as $Obj) {
                ...
                $row[] = '<a href="'.base_url(CNFCOMPANY.'mfabricuser/fabindentdetails').'/'.urlencode(base64_encode($Obj->id)).'">'.$Obj->isriorcode.'</a>';
                $row[] = $Obj->brandname;
                $row[] = $Obj->queueno;
                $row[] = $Obj->fabissuedto;
                $row[] = $Obj->fab_mat_ind_ref_no;
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->indentsmodel->fabIndentListCountAll(),
                "recordsFiltered" => $this->indentsmodel->fabIndentListCountFiltered(),
                "data" => $data,
            );
            echo json_encode($output);
        }
        else {
            $this->load->view('fabric/fabricindentlist',array());
        }
    }

    public function getFabIndents() {
        ...
        if (!empty($ArrFabIndentDetails)) {
            foreach ($ArrFabIndentDetails as $value) {
                $fabIndentDetails[$value->id] = $value->gridindent;
            }
            echo json_encode(array('errcode' => 1, 'fabIndentGrid' => $fabIndentDetails));
        }
    }

    public function fabindentdetails() {
        ...
        if ($VarRequestId >= 1) {
            ...
            $this->load->view('fabric/fabindentdetail', $ArrData);
        }
    }
}