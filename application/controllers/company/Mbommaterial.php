...
class mbommaterial extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->limit = LIMITPERPAGE;
        $this->load->model(CNFCOMPANY . "mbommaterialmodel");
    ...
    public function addedit() {
        ...
        if ($VarHashId <> '' && base64_decode(urldecode($VarHashId))) {
            ...
            $ArrResults = $this->mbommaterialmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addeditbommaterial', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarYarn <> '') {
            $ArrData = array('id' => $VarId, 'misc_name' => $VarYarn, 'misc_type' => 3, 'companyid' => $VarCompanyId, 'status' => $VarStatus,
                'updatedby' => $VarUpdatedBy, 'dateupdated' => date('Y-m-d H:i:s'));
            if (empty($VarId)) {
                $ArrData['datecreated'] = date('Y-m-d H:i:s');
            }
            $ArrResult = $this->mbommaterialmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'mbommaterial/manage/';
            $config['total_rows'] = $this->mbommaterialmodel->fnCount($VarYarn, $VarStatus);
            ...
            $ArrList = $this->mbommaterialmodel->fnList($VarYarn, $VarStatus, $this->limit, $offSet, $sortBy, $sortOrder)->result();
            $data['pagination'] = $this->pagination->create_linkswithajax('YarnCount');
            $i = 0;
            $ArrFnlList = array();
            foreach ($ArrList as $Obj) {
                $ArrFnlList[$i]['id'] = $Obj->id;
                $ArrFnlList[$i]['y'] = $Obj->misc_name;
            ...
            $this->load->view(CNFCOMPANY . 'managebommaterial', array('ArrStatus' => $ArrStatus));
        }
    }
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->mbommaterialmodel->fnChangeComStat($VarCid, $VarType);
        }
