...
class myarnname extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->limit = LIMITPERPAGE;
        $this->load->model(CNFCOMPANY . "myarnmodel");
    ...
    public function addedit() {
        ...
        if ($VarHashId <> '' && base64_decode(urldecode($VarHashId))) {
            ...
            $ArrResults = $this->myarnmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addedityarnname', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarYarn <> '') {
            ...
            $ArrResult = $this->myarnmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'myarnname/manage/';
            $config['total_rows'] = $this->myarnmodel->fnCount($VarYarn, $VarStatus);
            ...
            $ArrList = $this->myarnmodel->fnList($VarYarn, $VarStatus, $this->limit, $offSet, $sortBy, $sortOrder)->result();
            $data['pagination'] = $this->pagination->create_linkswithajax('Yarnname');
            $i = 0;
            $ArrFnlList = array();
            ...
            $this->load->view(CNFCOMPANY . 'manageYarnname', array('ArrStatus' => $ArrStatus));
        }
    }
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->myarnmodel->fnChangeComStat($VarCid, $VarType);
        }
