...
class Mconsignor extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "mconsignormodel");
    ...
    public function addedit() {
        ...
        if ($VarHashedId <> '' && base64_decode(urldecode($VarHashedId))) {
            ...
            $ArrResults = $this->mconsignormodel->fnGetInfo('', '', $VarId);
            $ArrData['ArrBasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]['id'];
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addeditconsignor', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarContactName <> '') {
            $ArrUpdateData = array('id' => $VarId, 'address' => $VarVendorAddr, 'emailid' => $VarEmailid, 'mobile' => $VarMobile, 'phone' => $VarPhone, 'companyid' => $VarCompanyId,
                'name' => $VarContactName, 'gstno' => $VarGstno, 'iecode' => $VarIecode, 'bankname' => $VarBankname, 'accountname' => $VarAccountname,
            ...
            $ArrResult = $this->mconsignormodel->saveInfo($ArrUpdateData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manageconsignor() {
        $VarFrom = xssclean($this->input->post('rfrom'));
        $VarContatname = xssclean($this->input->post('cn'));
        $VarMobileNo = xssclean($this->input->post('m'));
        $VarEmailId = xssclean($this->input->post('e'));
        $VarStatus = xssclean($this->input->post('s'));
        ...
        $ArrDbCols = array('name', 'emailid', 'phone', 'mobile', 'contactname', 'status', 'dateupdated');
        if ($VarFrom == 1) {
            $VarURLSegment = 4;
            $this->load->library('pagination');
            $config['base_url'] = base_url() . CNFCOMPANY . 'mconsignor/manageconsignor/';
            $config['total_rows'] = $this->mconsignormodel->fnCount($VarMobileNo, $VarContatname, $VarEmailId, $VarStatus, $VarAfilter);
            ...
            $ArrFabricTypeList = $this->mconsignormodel->fnList($VarMobileNo, $VarContatname, $VarEmailId, $VarStatus, $VarAfilter, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            foreach ($ArrFabricTypeList as $ObjFabric) {
                ...
                $ArrFnlList[$i]['vcn'] = $ObjFabric->name;
            ...
            $this->load->view(CNFCOMPANY . 'manageconsignor');
        }
    }
    function changemStatus() {
        ...
        if ($VarActDeactOption <> '' && $VarCid <> '') {
            if ($this->mconsignormodel->fnChangeComStat($VarCid, $VarActDeactOption))
