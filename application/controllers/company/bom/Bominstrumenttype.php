...
class Bominstrumenttype extends CI_Controller {
    public function __construct() {
        ...
        $this->load->model(CNFCOMPANY . "Bominstrumenttypemodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->Bominstrumenttypemodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'bom/instrument/addeditinstrument', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            ...
            $ArrResult = $this->Bominstrumenttypemodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'bom/Bominstrumenttype/manage/';
            $config['total_rows'] = $this->Bominstrumenttypemodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->Bominstrumenttypemodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'bom/instrument/instrumentlist', array('ArrStatus' => $ArrStatus));
        }
    }
    ...
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->Bominstrumenttypemodel->fnChangeComStat($VarCid, $VarType);
        }
