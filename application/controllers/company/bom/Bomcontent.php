...
class Bomcontent extends CI_Controller {
    public function __construct() {
        ...
        $this->load->model(CNFCOMPANY . "Bomcontentmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->Bomcontentmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'bom/content/addeditcontent', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            ...
            $ArrResult = $this->Bomcontentmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'bom/Bomcontent/manage/';
            $config['total_rows'] = $this->Bomcontentmodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->Bomcontentmodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'bom/content/contentlist', array('ArrStatus' => $ArrStatus));
        }
    }
    ...
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->Bomcontentmodel->fnChangeComStat($VarCid, $VarType);
        }
