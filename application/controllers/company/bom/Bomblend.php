...
class Bomblend extends CI_Controller {
    public function __construct() {
        ...
        $this->load->model(CNFCOMPANY . "Bomblendmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->Bomblendmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'bom/blend/addeditblend', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            ...
            $ArrResult = $this->Bomblendmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'bom/Bomblend/manage/';
            $config['total_rows'] = $this->Bomblendmodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->Bomblendmodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'bom/blend/blendlist', array('ArrStatus' => $ArrStatus));
        }
    }
    ...
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->Bomblendmodel->fnChangeComStat($VarCid, $VarType);
        }
