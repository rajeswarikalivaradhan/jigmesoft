...
class Bommaterial extends CI_Controller {
    public function __construct() {
        ...
        $this->load->model(CNFCOMPANY . "Bom1materialmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->Bom1materialmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'bom1/material/addeditmaterial', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            ...
            $ArrResult = $this->Bom1materialmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'bom1/Bommaterial/manage/';
            $config['total_rows'] = $this->Bom1materialmodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->Bom1materialmodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'bom1/material/materiallist', array('ArrStatus' => $ArrStatus));
        }
    }
    ...
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->Bom1materialmodel->fnChangeComStat($VarCid, $VarType);
        }
