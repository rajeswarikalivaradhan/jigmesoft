...
class Bomcontent extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "Bom1contentmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->Bom1contentmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'bom1/content/addeditcontent', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            $ArrData = array('id' => $VarId, 'type' => 3, 'description' => $VarBlend, 'companyid' => $this->companyid, 'status' => $VarStatus,
                'updatedby' => $this->userid, 'dateupdated' => date('Y-m-d H:i:s'));
            if (empty($VarId)) {
                $ArrData['datecreated'] = date('Y-m-d H:i:s');
            }
            $ArrResult = $this->Bom1contentmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'bom1/Bomcontent/manage/';
            $config['total_rows'] = $this->Bom1contentmodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->Bom1contentmodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'bom1/content/contentlist', array('ArrStatus' => $ArrStatus));
        }
    }
    ...
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->Bom1contentmodel->fnChangeComStat($VarCid, $VarType);
        }
