...
class Bominstrumenttype extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "Bom1instrumenttypemodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->Bom1instrumenttypemodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'bom1/instrument/addeditinstrument', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            $ArrData = array('id' => $VarId, 'type' => 1, 'description' => $VarBlend, 'companyid' => $this->companyid, 'status' => $VarStatus,
                'updatedby' => $this->userid, 'dateupdated' => date('Y-m-d H:i:s'));
            if (empty($VarId)) {
                $ArrData['datecreated'] = date('Y-m-d H:i:s');
            }
            $ArrResult = $this->Bom1instrumenttypemodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'bom1/Bominstrumenttype/manage/';
            $config['total_rows'] = $this->Bom1instrumenttypemodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->Bom1instrumenttypemodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'bom1/instrument/instrumentlist', array('ArrStatus' => $ArrStatus));
        }
    }
    ...
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->Bom1instrumenttypemodel->fnChangeComStat($VarCid, $VarType);
        }
