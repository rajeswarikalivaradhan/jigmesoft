...
class mbomblend extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "mbomblendmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->mbomblendmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addeditbomblend', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            ...
            $ArrResult = $this->mbomblendmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'mbomblend/manage/';
            $config['total_rows'] = $this->mbomblendmodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->mbomblendmodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'managebomblend', array('ArrStatus' => $ArrStatus));
        }
    }
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->mbomblendmodel->fnChangeComStat($VarCid, $VarType);
        }
