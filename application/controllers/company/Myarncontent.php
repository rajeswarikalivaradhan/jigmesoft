...
class myarncontent extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "myarncontentmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->myarncontentmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addedityarncontent', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarContent <> '') {
            ...
            $ArrResult = $this->myarncontentmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'myarncontent/manage/';
            $config['total_rows'] = $this->myarncontentmodel->fnCount($VarContent, $VarStatus);
            ...
            $ArrList = $this->myarncontentmodel->fnList($VarContent, $VarStatus, $this->limit, $offSet, $sortBy, $sortOrder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'manageyarncontent', array('ArrStatus' => $ArrStatus));
        }
    }
    function changeStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->myarncontentmodel->fnChangeComStat($VarCid, $VarType);
        }
