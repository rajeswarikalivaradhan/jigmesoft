...
class Mtestauth extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "Testauthmodel");
    ...
    public function addedit() {
        ...
        if ($VarHashedId <> '' && base64_decode(urldecode($VarHashedId))) {
            ...
            $ArrResults = $this->Testauthmodel->fnGetInfo('', '', $VarId);
            $ArrData['ArrBasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]['id'];
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addedit_testingauth', $ArrData);
    }
    public function updateInfo()
    {
        ...
        if ($VarVendor_name <> '') {
            ...
            $ArrResult = $this->Testauthmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'mtestauth/manage';
            $config['total_rows'] = $this->Testauthmodel->fnCount($VarVendor_name, $VarContactPerson, $VarEmailId, $VarMobno,$VarVendorCategory,$Varprimarypdtline,$VarStatus, $VarAfilter);
            ...
            $ArrList = $this->Testauthmodel->fnList($VarVendor_name, $VarContactPerson, $VarEmailId,$VarMobno,$VarVendorCategory,$Varprimarypdtline, $VarStatus, $VarAfilter, $this->limit, $offset, $sortby, $sortorder)->result();
            $data['pagination'] = $this->pagination->create_linkswithajax('Testauthdet');
            $i = 0;
            $ArrFnlList = array();
            ...
            $this->load->view(CNFCOMPANY . 'manage_testingauth');
        }
    }
    function changeStatus() {
        ...
        if ($VarRfrom == 1) {
            ...
            if ($VarStatus <> '' && $VarCid <> '') {
                ...
                if ($this->db->update(KN_MASTER_TESTINGAUTHORITY, array('status' => $VarStatus))) {
