...
class Mfabvendor extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "Mfabricvendormodel");
    ...
    public function addedit() {
        ...
        if ($VarHashedId <> '' && base64_decode(urldecode($VarHashedId))) {
            ...
            $ArrResults = $this->Mfabricvendormodel->fnGetInfo('', '', $VarId);
            $ArrData['ArrBasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]['id'];
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addedit_fabricvendor', $ArrData);
    }
    public function updateInfo()
    {
        ...
        if ($VarVendor_name <> '') {
            ...
            $ArrResult = $this->Mfabricvendormodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'mfabvendor/manage';
            $config['total_rows'] = $this->Mfabricvendormodel->fnCount($VarVendor_name, $VarContactPerson, $VarEmailId, $VarMobno,$VarVendorCategory,$Varprimarypdtline,$VarStatus, $VarAfilter);
            ...
            $ArrList = $this->Mfabricvendormodel->fnList($VarVendor_name, $VarContactPerson, $VarEmailId,$VarMobno,$VarVendorCategory,$Varprimarypdtline, $VarStatus, $VarAfilter, $this->limit, $offset, $sortby, $sortorder)->result();
            $data['pagination'] = $this->pagination->create_linkswithajax('Fabricvendordet');
            $i = 0;
            $ArrFnlList = array();
            ...
            $this->load->view(CNFCOMPANY . 'manage_fabricvendor');
        }
    }
    function changeStatus() {
        ...
        if ($VarRfrom == 1) {
            ...
            if ($VarStatus <> '' && $VarCid <> '') {
                ...
                if ($this->db->update(KN_MASTER_FABRICVENDOR, array('status' => $VarStatus))) {
