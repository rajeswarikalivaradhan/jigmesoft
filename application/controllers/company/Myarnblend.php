...
class myarnblend extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFCOMPANY . "myarnblendmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->myarnblendmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFCOMPANY . 'addedityarnblend', $ArrData);
    }
    public function updateInfo() {
        ...
        if ($VarBlend <> '') {
            ...
            $ArrResult = $this->myarnblendmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFCOMPANY . 'myarnblend/manage/';
            $config['total_rows'] = $this->myarnblendmodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->myarnblendmodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFCOMPANY . 'manageyarnblend', array('ArrStatus' => $ArrStatus));
        }
    }
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->myarnblendmodel->fnChangeComStat($VarCid, $VarType);
        }
