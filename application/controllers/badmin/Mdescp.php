...
class Mdescp extends CI_Controller {
    public function __construct() {
        parent::__construct();
        fnIfCheckUserLoggedIn();
        $this->load->model(CNFBADMIN . "Descriptionmodel");
    ...
    public function addedit() {
        ...
        if (is_numeric($VarId)) {
            $ArrResults = $this->Descriptionmodel->fnGetInfo('', '', $VarId);
            $ArrData['BasicInfo'] = $ArrResults[0];
            $ArrData['VarId'] = $ArrResults[0]->id;
        } else {
        }
        $this->load->view(CNFBADMIN . 'description/addedit_form', $ArrData);
    }
    public function updateInfo() {
        $ArrResult = array();
        $VarBlend = xssclean($this->input->post('d'));
        $VarStatus = xssclean($this->input->post('s'));
        $VarId = xssclean($this->input->post('id'));
        if ($VarBlend <> '') {
            $ArrData = array('id' => $VarId, 'description' => $VarBlend, 'companyid' => $this->companyid, 'status' => $VarStatus,
                'updatedby' => $this->userid, 'dateupdated' => date('Y-m-d H:i:s'));
            if (empty($VarId)) {
                $ArrData['datecreated'] = date('Y-m-d H:i:s');
            }
            $ArrResult = $this->Descriptionmodel->saveInfo($ArrData);
        } else {
            $ArrResult['errcode'] = -1;
    ...
    public function manage() {
        ...
        $VarBlend = trim(xssclean($this->input->post('d')));
        $VarStatus = xssclean($this->input->post('s'));
        ...
        if ($VarFrom == 1) {
            ...
            $config['base_url'] = base_url() . CNFBADMIN . 'mdescp/manage/';
            $config['total_rows'] = $this->Descriptionmodel->fnCount($VarBlend, $VarStatus);
            ...
            $ArrList = $this->Descriptionmodel->fnList($VarBlend, $VarStatus, $this->limit, $offset, $sortby, $sortorder)->result();
            ...
            $this->load->view(CNFBADMIN . 'description/descp_list', array('ArrStatus' => $ArrStatus));
        }
    }
    ...
    function changemStatus() {
        ...
        if ($VarType <> '' && $VarCid <> '') {
            $this->Descriptionmodel->fnChangeComStat($VarCid, $VarType);
        }
