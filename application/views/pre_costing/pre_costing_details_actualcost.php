...
//            });
    ...
    function preCostingGrid(component_id,grid_name)
    {
        ...
        MakeAsynPostRequest(base_path + "preCosting/preCostingColumns", "enquiry_id=" + enquiry_id + '&component_id='+component_id + '&grid_unique_id=' + grid_unique_id, "json", function(data) {
                ...
                //            fabric_validate1=validategridFields('garment_parts', data);
                //            alert(fabric_validate1);
                //           if (fabric_validate1 == 1) {
   
                //           } else if (fabric_validate1 > 1 && fabric_validate1 <= 5) {
                //              alert ("Please Enter Fabric Cost");
                //              return false;
                //          } else {
                ...
                if(validate == 0 && actual_cost_validate==''){
                    swalWithBootstrapButtons.fire(
                        ...
                        // commented by me on 20/03/23
                            ...
                            // swalWithBootstrapButtons.fire({
                            //     title: 'Warning',
                            //     text: (actual_cost_validate==1)?"Please fill all the fields of Fabric Cost in component details.":"Please fill all the fields to continue.",
                            //     icon: 'warning',
                            //     width:460,
                            //     confirmButtonText: 'OK',
                            //     customClass: {
                            ...
                            //     }
                            // })
                            if(actual_cost_validate==1){
                                swalWithBootstrapButtons.fire({
                                    title: 'Warning',
                                    text: "Please fill all the fields of Fabric Cost in component details.",
                                    icon: 'warning',
                                    width:460,
                                    confirmButtonText: 'OK',
                                    customClass: {
                            ...
                            else if(actual_cost_validate>=2 && actual_cost_validate<=6){
                               swalWithBootstrapButtons.fire({
                               title: 'Warning',
                              text:(
                                         (actual_cost_validate == 2)
    ? "Please ensure all fields under 'Embellishment Cost' are filled in and saved."
    : (actual_cost_validate == 3)
      ? "Please ensure all fields under 'BOM (Article - 1) Cost' are filled in and saved."
      : (actual_cost_validate == 4)
        ? "Please ensure all fields under 'BOM (Article - 2) Cost' are filled in and saved."
        : (actual_cost_validate == 5)
          ? "Please ensure all fields under 'cmt Cost' are filled in and saved."
          : (actual_cost_validate == 6)
            ? "Please ensure all fields under 'other' are filled in and saved."
            : "Please fill all the fields of Actual Cost Per Garment in component details."
                                    ),
                               icon: 'warning',
                               width: 460,
                               showCancelButton: true,
     ...
     const $target = $('#loader');
    if ($target.length) {
      const offset = $target.offset().top - 60; // adjust -60 if you have a fixed header
