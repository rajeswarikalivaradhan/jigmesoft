...
// $address = $company_data['companyAddress']; // your input string

// $parts = explode(',', $address);

// // Take the first 3 items
// $first_part = array_slice($parts, 0, 3);

// // Take the remaining items
// $remaining_part = array_slice($parts, 3);

// // Convert back to comma-separated strings
// $first_address = implode(',', $first_part);
// $remaining_address = implode(',', $remaining_part);
...
<table width="100%"  class="table">
  ...
  <thead>
     ...
     <th width="4%">S.No.</th>
      ...
      <th width="12%">Blend (%) / Content / Material  </th>
 ...
 <?php if (!empty($newpurchase_data['withinStateDetails'])): ?>
    <?php $i = 1; // counter starts from 1 ?>
    <?php foreach ($newpurchase_data['withinStateDetails'] as $row):    
        $total_amount = $row[12]; 
        $sub_total = $row[15];
        ...
        <tr width="100%">
            ...
            <td style="text-align:center;"><?php echo $i++; ?></td>
             <td style="text-align:left !important; padding:4px !important;"><?php echo htmlspecialchars($row[2]); ?></td>
            ...
            <td style="text-align:center !important; padding:4px !important;" ><?php echo htmlspecialchars($row[8]); ?></td>
            <td style="text-align:right !important; padding:4px !important;"><?php echo number_format($row[9]); ?></td>
            <td style="text-align:center"><?php echo htmlspecialchars($row[10]); ?></td>
            <td style="text-align:right !important; padding:4px !important;"><?php echo htmlspecialchars($row[11]); ?></td>
            <td style="text-align:right !important; padding:5px !important;"><?php echo htmlspecialchars(number_format($row[12],2)); ?></td>
            <td style="text-align:right !important; padding:4px !important;"><?php echo htmlspecialchars($row[13]); ?></td>
            <td style="text-align:right !important; padding:4px !important;"><?php echo number_format($row[14],2); ?></td>
            <td style="text-align:right !important; padding:4px !important;"><?php echo htmlspecialchars(number_format($row[15],2)); ?></td>
...
<?php endif; ?>
 ...
 <table width="100%" border="0"  height="30" align="center" style="table-layout: fixed; border-bottom:1px solid black;border-right:1px solid black;border-left:1px solid black;border-collapse:collapse;padding-bottom: 20px;">
        <tr >
   <td width="71%"  align="right" style="border-bottom: 1px dotted black;"><strong> Total:</strong> </td>
          
          
          <td  width="9%" align="right" style="border-bottom: 1px dotted black;"><strong><?php echo number_format($tot_amt,2); ?></strong></td>
          <td width="5%"   style="border-bottom: 1px dotted black;"></td>

          <td  width="6%" style="border-bottom: 1px dotted black;"></td>
          
          <td  width="9%" align="right" style="border-bottom: 1px dotted black;padding-right:4px;"><strong><?php echo number_format($all_tot,2); ?></strong></td>
...
<table width="100%" style="border-collapse: collapse; border: none; border-right: 1px solid black; border-left: 1px solid black;">
  <tr>
    <td width="13%" height="20" style="padding-top: 15px;  ">
      <b>&nbsp;&nbsp;Amount In Words:</b>
    </td>
    <td width="50%" height="20" style=" padding-top: 15px; ">
      <?php echo $purchase_datas['amount_in_words']; ?>
    </td>
  </tr>
  <tr>
    <td width="15%" height="30" style=" padding-top: 1px; padding-bottom: 10px;">
      <b>&nbsp;&nbsp;Payment Terms:</b>
    </td>
...
<table width="100%" height="50"  style="border-collapse:collapse;border-bottom:1px solid; solid black;border-right:1px solid black;border-left:1px solid black;">
 ...
 <script>
        // Remove page title before printing to avoid showing in print header
