...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'request/bomrequest/getMgmtBillPaidListBOM2',
			type:'POST',
...
$('#refreshBtn').on('click', function () {
  ...
  $('#searchButton').on('click', function() {
	...
	var vendor_name = $('#vendor_name').val().toLowerCase();
	 var Invoice_no   = $('#Invoice_no').val().toLowerCase();
	var Invoice_value = $('#Invoice_value').val().toLowerCase();
    ...
    table
      ...
      .column(2).search(brandId)
	   .column(3).search(pi_ref_no)
      .column(4).search(vendor_name)
	   .column(5).search(Invoice_no)
      .column(7).search(Invoice_value)
