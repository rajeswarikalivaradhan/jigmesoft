...
<div class="card-header pt-2 pb-3 pl-0 bgc-white border-0 " >
...
<div class="d-flex gap-2">
    <?php if($userType == 3){  ?>
                      <a class="btn btn-sm btn-royal-blue btn-text-slide-x" href="<?= base_url('merchant/manageWip'); ?>">
        <i class="fa fa-circle-o"></i>&nbsp;Back
    </a>&nbsp;&nbsp;
   
                 <?php }else{ ?>
                    <a class="btn btn-sm btn-royal-blue btn-text-slide-x" href="<?= base_url('management/manageWip'); ?>">
        <i class="fa fa-circle-o"></i>&nbsp;Back
    </a>&nbsp;&nbsp; 
...
<ul class="pt-2 nav nav-tabs nav-tabs-simple nav-tabs-static nav-tabs-faded flex-nowrap radius-0 px-3 pt-2 nav-tabs-scroll is-scrollable pad-l-0-i" id="component" role="tablist">
    <?php
    ...
    foreach ($components as $key => $component)
    {
        ...
        <div class="tab-pane fade <?= $key == 0 ? 'active show' : '' ?>" id="component_tab_content_<?= $component['component_id'] ?>"
            ...
            <ul class="nav nav-justified nav-tabs nav-tabs-static nav-tabs-faded radius-b-0 radius-t-1 overflow-hidden" id="component_sub_tab_<?= $component['component_id'] ?>" role="tablist">
                ...
                <li class="nav-item nav-item-r-border">
                    <a class="btn btn-light-lightgrey btn-a-purple border-0 radius-b-0 radius-t-1  btn-sm f-13 py-2 " id="dyeing_cost_<?= $component['component_id'] ?>" data-toggle="pill"
                ...
                <li class="nav-item nav-item-r-border">
                    <a class="btn btn-light-lightgrey btn-a-purple border-0 radiradius-b-0 radius-t-1  btn-sm f-13 py-2" id="Bom_art1_cost_<?= $component['component_id'] ?>" data-toggle="pill"
                ...
                <li class="nav-item nav-item-r-border">
                    <a class="btn btn-light-lightgrey btn-a-purple border-0 radius-b-0 radius-t-1  btn-sm f-13 py-2" id="Bom_art2_cost_<?= $component['component_id'] ?>" data-toggle="pill"
                ...
                <li class="nav-item nav-item-r-border">
                    <a class="btn btn-light-lightgrey btn-a-purple border-0 radius-b-0 radius-t-1  btn-sm f-13 py-2" id="cip_cost_<?= $component['component_id'] ?>" data-toggle="pill"
               ...
               <li class="nav-item">
                    <a class="btn btn-light-lightgrey btn-a-purple border-0 radius-b-0 radius-t-1  btn-sm f-13 py-2" id="test_<?= $component['component_id'] ?>" data-toggle="pill"
                       href="#test_content_<?= $component['component_id'] ?>" role="tab"
                       aria-controls="test_content_<?= $component['component_id'] ?>" aria-selected="false">Test</a>
                </li>
            </ul>
            <div class="tab-content p-0 border-0 brc-grey-l1" id="component_sub_tab_<?= $component['component_id'] ?>_content">
                ...
                <div class="tab-pane fade pr-0 pl-0" id="dyeing_cost_content_<?= $component['component_id'] ?>" role="tabpanel"
                    ...
                    <div class="card border-0 mar-b-20">
                        <div class="mt-3 mb-2 pl-0 ml-2 text-royal-blue f-16">Dyeing & Processing Cost - <span style="color:grey"><?= $component['dying_type'] == 1 ?  'Solid Color' : 'Yarn Dyeing & Multi Color' ?></span></div>
                ...
                <div class="tab-pane fade" id="other_exp_content_<?= $component['component_id'] ?>" role="tabpanel"
                    ...
                    <div class="col-12 text-right pr-3 py-3">
                        <button role="tab"
                         ...
                         <span class="mar-lr-10 bf-af-pg">...</span>
                        <button class="btn btn-sm btn-royal-blue btn-text-slide-x px-3 pag-active af-bf">9</button>
                        <span class="mar-lr-10 bf-af-pg">...</span>
                         <button role="tab" aria-controls="component_tab_content_<?= $component['component_id'] ?>"
                                onclick='$("#test_<?= $component['component_id'] ?>").trigger("click")'
                                class="btn btn-sm btn-royal-blue btn-text-slide-x">
                            Next <i class="btn-text-2 move-right fa fa-arrow-right text-120 align-text-bottom mr-2"></i>
                        </button>
                    </div>
                ...
                <div class="tab-pane fade" id="test_content_<?= $component['component_id'] ?>" role="tabpanel"
                    ...
                    <div class="col-12 text-right pr-3 py-3">
                        ...
                        <span class="mar-lr-10 bf-af-pg">...</span>
                        <button class="btn btn-sm btn-royal-blue btn-text-slide-x px-3 pag-active">10</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    if (document.referrer !== "") {
      ...
      window.location.href = "fallbackPage.html"; // fallback if no referrer
    }
  }
...
//            });
    ...
    function loadGrids(){
        $.each(components,function (index,component) {
            ...
            preCostingGrid(component.id,'test_grid');
                   ...
                   let options = {
                    ...
                    $('#' + grid_name + '_btn_'+ component_id).click(function (){
                        ...
                        swalWithBootstrapButtons.fire(
                            {
                                title: 'Are you sure want to save the '+(getGridNameId(grid_name))+' details ?',
                            ...
                            if (result.value) {
                                k.submitData();
                                swalWithBootstrapButtons.fire({title: 'Saved!',text: 'Operation completed successfully.',type: 'success',icon: 'success',customClass: {'confirmButton': 'btn btn-info px-5'}});
         ...
         MakeAsynPostRequest(base_path + "preCosting/preCostingColumns", "enquiry_id=" + enquiry_id + "&component_id=" + component_id + "&grid_unique_id=" + grid_unique_id, "json", function(gridData) {
             ...
             gridData.forEach(function(value) { 
                ...
                if($('#'+combo_div_id).length===0){
                    comboHtml = '\
                    <div class="p-0 card border-0 mar-b-20">\
                      ...
                      <div class="card-footer clearfix bgc-white border-0 p-3"><div class="text-right col-12">\
                ...
                let options = {
                    ...
                    columns:[
                        ...
                        { type:'dropdown',title:'Yarn\nCounts', width:'6%', source: data.counts, filter: countFilter,align:'center' },
                        { type:'dropdown',title:'Dye. Special\nReq. If Any', width:'10%',source: data.sp_data,align:'center'},
    ...
    function iorCostGrid(enquiry_id)
    {
        ...
        let options = {
                ...
                updateTable: function (instance, cell, col, row, val, label, cellName) { 
                     ...
                     if(col === 5)
                     {
                        comm_value = ((parseFloat(invoice_val) * parseFloat(exch_rate)) / 10);
                        comm_value = numeral(comm_value).format('0.00');
                        $(cell).text(comm_value);
                        instance.jexcel.options.data[row][col] = comm_value;
                     }
