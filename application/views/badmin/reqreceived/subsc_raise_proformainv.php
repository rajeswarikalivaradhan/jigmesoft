                    ...
                    <div class="col-xs-12">
                        <h2 class="page-header text-royal-black mx-4" style="border-bottom:0px">Raise Proforma Invoice
                          <div class="pull-right">
                                            ...
                                            <input type="hidden" id="recent_active_proforma_id" value="<?php echo $recent_proforma_id;?>">
                                    ...
                                    <td class="ord-procs-cell">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="process-value" <?php if (!empty($addressstr)  && (strlen($addressstr)>=50)) { ?> style="padding:6px 6px 80px 6px!important;"  <?php } else { ?> style="padding:6px 6px 114px 6px!important;" <?php } ?>>
                                                <?php 
                                    ...
                                    <td class="ord-procs-cell">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <select class="cus-sel form-control js-example-basic-single" id="purchase_type" onchange="Cgst_Sgst_Grid_Draft(<?php echo $SubscriberId; ?>);">
                                                <option value="" >Select</option>
                                                <?php
                                                $purchase_type = unserialize(ARRPURCHASETYPE);
                                                if (array_key_exists(1, $purchase_type)) {
                                                    unset($purchase_type[1]);
                                                }
                                                foreach ($purchase_type as $key => $item)
                                                {
                                                    ?>
                                                    <option value="<?php echo $key ?>" <?php if (@$draftdata->purchasetype == $key) { echo "selected";} ?>>
                                                        <?php echo $item ?>
                                                    </option>
                                                    <?php
                                                }
                        ...
                        <div class="card border-0">
                            ...
                            <div class="card-footer clearfix bgc-white border-0 p-2">
                                ...
                                <div id="scgstbtn_grid">
                                    ...
                                    <div id="scgst_savedraft"><button class="btn btn-royal-blue pull-right mx-2 hide" id="cgst_sgst_proforma_grid_draftbtn" >Save as Draft</button></div>
                                </div>
                                <div id="igstbtn_grid">
                                 <button class="btn btn-info pull-right mx-2" id="igst_proforma_grid_btn" >Save</button>
                                 <div id="igst_savedraft"><button class="btn btn-royal-blue pull-right mx-2 hide" id="igst_proforma_grid_draftbtns" >Save as Draft</button></div>
...
<script type="text/javascript">
    ...
    <?php $serializedArr = json_encode(unserialize(ARRPURCHASETYPE)); ?>;
    var purchase_type_arr = <?php echo $serializedArr; ?>;
    //console.log(purchase_type_arr[2]);
    var master_proforma_id= $("#recent_active_proforma_id").val();
    ...
    let redirectpath = base_path + GlbBAdminFdr + 'msubscription/detview/'+ encodeURIComponent(base64_encode(lasturi));
...
//        total  = parseFloat(total).toFixed(2);
    ...
    {    var checkboxval = '';
        ...
        if ((purchasetype != '') && (purchasetype != 2 && purchasetype != 3 && master_proforma_id=='')) { 
                 msg =  "There is no active proforma to create supplementary invoice.";
        ...
        $("#cgst_sgst_proforma_grid").html("");
        MakeAsynPostRequest(base_path + "badmin/mreqrcved/getsupplquotedetails_cgst_sgst", "enquiry_id=" + enquiry_id + "&proformatype=" + 'SPI' + "&purchasetype=" + purchasetype + "&invotype=" + checkboxval, "json", function(data) {
            ...
            let k = new Vue({
                    ...
                    methods: {
                        submitsaveData: function () {
                            ...
                            var purchasetype=$('#purchase_type').val();
                            var master_proformaid=(purchasetype!=2 && purchasetype!=3)?master_proforma_id:'';
                            MakeAsynPostRequest(base_path + GlbBAdminFdr + "mreqrcved/updateproformaInfo",
                            "rfrom=1&draftstatus=2&invotype=" + checkboxval + "&validity=" + validity + "&purchasetype=" + purchasetype + "&terms=" + terms + "&invoice_no=" + invoice_no +  "&subscription_period=" + subscription_period +
                            "&subsciber_id=" + GlbsubscriberId + "&id=" + GlbId + "&master_proforma_id=" + master_proformaid + "&object=" + JSON.stringify(data), 
                            "json",function (data) {
                                    if (data != '') {
                                        if (data.errcode == '404') {
                                        fnCallSessionExpire();
            ...
            let pckgnamecheck = [];
            let qtychk=true;
            let packageCount = 0;  // Counter for selected packages
            if (data.length > 0) {
                for (i = 0; i < data.length; i++) {
                    if (data[i][0]) {
                        precheck[i] = data[i][0];
                        var selectedSourceIndex = data[i][0];
                        
                        let columnIndex = 0; // Assuming package names are stored in the second column (0-indexed)
    
                        // Find the matching package name using the package ID
                        let selectedPackage = options.columns[columnIndex].source.find(pkg => pkg.id == selectedSourceIndex);
                        
                        // Store the package name in precheck[]
                        pckgnamecheck[i] = selectedPackage ? selectedPackage.name : '';

                        //console.log(pckgnamecheck);
                    }
     ...
     // Check how many package-related options are chosen
     packageCount = pckgnamecheck.filter(name => name.toLowerCase().includes('package')).length;
     if (packageCount > 1) {
        swalWithBootstrapButtons.fire({
            title: 'Warning',
            html: "You can select only <b class='swal2-texts'>one package</b> at a time.",
            icon: 'warning',
            width: 460,
            confirmButtonText: 'OK',
            customClass: {
    ...
    if (hasDuplicatesResult == true) {
        ...
        return false;
            ...
            var msg = "";
            //  if(renewalchk==true){ 
            //     msg="Proforma Invoice for <b class='swal2-texts'>" + purchasetype +"</b> is pending.Duplication is not allowed.";
            // }
            if (validity == '') {
                msg = "Please fill <b class='swal2-texts'> prof. invoice validity </b> to continue.";
            } else if (purchasetype == '') {
                msg = "Please fill <b class='swal2-texts'> purchase type </b> to continue.";
            }else if ((purchasetype != '') && (purchasetype == 2 || purchasetype == 3)) {
                // proforma_exsist(purchasetype, GlbsubscriberId, function(result_set) {
                ...
                let result_set = proforma_exsist_sync(purchasetype, GlbsubscriberId);
                //console.log(result_set[0]['purchasetype']);
                if (result_set && result_set.length > 0) {
                     msg =  "Proforma Invoice for <b class='swal2-texts'>" + purchase_type_arr[result_set[0]['purchasetype']] + "</b> is pending. Duplication is not allowed.";
                }else{
                   if (subscription_period == '') {
                msg = "Please fill <b class='swal2-texts'> subscription period </b> to continue.";
            } else if (validate != 0) {
                msg = "Please fill all the fields to continue.";
            }  else if (purchasetype == 4 && subscription_period != '') {
                if ((precheck.includes('5') == false) && (precheck.includes('6') == true)) {
                ...
                } else if (precheck.includes('5') == false && (precheck.includes('6') == false)) {
                    msg = "Add both <b class='swal2-texts'>additional user and additional user rent</b> in description.";
                }else if(qtychk == false) {
                msg = "Only <b class='swal2-texts'>GB</b> value should be selected in details column.";
                } else if(terms == '') {
                msg = "Please fill <b class='swal2-texts'>terms & condition</b> to continue.";
                }
            }else if(qtychk == false) {
                msg = "Only <b class='swal2-texts'>GB</b> value should be selected in details column.";
            }else if(terms == '') {
         ...
         $("#igst_proforma_grid").html("");
         MakeAsynPostRequest(base_path + "badmin/mreqrcved/getsupplquotedetails_igst", "enquiry_id=" + enquiry_id + "&proformatype=" + 'SPI' + "&purchasetype=" + purchasetype + "&invotype=" + checkboxval, "json", function(data) {
            let min_dimensions = data.column.length;
            let options = {
                    data: data.data,
                    editable:true,
    ...
    function proforma_exsist(purchasetype, subscriberId, callback) {
    $.ajax({
        url: base_path + "badmin/mreqrcved/getexsistproforma",
        type: "POST",
        data: { 
        ...
        success: function (dataset) {
            if (dataset.success === 1) {
                callback(dataset.result);
            } else {
                callback([]); // No existing proforma found
            }
        },
...
function proforma_exsist_sync(purchasetype, subscriberId) {
    // Synchronous version of your proforma_exsist function
    // You should avoid synchronous AJAX requests if possible
    // Here, you might want to make an asynchronous request instead
    let result = []; 
    $.ajax({
        url: base_path + "badmin/mreqrcved/getexsistproforma",
        type: "POST",
        ...
        success: function (dataset) {
            if (dataset.success == 1) {
                result.push(dataset); // Assuming data is an array of results
            }else{
              result = [];   
            }
        }
