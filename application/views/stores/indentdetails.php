        ...
        <section class="content">
            <div class="row">
                <div class="col-md-12">
                            ...
                            <form class="form-horizontal" name="frmBasicInfo" id="frmBasicInfo" method="post"
                                        ...
                                        <div class="col-sm-8">
                                            ...
                                            <select name="" id="" class="form-control" disabled>
                                                ...
                                                foreach ($ArrApprovalType as $key => $approvaltype) {
                                                    ?>
                                                    <option
                                                        value="<?php echo $approvaltype ?>" <?php echo $ArrBasicInfo->approvaltype == $key ? 'selected' : '' ?>><?php echo $approvaltype ?></option> <?php
                                                }
                                                ?>
                                            </select>
                                        </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-8">
                                        <textarea readonly class="form-control" style="height: 64px">
                                            <?php if (!empty($caddeptremarks)) echo $caddeptremarks; ?>
                                        </textarea>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="box box-info">
                        <div class="box-body">
                            <div class="bomIndentHere"></div>
                            <form class="form-horizontal" id="frmBasicBomMatIndent">
                                <div class="box-body pdl0">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="BomMatIndRefNo" class="col-sm-4 control-label">Material Indent Ref. No.</label>
                                            <div class="col-sm-8">
                                                <input type="text" id="BomMatIndRefNo" class="form-control" readonly
                                                       style="height: 45px" value="<?php echo $ArrBasicInfo->bom_mat_ind_ref_no; ?>">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" readonly
                                                       value="<?php echo $ArrBasicInfo->bomissuedto; ?>">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" readonly
                                                       value="<?php echo dateTimeHelp($ArrBasicInfo->bomindentcutoffdatetime,false) ?>">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    MakeAsynPostRequest(base_path+"storesuser/getBomIndents","requestid="+GlbRequestId,"json",function (data) {
        console.log(data,'data');
        console.log(data.bomIndentGrid,'data.bomIndentGrid');
        var b = 1;
        $.each(data.bomIndentGrid,function (index,bomValue) {
            console.log(b,'b');
            console.log(index,'index');
            GlbIndentIds.push(index);
            console.log(bomValue,'bomValue');
            $(".bomIndentHere").append('<div id="gridBomIndent_'+b+'" class="moreBom table"></div>');
            StoresGridBomIndent = jexcel(document.getElementById('gridBomIndent_'+b), {
                columns: [
                    {type: 'text', title: 'Item Description / Content / Material', width: 300, readOnly: true},
                    {type: 'text', title: 'Gar. / Label Size', width: 50, wordWrap: true, readOnly: true},
                    ...
                    {type: 'text', title: 'UOM', width: 80, wordWrap: true, readOnly: true},
                    {type: 'checkbox', title: 'Select', width: 80, wordWrap: true},
                    {type:'hidden'}
                ],
	...
	function fnNewStockList() {
        var indcodt = $("#BomMatIndentCutoff").val();
        var indrefno = $("#BomMatIndRefNo").text();
        //var BomIndentKey = [];
        var BomIndent = [];
        //var CheckboxVal = [];
        for(var a = 1; a <= sampleRequestCount; a++) {
	        var gridBomIndent = $("#gridBomIndent_"+a).jexcel('getData');
            for(var i = 0; i < gridBomIndent.length; i++) {
                //console.log(gridBomIndent[i][10],'gridBomIndent[i][10]');
                if(gridBomIndent[i][8] == 1) {
                    /*BomIndentKey.push([gridBomIndent[i][0]+"|#|"+gridBomIndent[i][1]+"|#|"+gridBomIndent[i][2]+"|#|"+
                                        gridBomIndent[i][3]+"|#|"+gridBomIndent[i][4]+"|#|"+gridBomIndent[i][5]]);*/

                    BomIndent.push([gridBomIndent[i][0],gridBomIndent[i][1],gridBomIndent[i][2],gridBomIndent[i][3],
                        gridBomIndent[i][4],gridBomIndent[i][5],gridBomIndent[i][6],gridBomIndent[i][7],gridBomIndent[i][8]]);
                }
                //CheckboxVal.push(gridBomIndent[i][10]);
            }
        }
        var url = $(location).attr('href'); var lasturlpart = url.substr(url.lastIndexOf('/')+1);
        //console.log(CheckboxVal,'CheckboxVal');
        MakeAsynPostRequest(base_path+'storesuser/updateSend2StockListBomIndent',
            //"BomIndentKey="+JSON.stringify(BomIndentKey)+"&BomIndent="+JSON.stringify(BomIndent)+
            "BomIndent="+JSON.stringify(BomIndent)+
            //"&CheckboxVal="+JSON.stringify(CheckboxVal)+
            "&oid="+GlbOrderId+
            "&reqid="+lasturlpart+
            "&indentCutoff="+indcodt+
            "&indrefno="+indrefno+
            "&BomIndToStoresStockId="+JSON.stringify(GlbIndentIds),
            'json',fnSend2StockListRes);
	}

    function fnSend2StockListRes(data) {
        console.log(data,'data');
        if(data.id != '') {
            location.reload();
            //var currenturl = window.location.href;
            //GlbBomIndToStoresStockId = data.id;
            //fnRedirectPageTimeOut(currenturl+'/'+GlbBomIndToStoresStockId);
        }
    }
