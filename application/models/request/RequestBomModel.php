...
class RequestBomModel extends CI_Model
{
    ...
    public function getPurchaseRequestDetailss($id) {
        ...
        $type_sql = "SELECT * FROM tbl_request as a where a.enquiry_id='$id' AND a.type='3' AND a.purchase_req_type = 'BULK'  ";
        $type_data = $this->db->query($type_sql)->num_rows();
        
        $bulk_sql = "SELECT * FROM tbl_request_bom as a 
                INNER JOIN tbl_request as b ON a.request_id=b.request_id
                WHERE a.enquiry_id='$id' AND b.purchase_req_type = 'BULK' ";
...
/*************  ✨ Windsurf Command ⭐  *************/
/*******  8241226c-17df-4572-8844-5b27a7aae1e9  *******/
        ...
        if($mode == "add") {
            ...
            if($primaryId) {
                foreach($bom_data as $key => $value) {
                    ...
                    //$item_code = trim($Value[6]);
                    //$item_code = preg_replace('/\s*-\s*/', '-', $Value[6]);
                    //$item_code = preg_replace('/\s*-\s*/', '-', trim($Value[6]));
                    ...
                    $requestBOMValue['appr_item_code'] = $item_code;
    ...
    public function getMerchantBomQueueDetailss($enqId, $reqId) {
        $sql = "SELECT * FROM tbl_request_bom b
                ...
                WHERE c.request_id = " . $reqId . "  AND c.mgmt_approval = 1 AND c.deprt_approval = 1 ";
        $data = $this->db->query($sql)->result_array();
        //print_r($data); exit;
        ...
        $pisql = "SELECT * FROM tbl_request_bom b
                ...
                WHERE c.request_id = " . $reqId . " AND c.mgmt_approval = 1 AND c.deprt_approval = 1 and f.type_of_mode='M' GROUP BY b.request_bom_id ";
        ...
        foreach ($pidata as $key => $value) {
            if($value['pi_list_status'] == 0) {
                $status = 'PENDING';
            } else if($value['pi_list_status'] == 1) {
                $status = 'APPROVED';
            } else if($value['pi_list_status'] == 2) {
                $status = 'DECLINED';
            } else if($value['pi_list_status'] == 1) {
                $status = 'PENDING - RR';
            } else {
        ...
        $pisql1 = "SELECT b.*,a.*,e.pi_dt,e.pi_ref_queue_no,e.pi_appl_status,f.appr_dt,e.exp_dod FROM tbl_surplus_issued_details b
                ...
                LEFT JOIN tbl_request_status f on e.purchase_indent_id = f.purchase_indent_id
                WHERE c.request_id = " . $reqId . "  AND c.mgmt_approval = 1 AND c.deprt_approval = 1 and f.type_of_mode='M' AND e.purchase_indent_id != 0 GROUP BY b.bom_id ";
        ...
        $sql2 = "SELECT c.*, b.order_stock_status,b.received_uom ,b.supply_closure_status,b.supply_closure_date, SUM(b.received_qty) as received_qtys, c.plan_bom_qty FROM tbl_request_bom c
                ...
                WHERE c.request_id = " . $reqId . "  GROUP BY c.item_desc,c.garment_size,c.appr_item_code,c.appr_item_col_code,c.size_dim,c.uom ";
        ...
        //print_r($inhouseconsolidatedqtydetails);
        return $output;
    }
    ...
    public function getSupplyClosureListt($enqId, $reqId) {
        ...
        $sql2 = "SELECT * FROM tbl_request_bom b
                ...
                WHERE c.request_id = " . $reqId . "  AND c.order_stock_status=0 ";
        $data2 = $this->db->query($sql2)->result_array();
    ...
    {
         $sql = "SELECT a.*,b.ref_queue_no,c.req_status as pi_req_status, d.contactname as req_name,e.contactname as auth_name, b.type as type,f.contactname as pi_req_name, g.appr_status 
    ...
    {
         $sql = "SELECT a.*,b.type,b.ref_queue_no,c.req_status as pi_req_status, d.contactname as req_name,e.contactname as auth_name, f.contactname as pi_req_name, g.appr_status, h.contactname as pi_appr_name 
                FROM tbl_purchase_indent a 
                INNER JOIN tbl_request as b ON a.request_id=b.request_id
                INNER JOIN tbl_request_bom as c ON a.request_id=c.request_id
                INNER JOIN ".KN_USERS." d on b.req_by=d.id
    ...
    function getorderstock($enqId, $reqId, $pId)
    {
        $sql = "SELECT b.*,c.plan_bom_qty FROM tbl_bom_in_house b
                LEFT JOIN tbl_request_bom c on b.request_bom_id = c.request_bom_id
                WHERE b.purchase_indent_id = " . $pId . " and  b.order_stock_status=1  ";
        $req_data = $this->db->query($sql)->result_array();
        $sql1 = "SELECT b.*,c.plan_bom_qty FROM tbl_bom_in_house b
                LEFT JOIN tbl_request_bom c on b.request_bom_id = c.request_bom_id
                WHERE b.purchase_indent_id = " . $pId . "  AND b.mgmt_ovrd_status NOT IN (2, 3, 4)     ";
    ...
    public function getDraftPIRequestDetailss($enqId, $reqId) {
        ...
        $sql = "SELECT b.*,d.unit_rate,d.gst,d.gst_value,d.igst,d.igst_value,d.currency,d.sub_total,d.vendor_id FROM tbl_request_bom b
                ...
                LEFT JOIN tbl_request_purchase_indent d on b.request_bom_id=d.request_bom_id AND b.plan_bom_qty = d.qty
                WHERE b.request_id = " . $reqId . " AND b.pi_status=0 AND c.mgmt_approval = 1 AND c.deprt_approval = 1";
                
        $data = $this->db->query($sql)->result_array();

       

         //$sql2 = "SELECT * from tbl_bom_in_house where enquiryid=".$enqId."   group by appr_item_code";
          $sql2 = "SELECT * from tbl_request_bom where enquiry_id=".$enqId."   group by appr_item_code";
      ...
      foreach ($data2 as $key => $value1)
        {
             ...
             $itemcodes[$key] = [ $enqId, $value1['appr_item_code']];
...
//print_r($pi_data); exit;
        ...
        foreach ($data as $key => $value)
        {
            ...
            //$itemcode=$value['appr_item_code'];
            //$surplux_data_val = 0;
            // $surplux_sql = "SELECT *, item_code  FROM tbl_surplus_stock_details  where item_code='$itemcode'";
            // $surplux_data = $this->db->query($surplux_sql)->result_array();
            // if (empty($surplux_data)) {
            //     $surplux_data_val = 1;
            //  } else {
            //       $surplux_data_val = 1;
            //       }
...
//print_r($payment_sql); exit;
    ...
    $vendor_id = $this->db->where('purchase_indent_id',$pId)->get('tbl_purchase_indent')->row()->vendor_id;
       ...
       // $inv_nos = $this->db->where('purchase_indent_id',$pId)->group_by('invoice_no')->get('tbl_bom_in_house')->result_array();
        $inv_nos = $this->db
    ->select('invoice_no, SUM(total_invoice_value) as total_invoice_sum,dc_no,invoice_date,dc_date')
    ->where('purchase_indent_id', $pId)
    ->group_by('invoice_no')
    ->get('tbl_bom_in_house')
    ->result_array();
       ...
       // $inv_sql = "SELECT * FROM tbl_bom_in_house as a where a.purchase_indent_id='$pId' GROUP BY a.invoice_no";
        ...
        foreach($inv_nos as $key =>$value) {
            ...
            $pi_invno_total[] = [ 'id'=> $value['invoice_no'], 'item_id'=> $value['dc_no'], 'name'=> $value['total_invoice_sum'] ];
        ...
        $output['pi_invno_total'] = $pi_invno_total;
     ...
     public function getPIRequestSendDetailss_spi($enqId, $reqId, $pId) {
        $sql = "SELECT a.*,c.request_bom_id,c.spi_list_status,c.purchase_indent_id as pi_id,c.item_desc,c.bcm,c.garment_size,c.appr_item_code,c.appr_item_col_code,c.size_dim,c.uom,c.plan_bom_qty,c.requirement_uom FROM tbl_purchase_indent a
                INNER JOIN tbl_request b on a.request_id=b.request_id
                INNER JOIN tbl_request_bom c on a.purchase_indent_id=c.purchase_indent_id
                WHERE a.purchase_indent_id = " . $pId . " ";
        $data = $this->db->query($sql)->result_array();
        ...
        foreach ($data as $key => $value)
        {
            ...
            @$withinResult[$key] = [ 'add', $req_pi_data[$key]['request_purchase_indent_id'], '',$value['item_desc'], $value['bcm'],
                $value['garment_size'], $value['appr_item_code'], $value['appr_item_col_code'],
                $value['size_dim'], $value['uom'], $value['plan_bom_qty'], $value['requirement_uom'], $req_pi_data[$key]['unit_rate'], 
                $req_pi_data[$key]['amount'], $req_pi_data[$key]['gst'], $req_pi_data[$key]['gst_value'],$req_pi_data[$key]['sub_total'],$value['pi_id'],$value['request_bom_id'],$value['spi_list_status'],
            ];
    ...
    public function getManagementPIApprovalListt()
    {
        ...
        $sql = "SELECT a.*, a.log as recent_log,f.cutoff_date,f.auth_type, c.brandname, b.isriorcode, d.contactname as auth_name, e.req_dt, e.appr_type, e.request_type, e.log as logs, e.* FROM tbl_purchase_indent as a
                ...
                where f.type=3 and f.mgmt_approval=1 and a.pi_appl_status=1 and a.pi_list_status=0  and f.subscriberid = " . $this->db->escape($this->subscriberId)." ORDER BY e.log DESC";
    ...
    public function updateManagementPurchaseIndentAppll($eId, $reqId, $req_data, $inv_data, $amt_pay, $pId)
    {
        ...
        //if($queue_no == "") { $queue_no = 1; }
             ...
             $wipcount = $this->commonmodel->getbomrequestCountPI($subid);
    ...
    public function getFinanceReqRecListt()
    {
        $sql = "SELECT a.*, h.type,h.cutoff_date,h.supply_lead_time, c.brandname, b.isriorcode, e.*, e.log as logs, f.proforma_value, f.currency as pay_currency, f.pay_by_date, f.request_payment_id, g.vendorname FROM tbl_purchase_indent as a 
                ...
                left join tbl_request_payment as f on f.type_of_mode='M' and a.purchase_indent_id=f.purchase_indent_id
                left join kn_master_bom_vendor as g on a.vendor_id=g.id
                where h.type IN ('3','4') and h.mgmt_approval=1 and a.pi_list_status=1 and a.pi_appl_status=1 and e.type_of_mode='M'   and a.bill_paid_status = 0 and h.subscriberid = " . $this->db->escape($this->subscriberId)."ORDER BY e.log DESC";
        $result = $this->db->query($sql)->result_array();
    ...
    $results = [];
        $sql = "SELECT a.*, h.type,h.cutoff_date,h.supply_lead_time, c.brandname, b.isriorcode, e.*, e.log as logs, f.proforma_value, f.currency as pay_currency, f.pay_by_date, f.request_payment_id, g.vendorname FROM tbl_purchase_indent as a 
                ...
                left join tbl_request_payment as f on f.type_of_mode='M' and a.purchase_indent_id=f.purchase_indent_id
                left join kn_master_bom_vendor as g on a.vendor_id=g.id
                where h.type IN ('3') and h.mgmt_approval=1 and a.pi_list_status=1 and a.pi_appl_status=1 and e.type_of_mode='M'   and a.bill_paid_status = 0 and h.subscriberid = " . $this->db->escape($this->subscriberId)."ORDER BY e.log DESC";
        $result = $this->db->query($sql)->result_array();
      ...
      public function getFinanceReqRecListt_bom2()
    {
        $sql = "SELECT a.*, h.type,h.cutoff_date,h.supply_lead_time, c.brandname, b.isriorcode, e.*, e.log as logs, f.proforma_value, f.currency as pay_currency, f.pay_by_date, f.request_payment_id, g.vendorname FROM tbl_purchase_indent as a 
                ...
                left join tbl_request_payment as f on f.type_of_mode='M' and a.purchase_indent_id=f.purchase_indent_id
                left join kn_master_bom_vendor as g on a.vendor_id=g.id
                where h.type IN ('4') and h.mgmt_approval=1 and a.pi_list_status=1 and a.pi_appl_status=1 and e.type_of_mode='M'   and a.bill_paid_status = 0 and h.subscriberid = " . $this->db->escape($this->subscriberId)."ORDER BY e.log DESC";
        $result = $this->db->query($sql)->result_array();
    ...
    public function getMgmtBillPaidListt()
    {
        $sql = "SELECT a.*, a.flag as flags, h.type,h.cutoff_date,h.supply_lead_time, c.brandname, b.isriorcode, e.*, e.log as logs, a.log as clog, f.invoice_no, f.invoice_date, f.invoice_value, f.curency as pay_currency, f.pay_by_date, f.vendor_name FROM tbl_purchase_indent as a 
                ...
                where type IN ('3','4') and h.mgmt_approval=1 and a.pi_list_status=1 and a.pi_appl_status=1 and a.bill_paid_status=1 and e.type_of_mode='M'  and h.subscriberid = " . $this->db->escape($this->subscriberId)." GROUP BY a.purchase_indent_id ORDER BY e.log DESC";
        $result = $this->db->query($sql)->result_array();
    ...
    public function getNewItemDetailss($enqId, $reqId) {
     ...
     public function clearsurplusstockdatatransferr($enqId, $reqId, $pId, $req_data) {
                //$purid = $pId;
                //print_r($req_data);

                 foreach ($req_data as $key1 => $value)
        {


             $this->db->where_in('issued_id', $value[0])->where('draft_status', 1)->delete('tbl_surplus_issued_details');
     ...
     public function getstmstatus($VarEnqId, $reqId, $pId)
    {
        //$mi_bom_id = $this->db->where('request_id',$req_id)->get('tbl_mi_bom')->row()->mi_bom_id;
        
        $bomStatus = $this->db->where('pid',$pId)->where('draft_status',1)->get('tbl_surplus_issued_details')->result_array();
        //print_r($bomStatus);
        return $bomStatus;
    }

     public function getsplstatus($VarEnqId, $reqId, $pId)
    {
        //$mi_bom_id = $this->db->where('request_id',$req_id)->get('tbl_mi_bom')->row()->mi_bom_id;
        
        $bomStatus = $this->db->where('purchase_indent_id',$pId)->get('tbl_request_bom')->result_array();
        $bomCount = count($bomStatus);
        $bomStatus2 = $this->db->where('purchase_indent_id',$pId)->where('spi_list_status',1)->get('tbl_request_bom')->result_array();
         $bomCount2 = count($bomStatus2);
       
        //print_r($bomCount);
        //print_r($bomCount2);
        if($bomCount == $bomCount2)
        {
            return 1;
        }else{
            return 0;
     ...
     public function getsststatus_old($VarEnqId, $reqId, $pId)
    {
        //$mi_bom_id = $this->db->where('request_id',$req_id)->get('tbl_mi_bom')->row()->mi_bom_id;
        
        $bomStatus = $this->db->where('purchase_indent_id',$pId)->get('tbl_request_bom')->result_array();
        $bomCount = count($bomStatus);
        $bomStatus2 = $this->db->where('pid',$pId)->where('draft_status',1)->get('tbl_surplus_issued_details')->result_array();
         $bomCount2 = count($bomStatus2);
    ...
    public function updateOrderClosestatus($req_data, $enqId, $itemCode)
    {
       ...
       // print_r($req_data);
        ...
        //die();
         foreach($req_data as $key => $value) {
              $lotno = $value[3];
             $updateValue['stock_closed_status'] = $value[12];
             $updateValue['stock_closed_status_date'] = $this->mysqldatetime;
             //$updateValue['scrap_aproved'] = $value[10];
             $updateValue['approved_date'] = $this->mysqldatetime;
           
            $this->db->where('item_code', $itemCode);
            $this->db->where('lot_no', $lotno);
            $this->db->update('tbl_surplus_stock_details', $updateValue);
    ...
    public function getsststatus($VarEnqId, $reqId, $pId)
    {
        //$mi_bom_id = $this->db->where('request_id',$req_id)->get('tbl_mi_bom')->row()->mi_bom_id;
       
        $bomStatus2 = $this->db->where('pid',$pId)->where('draft_status',1)->where('save_status',1)->get('tbl_surplus_issued_details')->result_array();
         $bomCount2 = count($bomStatus2);
     ...
     public function getdcsststatus($VarEnqId, $reqId, $pId)
    {
        $mi_bom_id = $this->db->where('request_id',$req_id)->get('tbl_mi_bom')->row()->mi_bom_id;
        
        $bomStatus = $this->db->where('purchase_indent_id',$pId)->get('tbl_request_bom')->result_array();
        $bomCount = count($bomStatus);
        $bomStatus2 = $this->db->where('pid',$pId)->where('draft_status',0)->get('tbl_surplus_issued_details')->result_array();
        $bomCount2 = count($bomStatus2);

        $this->db->select_sum('plan_bom_qty');$this->db->where('purchase_indent_id', $pId);$query = $this->db->get('tbl_request_bom');
        $result = $query->row_array();
        $qtybomsum1 = $result['plan_bom_qty'];

        $this->db->select_sum('issued_qty');$this->db->where('pid', $pId)->where('draft_status',0);$query = $this->db->get('tbl_surplus_issued_details');
        $result1 = $query->row_array();
        $qtysstsum1 = $result1['issued_qty'];
     ...
     public function getspisavestatus($VarEnqId, $reqId, $pId)
    {
       
        $this->db->select('*'); // Or select specific columns instead of '*'
$this->db->where('pid', $pId)
         ->where('draft_status', 0)
         ->where('save_status', 0);
$query = $this->db->get('tbl_surplus_issued_details');
$result1 = $query->result_array();
        
        return $result1;
    }
     public function getspisavestatus1($VarEnqId, $reqId, $pId)
    {
...
$result1 = $query->result_array();
      ...
      public function getsurplusissuedetailss($enqId, $reqId, $pId) {
              $itemdescription=[];$garmentSize  =[];
       
        $pursql_1 = "SELECT * FROM tbl_request_bom WHERE purchase_indent_id = " . $pId . " AND spi_list_status = 1 ";
        $purdata_1 = $this->db->query($pursql_1)->result_array();
        //PRINT_R($pursql_1);
         foreach ($purdata_1 as $key => $Value)
        {
                   
                        
                       $item_code[$key] =  $Value['appr_item_code'];
                       $bom_qty[$key] =  $Value['plan_bom_qty'];
                         
                        $this->db->select_sum('issued_qty');
                        $this->db->where('pid', $pId)->where('item_code', $item_code[$key]);
                        $query = $this->db->get('tbl_surplus_issued_details');
                        $result = $query->row_array();
                        $qtybomsum = $result['issued_qty']; 
                           //print_r($bom_qty[$key]);
                            //print_r($qtybomsum);
                        //print_r("our"+$bom_qty[$key]+"issued"+$qtybomsum);
                        //print_r($bom_qty[$key]);

                   if($bom_qty[$key] == $qtybomsum){
                   }else{
                       $bom_id1[$key] = ['name'=>$Value['request_bom_id']];
                       $itemdescription_a[$key] = ['id'=>$Value['item_desc'] ,'item_id'=>$Value['item_desc'],'name'=>$Value['item_desc']];
                       $garmentSize_a[$key] = ['id'=>$Value['garment_size'],'item_id'=>$Value['item_desc'],'item_id'=>$Value['item_desc'],'name'=>$Value['garment_size']];
                       $itemCode_a[$key] = ['id'=>$Value['appr_item_code'],'item_id'=>$Value['garment_size'] ,'item_des'=>$Value['item_desc'],'name'=>$Value['appr_item_code']];
                       $itemColorCode_a[$key] = ['id'=>$Value['appr_item_col_code'],'item_id'=>$Value['appr_item_code'] , 'name'=>$Value['appr_item_col_code']];
                       $sizes_a[$key] = ['id'=>$Value['appr_item_code'],'size_id'=>$Value['garment_size'],'item_id'=>$Value['appr_item_col_code'] , 'name'=>$Value['size_dim']];
                       $uom_a[$key] = ['id'=>$Value['appr_item_code'],'size_id'=>$Value['garment_size'],'name'=>$Value['uom']];
                       $bomqty[$key] = ['id'=>$Value['plan_bom_qty'],'size_id'=>$Value['appr_item_code'],'name'=>$Value['plan_bom_qty']];
...
//print_r($item_code);
//$item_code = ["ML-1", "WCL-1"];  // Example array with multiple item codes

// Convert the item_code array to a string format suitable for SQL query
$item_code_str = implode("', '", $item_code);
//$item_code_str = ($item_code);


         $insql = "SELECT a.*,  a.uom  as uoms ,c.isriorcode as isriorcodess FROM tbl_surplus_stock_details a
                INNER JOIN tbl_request b on a.req_id=b.request_id
                INNER JOIN kn_order_enquiry as c on b.enquiry_id=c.id
                WHERE  a.item_code IN ('$item_code_str')   ";
        ...
        foreach ($indata as $key => $value)
        {
            ...
            //$lot_no[] = $value['lot_no'];
            $item_desc[$key] = [ 'id'=> $value['inv_no'], 'name'=> $value['item_code'] ];
            $inv_no[$key] = [ 'id'=> $value['inv_no'],  'item_id'=> $value['item_code'], 'name'=> $value['inv_no'] ];
            $inv_date[$key] = [ 'id'=> $value['inv_date'], 'item_id'=> $value['inv_no'], 'name'=> $value['inv_date'] ];
            $lot_no[$key] = [ 'id'=> $value['lot_no'], 'item_id'=> $value['item_code'], 'name'=> $value['lot_no'] ];
            $rateList[$key] = [ 'id'=> $value['rate'], 'item_code'=> $value['item_code'], 'item_id'=> $value['lot_no'], 'name'=> $value['rate'] ];
            $gstList[$key] = [ 'id'=> $value['gst'], 'item_id'=> $value['lot_no'], 'name'=> $value['gst'] ];
            $uomList[$key] = [ 'id'=> $value['uoms'], 'item_id'=> $value['lot_no'], 'name'=> $value['uoms'] ];
            $inhousestatusdetails[$key] = [ $value['surplus_id'], '', $value['isriorcodess'], $value['pi_ref_no'], $value['inv_no'],
                             $value['inv_date'], $value['lot_no'], $value['surplus_qty'], $value['uoms'], $value['rate'], '', $value['gst'], '', $value['order_stock_date'], $value['bin']
                        ];
        ...
        //print_r($pursql); exit;
        $k=0;
          $pursql = "SELECT c.pi_ref_queue_no,c.enquiry_id as enquiry_ids,queue_no, b.ref_queue_no,b.cutoff_date,b.purchase_req_type,c.pi_dt,a.isriorcode  FROM kn_order_enquiry a
                   INNER JOIN tbl_request b on a.id=b.enquiry_id
                   INNER JOIN tbl_purchase_indent c on b.request_id = c.request_id
                  
                  WHERE  c.purchase_indent_id = " .$pId. "  AND c.supply_closed_status = 0 AND c.pi_list_status = 1  ";
        ...
        $issql = "SELECT a.pi_ref_no as piRefNo, a.* FROM tbl_surplus_issued_details a WHERE a.pid = '$pId' AND a.draft_status = 1";
         $isdata = $this->db->query($issql)->result_array();
        
        foreach($isdata as $key => $value) {
            $issued_details[$key] = [ $value['issued_id'], '', '', $value['piRefNo'], $value['transfer_category'],$value['item_desc'],$value['garment_size'],$value['item_code'],$value['item_colour_code'],$value['size_dim'],$value['siz_uom'],
                             $value['inv_no'], $value['inv_date'], $value['lot_no'], $value['rate'] , $value['gst'], $value['issued_qty'], $value['uom']
                        ];
        }
        $uniqueitemdescription = array_map("unserialize", array_unique(array_map("serialize", $itemdescription_a)));
        $unique_item_desc = array_map("unserialize", array_unique(array_map("serialize", $item_desc)));  
        $unique_inv_no = array_map("unserialize", array_unique(array_map("serialize", $inv_no)));
        $unique_inv_date = array_map("unserialize", array_unique(array_map("serialize", $inv_date)));
        $unique_lot_no = array_map("unserialize", array_unique(array_map("serialize", $lot_no)));
        $unique_rateList = array_map("unserialize", array_unique(array_map("serialize", $rateList)));
        $unique_gstList = array_map("unserialize", array_unique(array_map("serialize", $gstList)));
        $unique_uomList = array_map("unserialize", array_unique(array_map("serialize", $uomList)));
        //$itemdescription2 = array_values($uniqueitemdescription);
       
        
        //print_r($purchaseIndent);
        // $output['issued_details'] = $issued_details;
        // $output['piRefNo'] = $piRefNo;
        // $output['inv_no'] = $unique_inv_no;
        // $output['item_desc'] = $unique_item_desc;
        // $output['inv_date'] = $unique_inv_date;
        // $output['lot_no'] = $unique_lot_no;
        // $output['rateList'] = $unique_rateList;
        // $output['gstList'] = $unique_gstList;
        // $output['uomList'] = $unique_uomList;
        // $output['itemdescription'] = $itemdescription2;
        // $output['garmentSize'] = $garmentSize_a;
        // $output['itemCode'] = $itemCode_a;
        // $output['itemColorCode'] = $itemColorCode_a;
        // $output['sizedim'] = $sizes_a;
        // $output['uom'] = $uom_a;
        
         $output['issued_details'] = array_values($issued_details);
        $output['piRefNo'] = array_values($piRefNo);
        $output['inv_no'] = array_values($unique_inv_no);
        $output['item_desc'] = array_values($unique_item_desc);
        $output['inv_date'] = array_values($unique_inv_date);
        $output['lot_no'] = array_values($unique_lot_no);
        $output['rateList'] = array_values($unique_rateList);
        $output['gstList'] = array_values($unique_gstList);
        $output['uomList'] = array_values($unique_uomList);
        $output['itemdescription'] = array_values($uniqueitemdescription);
        $output['garmentSize'] = array_values($garmentSize_a);
        $output['itemCode'] = array_values($itemCode_a);
        $output['itemColorCode'] = array_values($itemColorCode_a);
        $output['sizedim'] = array_values($sizes_a);
        $output['uom'] = array_values($uom_a);
        $output['bomqty'] = array_values($bomqty);
    ...
    public function getSurplusStockDetailss($enqId, $reqId, $pId, $itemCode) {
        ...
        //         WHERE a.item_code = " .$itemCode. " AND a.enq_id=".$enqId." AND a.flag=1  GROUP BY a.lot_no ";
        // $data2 = $this->db->query($insql)->result_array();
         $insql = "SELECT a.*,SUM(d.issued_qty) as issued_qtys,d.uom as uoms FROM tbl_surplus_stock_details a
                INNER JOIN tbl_request b on a.req_id=b.request_id
                INNER JOIN kn_order_enquiry as c on b.enquiry_id=c.id
                INNER JOIN tbl_surplus_issued_details as d on a.item_code = d.item_code AND a.lot_no = d.lot_no
                WHERE a.item_code = " .$itemCode. "    GROUP BY a.lot_no ";
        $data2 = $this->db->query($insql)->result_array();
       // print_r($insql); exit;
        ...
        foreach ($data2 as $key => $value)
        {
            ...
            $inhouseconsolidatedqtydetails[$key] = [ $value['surplus_id'], '', '', $value['lot_no'], $value['rate'], $value['surplus_qty'], $value['issued_qtys'], '', $value['uom'], '', $value['uoms'], '',$value['stock_closed_status'], $value['stock_closed_status_date'], '',$value['approved_date'] ];
        ...
        $pursql = "SELECT d.request_bom_id,c.pi_ref_queue_no,c.enquiry_id as enquiry_ids,queue_no, b.ref_queue_no,b.cutoff_date,b.purchase_req_type,c.pi_dt,d.plan_bom_qty,d.uom,a.isriorcode  FROM kn_order_enquiry a
                   INNER JOIN tbl_request b on a.id=b.enquiry_id
                   INNER JOIN tbl_purchase_indent c on b.request_id = c.request_id
                   INNER JOIN tbl_request_bom d ON b.request_id = d.request_id 
                  WHERE  d.appr_item_code = " .$itemCode. " AND d.spi_list_status = 1 AND c.supply_closed_status = 0 AND c.pi_list_status = 1  ";
        $purdata = $this->db->query($pursql)->result_array();
        //print_r($pursql); exit;
        $k=0;
        foreach ($purdata as $key1 => $value1)
        {
            ...
            // }
            $piRefNo[$key1] = $value1['pi_ref_queue_no'];
            //$mienqur_id[$key1] = $value1['enquiry_ids']+$value1['pi_ref_queue_no'];
             $mienqur_id[$key1] = [ 'id'=> $value1['pi_ref_queue_no'], 'name'=> $value1['enquiry_ids'] ];
    ...
    public function getPurchaseRequestSentDetailss($id, $reqId) {
        $sql = "SELECT a.*, b.vendorname as v_name, b.address as v_address FROM tbl_request_bom as a 
                ...
                where a.enquiry_id='$id' AND a.request_id = '$reqId' ";
        $data = $this->db->query($sql)->result_array();
    ...
    public function updateDeptBOMRequestt($eid, $reqId, $req_status)
    {
        ...
        $wipcount = $this->commonmodel->getBOMrequestCountbq($subid);
        $count = ($wipcount > 0) ? $wipcount+1 : 1; // Default to 1 if no records exist
        $queue_no = $count; 
            ...
            //$ref_queue_no = $ArrIsrIor[$reqforisrior]."/".date('my')."/BSG-".$eid."/BQ-".$queue_no;
             $ref_queue_no = $ArrIsrIor[$reqforisrior]."/".date('my')."/".$companyname."/BQ-".$queue_no;
    ...
    public function getPurchaseBomQueueDetailss($enqId, $reqId) {
        ...
        $insql = "SELECT * FROM tbl_bom_in_house b
                LEFT JOIN tbl_request c on b.request_id=c.request_id
                LEFT JOIN tbl_purchase_indent d on b.purchase_indent_id=d.purchase_indent_id
                WHERE c.request_id = " . $reqId . "  AND c.mgmt_approval = 1 AND c.deprt_approval = 1 AND b.type='M' ";
        $inHousedata = $this->db->query($insql)->result_array();
        ...
        $insql1 = "SELECT * FROM tbl_bom_in_house b
                LEFT JOIN tbl_request c on b.request_id=c.request_id
                LEFT JOIN tbl_purchase_indent d on b.purchase_indent_id=d.purchase_indent_id
                WHERE c.request_id = " . $reqId . "  AND c.mgmt_approval = 1 AND c.deprt_approval = 1 AND b.type='S' ";
    ...
    public function updatePaymentDetailss($eId, $reqId, $pId, $paid_data)
{
    ...
    public function getMIBOMReceivedDetailss($id, $reqId) {
        ...
        $sql = "SELECT a.*, b.*, c.*, d.*, e.*
                ...
                WHERE a.enquiry_id = ".$id."  and a.req_sent_status = 1 ORDER BY a.sample_requirement_id desc";
        $data = $this->db->query($sql)->result_array();
        ...
        if($reqId != null) {
            $mi_sql = "SELECT * FROM tbl_mi_details WHERE request_id = ".$reqId."  ";
    ...
    public function getMIReceivedDetailss($id, $reqId) {
        ...
        $sql = "SELECT a.*, b.*, c.* FROM tbl_sample_requirement as a 
                ...
                WHERE a.enquiry_id = ".$id." AND d.request_id = ".$reqId." and a.req_sent_status = 1 ORDER BY a.sample_requirement_id asc";
        $data = $this->db->query($sql)->result_array();
        // print_r($data); exit;
        $referResult = [];
        $ref_status = 0;
...
if ($drafdaa == $i) {
    ...
    public function updatesurplusstockdatas($enqId, $reqId,$pid, $req_data)
    {
foreach ($req_data as $key => $value) {
    
    if ($value[2] == true) {
      
        $this->db->where('request_bom_id', $value[18]);
        $this->db->update('tbl_request_bom', array('spi_list_status' => 1));
     ...
     public function updatesurplusstockdatatransferr($enqId, $reqId,$pid, $issuedData, $save_status)
    {
        //print_r($pid);
        //die;
foreach($issuedData as $key => $value) {
    $itemCode = $value[7];
    $item_code = preg_replace('/\s+/', '', $itemCode); 
    //$normalized_item_code = str_replace(' ', '', $itemCode);
     $invno = $value[11];
      $lotno = $value[13];
       $purin_id = $pid;
    
            $bom_id = $this->db->where('inv_no',$invno)->where('item_code', $item_code)->where('lot_no',$lotno)->get('tbl_surplus_stock_details')->row();
            ...
            $data['item_desc'] = $value[5];
            $data['garment_size'] = $value[6];
            $data['item_colour_code'] = $value[8];
            $data['size_dim'] = $value[9];
            $data['siz_uom'] = $value[10];
            // $data['stm_date_time'] = $value[6];
            $data['inv_no'] = $value[11];
            $data['inv_date'] = $value[12];
            $data['lot_no'] = $value[13];
            $data['rate'] = $value[14];
            $data['gst'] = $value[15];
            $data['issued_qty'] = $value[16];
            $data['uom'] = $value[17];
            $data['log'] = LOGTIME;
            ...
            if($value[0] == '' && $value[2] == 1){ 
                ...
                if($value[2] == 1){
                    $surplus_id=$value[0] ;
                $this->db->where('issued_id', $surplus_id);
                $this->db->update('tbl_surplus_issued_details', $data);
                }
    ...
    public function updateOrderStockissuedcc($enqId, $reqId, $req_data1)
    {
        ...
        //if($queue_no == "") { $queue_no = 1; }
             $subid = $this->subscriberId;
             $wipcount = $this->commonmodel->getbomrequestCountDC($subid);
             $count = ($wipcount > 0) ? $wipcount+1 : 1; // Default to 1 if no records exist
             $queue_no = $count; 
    ...
    public function getOrderStockDetailss($enqId, $reqId, $code, $pId) {
        ...
        $sql = "SELECT * FROM tbl_request_bom b
                ...
                WHERE c.request_id = " . $reqId . " AND d.purchase_indent_id = " . $pId . " AND b.appr_item_code = " . $code . " AND d.bill_paid_status = 1";
        $data = $this->db->query($sql)->result_array();
        ...
        $h_sql = "SELECT a.*,b.*, c.pi_ref_queue_no  FROM tbl_bom_in_house a 
                INNER JOIN tbl_request b ON a.request_id = b.request_id
                INNER JOIN tbl_purchase_indent c ON a.purchase_indent_id = c.purchase_indent_id
                WHERE a.order_stock_status = 1 AND b.enquiry_id = " . $enquiry_id . " AND a.appr_item_code = " . $code . "   ";
        $h_data = $this->db->query($h_sql)->result_array();
...
//print_r($mi_sql); exit;
        ...
        $shipment_sql = "SELECT a.batch_ref_no,a.invoice_rate,SUM(a.received_qty) as invoice_qty, a.uom as ind_uom FROM tbl_bom_in_house a
                        INNER JOIN tbl_request b ON a.request_id = b.request_id
        ...
        foreach ($h_data as $key => $value)
        {
            ...
            $inhousestatusdetails[$key] = [ 'edit', $value['bom_in_house_id'], $value['pi_ref_queue_no'], $value['invoice_no'], date('d-m-Y',strtotime($value['invoice_date'])), $value['batch_ref_no'], $value['received_qty'], $value['uom'], $value['invoice_rate'], $value['invoice_value'],
        ...
        foreach ($shipment_data as $key => $value) {
            ...
            $misql = "SELECT d.supply_closed_status,d.order_closure_status1,a.supply_closed_date, SUM(a.issued_qty) as issued_qty,SUM(a.return_defective_qty) as return_defective_qty,SUM(a.replace_defective_qty) as replace_defective_qty,SUM(a.return_excess_qty) as return_excess_qty FROM tbl_mi_issued_details a 
            ...
            // $supply_closed_status_bominhouse = $misql_data1->supply_closed_status;
              $supply_closed_status_bominhouse = $misql_data1->order_closure_status1;
    ...
    public function supplyclosuredataa($enqId, $reqId, $code, $pId) {
        ...
        // $shipment_sql = "SELECT a.batch_ref_no,a.invoice_rate,dc_qty as invoice_qty, a.uom as ind_uom FROM tbl_bom_in_house a
        //                 INNER JOIN tbl_request b ON a.request_id = b.request_id
        ...
        // $shipment_data = $this->db->query($shipment_sql)->result_array();
           ...
           $shipment_sql = "SELECT a.batch_ref_no,a.invoice_rate,SUM(a.received_qty) as invoice_qty, a.uom as ind_uom FROM tbl_bom_in_house a
                        INNER JOIN tbl_request b ON a.request_id = b.request_id
                        WHERE a.order_stock_status = 1 AND b.enquiry_id = " . $enquiry_id . " AND a.appr_item_code = " . $code . " GROUP BY batch_ref_no ";
        $shipment_data = $this->db->query($shipment_sql)->result_array();
        ...
        foreach ($h_data as $key => $value)
        {
            ...
            $inhousestatusdetails[$key] = [ 'edit', $value['bom_in_house_id'], $value['pi_ref_queue_no'], $value['invoice_no'], date('d-m-Y',strtotime($value['invoice_date'])), $value['batch_ref_no'], $value['dc_qty'], $value['uom'], $value['invoice_rate'], $value['invoice_value'],
                    $value['gst'], $value['total_invoice_value'], $value['received_date'], $value['storage_bin']
                ];
    ...
    public function getInHouseDetailss($enqId, $reqId) {
        ...
        $sql = "SELECT * FROM tbl_request_bom b
                INNER JOIN tbl_request c on b.request_id=c.request_id
                WHERE c.request_id = " . $reqId . "  AND c.bill_paid_status = 1";
        $data = $this->db->query($sql)->result_array();
    ...
    public function getBOMInHouseDetailss($enqId, $reqId, $pId) {
        ...
        $sql = "SELECT * FROM tbl_request_bom b
                ...
                WHERE d.purchase_indent_id = " . $pId . "  ";
        $data = $this->db->query($sql)->result_array();
...
if (!empty($bomid)) {
    ...
    $pending_qty = null;
     ...
     public function updateSaveStockDetailss($enqId, $reqId, $mi_issued_details, $bom_ref_no, $bom_dept, $received_by, $draf_id, $save_status)
    {
        //echo "<pre>"; print_r($mi_issued_details); exit;
        foreach ($mi_issued_details as $key => $value) {
            ...
            $miissuedData['enquiryId'] = $enqId;
            $miissuedData['save_status'] = $save_status;
            //  print_r($miissuedData); exit;
            if($value[1] == '' &&  $value[0] == '' )
            {
                $sam_val = $this->db->where('mi_bom_details_id',$value[2])->get('tbl_mi_bom_details')->row()->sample_no;
        ...
        foreach ($mi_issued_details as $key1 => $value1) {
                ...
                $issuse_data['save_status'] = $save_status;
                //$issuse_data['received_name'] = $received_by;
                $issuse_data['received_name'] = $received_by;
...
//            
    ...
    public function updateOrderCloseDetailss($enqId, $reqId, $pId, $shipment_order_details, $itemDetails, $in_house_qty)
    {
        foreach ($itemDetails as $key => $value) {
            ...
            $item_code1 = preg_replace('/\s+/', '', $item_code);
            //$item_code1 = trim($item_code);
        ...
        foreach ($shipment_order_details as $key => $value) {
           ...
           if($value[14] == 1 && $value[15] == '' ) {
               if($value[9] > 0) {
                ...
                $this->db->where($where);
                $this->db->update('tbl_bom_in_house', array('supply_closed_status' => 1, 'order_closure_status1' => 1,'enquiryId' => $enqId,'log'=>LOGTIME));
                $this->db->where('request_id',$reqId)->update('tbl_request',array('log'=>LOGTIME));
     ...
     * @return void
     */
     public function updateorderclosurelistt($enqId, $reqId,$item_code)
    {
            ...
            //      $this->db->where('enquiryId',$enqId);
            //      $this->db->update('tbl_bom_in_house', array('supply_closed_status_moved' => 1,'order_closure_status_moved1' => 1 )); 
            //      $this->db->where('enq_id',$enqId);
            //      $this->db->update('tbl_surplus_stock_details', array('supply_closed_status_moved' => 1 ));    
            // }
//prinr($item_code);
             if (!empty($enqId)) {
    // optional: wrap all updates in a transaction
    $this->db->trans_start();
    // 2) tbl_bom_in_house with extra WHEREs
    $this->db->where('enquiryId', $enqId)
             ->where('request_id', $reqId)              // <-- use $reqId as a value
             ->where('appr_item_code', $item_code)     // <-- extra condition
             ->update('tbl_bom_in_house', [
                 'supply_closed_status_moved'   => 1,
                 'order_closure_status_moved1'  => 1
             ]);


    // 1) tbl_mi_issued_details
    $this->db->where('enquiryId', $enqId)
             ->update('tbl_mi_issued_details', [
                 'supply_closed_status_moved' => 1
             ]);

    
    // 3) tbl_surplus_stock_details (note column is enq_id here)
    $this->db->where('enq_id', $enqId)
             ->update('tbl_surplus_stock_details', [
                 'supply_closed_status_moved' => 1
             ]);
    ...
    if ($this->db->trans_status() === FALSE) {
        // handle failure (log, return error, etc.)
    }
}
    ...
    public function getBOMMIData($miId, $dc)
    {
        ...
        $sql = 'SELECT a.*,b.*,c.*,e.ref_queue_no FROM tbl_mi_details as a 
                INNER JOIN tbl_mi_bom as b ON a.request_id=b.request_id
                INNER JOIN tbl_mi_bom_details as c ON b.mi_bom_id=c.mi_bom_id
                INNER JOIN tbl_request e on a.req_bom_id = e.request_id
                WHERE c.dc_ref_queue_no = '.$dc.' GROUP BY c.dc_ref_queue_no';
    ...
    public function getDCListt($id, $reqId, $miId)
    {
        ...
        $sql = "SELECT * FROM tbl_mi_issued_details as a
                ...
                INNER JOIN tbl_mi_bom as c on b.mi_bom_id=c.mi_bom_id
                INNER JOIN tbl_mi_details as d on c.request_id=d.request_id
                WHERE a.dc_status = 0 AND c.request_id = ".$reqId." ";
        $result = $this->db->query($sql)->result_array();
        //print_r($sql); exit;
     ...
     public function get_draft_typee($enq_id)
    {
        ...
        $tot = 0;
        $type = '';
        $sql = $this->db->where('enquiry_id',$enq_id)->where('req_draft_status',1)->get('tbl_bom_article_1_req_consld')->result_array();
        $tot = count($sql);
        foreach($sql as $r) {
            $res[] = $r['request_id'];
        }
    ...
    public function getBomStoreDetailss($enqId, $reqId, $pId) {
        ...
        $sql = "SELECT * FROM tbl_request_bom b
                ...
                LEFT JOIN kn_master_bom_vendor d on b.vendor_id=d.id
                LEFT JOIN tbl_bom_in_house e on b.request_bom_id = e.request_bom_id
                WHERE b.purchase_indent_id = " . $pId . "  AND c.mgmt_approval = 1 AND c.deprt_approval = 1 ";
        $data = $this->db->query($sql)->result_array();
        //print_r($data); exit;
        ...
        $sql2 = "SELECT c.*, b.order_stock_status as order_stock_status ,b.received_uom,b.supply_closure_status,b.supply_closure_date, SUM(b.received_qty) as received_qtys, c.plan_bom_qty FROM tbl_request_bom c
                LEFT JOIN tbl_bom_in_house b on c.request_bom_id = b.request_bom_id  AND $where
                LEFT JOIN tbl_request d on c.request_id=d.request_id
                WHERE c.purchase_indent_id = " . $pId . "  GROUP BY c.item_desc,c.garment_size,c.appr_item_code,c.appr_item_col_code,c.size_dim,c.uom  order by c.request_bom_id  ";
        @$req_data1 = $this->db->query($sql2)->result_array();
    ...
    public function getMIUomm($data)
    {
        ...
        $sql = "SELECT a.uom FROM tbl_mi_bom_details a
                ...
                WHERE a.item_code = ".$itemCode." AND c.bom_ref_no = ".$mi_id." AND a.sample_no = ".$sam_val."  ";
    ...
    public function getMIUom11($data)
    {
        ...
        $sql = "SELECT a.uom FROM tbl_mi_bom_details a
                INNER JOIN tbl_mi_bom b on a.mi_bom_id=b.mi_bom_id
                INNER JOIN tbl_mi_details c on b.request_id=c.request_id
                WHERE a.item_code = ".$itemCode." AND c.bom_ref_no = ".$mi_id." AND a.mi_bom_details_id = ".$sam_val."  ";
        $datass = $this->db->query($sql)->row();
    ...
    public function getsurplusstocklistt() {
        $sql = "SELECT a.*,c.*, a.log as logs ,SUM(a.surplus_qty) as surplus_qtys,d.cutoff_date FROM tbl_surplus_stock_details as a 
    ...
    public function updateSurplusDraftt($enqId, $reqId, $pId, $itemCode, $issuedData)
    {
        ...
        foreach($issuedData as $key => $value) {
            $bom_id = $this->db->where('inv_no',$value[7])->where('item_code',$itemCode)->where('lot_no',$value[9])->get('tbl_surplus_stock_details')->row();
            //$data['enqId'] = $enqId;
            $data['enqId'] = $value[14];
            $data['reqId'] = $reqId;
            $data['pId'] = $pId;
    ...
    public function getSurplusDCList($id, $reqId, $itemCode,$pid)
    {
        ...
        // $result = $this->db->query($sql)->result_array();
          ...
          $sql = "SELECT *  from   tbl_surplus_issued_details    WHERE item_code = ".$itemCode."   AND draft_status=1  ";
          $result = $this->db->query($sql)->result_array();
        ...
        foreach ($result as $key => $value) {
             $bom1 = "SELECT * FROM tbl_bom_in_house AS a 
         WHERE a.appr_item_code = ".$itemCode." 
         AND a.batch_ref_no = ".$value['lot_no'];
         $value1 = $this->db->query($bom1)->result_array();
        
                

                $draftData[$key] = [
                $value['issued_id'], '', true, $value['pi_ref_no'], $value['inv_no'], $value1[0]['item_desc'], $bcm, $value['garment_size'], $value['item_code'],
                $value1[0]['appr_item_color_code'], $value1[0]['size_dim'], $value['uom'], $value['lot_no'], $value['issued_qty'], $value['uom'], $value['rate'], '', $value['gst'], ''
            ];
   ...
   public function clearSurplusDraftFunctionn($enqId, $reqId, $itemCode, $req_data)
{
    ...
    //print_r($req_data);
    // Initialize arrays to store issued_ids and batch_ref_nos for batch delete
    $issued_ids = [];
    $batch_ref_nos = [];
    $itemCodes = [];

    // First loop to collect issued_ids for batch deletion from tbl_surplus_issued_details
    foreach ($req_data as $key => $value) {
        $issued_ids[] = $value[0]; // Collect issued_id
    }

    // Perform batch delete for tbl_surplus_issued_details
    if (!empty($issued_ids)) {
        $this->db->where_in('issued_id', $issued_ids)
                 ->where('draft_status', 1)
                 ->delete('tbl_surplus_issued_details');
    }

    // Second loop to collect batch_ref_nos for batch deletion from tbl_bom_in_house
    foreach ($req_data as $key => $value) {
        $batch_ref_nos[] = $value[4];
         $itemCodes[] = $value[8];  // Collect batch_ref_no
    }

    // Perform batch delete for tbl_bom_in_house
    if (!empty($batch_ref_nos)) {
        $this->db->where('request_id', $reqId)
                 ->where_in('appr_item_code', $itemCodes)
                 ->where_in('batch_ref_no', $batch_ref_nos)
                 ->delete('tbl_bom_in_house');
    }

    // Optionally, return or log the status of the operation
    if ($this->db->affected_rows() > 0) {
    ...
    public function updateSurplusDCListt($enqId, $reqId, $pId, $itemCode, $req_data, $received_by, $amt_words, $pay_terms)
    {
        ...
        foreach ($req_data as $key => $value) {
            if($value[2] == true)
            {
                ...
                $in_data['supply_closure_date'] = $this->mysqldatetime;
     ...
     public function updateSurplusSSTDCListt($enqId, $reqId, $pId)
    {
        //echo "<pre>"; print_r($enqId); exit;
        $pids = "'".$pId."'";
         //$itemCode = "'".$itemCode."'";
        $ord_sql = "SELECT pi_ref_no FROM tbl_surplus_issued_details WHERE pid=".$pids." AND draft_status=1 " ;
        $ord_data = $this->db->query($ord_sql)->result_array();

        $ord_sql1 = "SELECT * FROM tbl_surplus_issued_details WHERE pid=".$pids." AND draft_status=1 " ;
        $ord_data1 = $this->db->query($ord_sql1)->result_array();
        ...
        foreach ($ord_data1 as $key => $value) {

                $surplusid=$value['pi_ref_no'];
                $issued_id=$value['issued_id'];
                $itemCodes=$value['item_code'];
                $reivedqty=$value['issued_qty'];
                $lot_no=$value['lot_no'];
                $itemCode = "'".$itemCodes."'";
                ...
                $bom_data = $this->db->where('issued_id',$issued_id)->get('tbl_surplus_issued_details')->row();
                
                $lot_no = "'".$bom_data->lot_no."'";
                //$ord_sql = "SELECT * FROM tbl_bom_in_house WHERE request_bom_id=$bom_data->bom_id AND batch_ref_no=".$lot_no." AND appr_item_code=$itemCode" ;
                $ord_sql = "SELECT * FROM tbl_bom_in_house WHERE request_bom_id=$bom_data->bom_id AND batch_ref_no=".$lot_no." " ;
                ...
                $in_data['gst'] = $item_data->gst;
                $in_data['total_invoice_value'] = $item_data->total_invoice_value;
                $in_data['received_qty'] = $reivedqty;
                $in_data['received_uom'] = $item_data->received_uom;
                $in_data['received_date'] = $item_data->received_date;
                $in_data['storage_bin'] = $item_data->storage_bin;
                $in_data['merchant_item_status'] = $item_data->merchant_item_status;
    ...
    public function getstocktransferlistt_old() {
        $sql = "SELECT a.*,b.isriorcode, c.brandname,d.cutoff_date,e.pi_dt FROM tbl_surplus_issued_details as a 
                inner join kn_order_enquiry as b on a.enqId=b.id 
                inner join ".KN_MASTER_BRANDS." as c on b.brandId=c.id 
                inner join tbl_request as d on a.reqId=d.request_id 
        ...
        foreach ($result as $key => $res) {
            ...
            $data[$key]['pi_dt'] = $res['pi_dt'];
            $data[$key]['cutoff_date'] = $res['cutoff_date'];
            $data[$key]['stm_ref_no'] = implode('<span style="display:block;  margin-bottom:10px;"></span>', $stm_ref_no);
            $data[$key]['stm_date_time'] = implode('<span style="display:block; margin-bottom:10px;"></span>', $stm_date);
            $data[$key]['transfer_category'] = implode('<span style="display:block; font size:12px; margin-top:7px; "></span>', $trans_category);
            $data[$key]['status'] = implode('<span style="display:block;font size:12px;margin-top:3px; "></span>', $status);
            $data[$key]['log'] = implode('<span style="display:block; margin-top:5px;"></span>', $log);
            $data[$key]['flag'] = $res['flag'];
     ...
     public function getstocktransferlistt() {
        $sql = "SELECT a.*,a.flag as flags,b.isriorcode, e.purchase_indent_id as purchase_indent_id,c.brandname,d.cutoff_date,e.pi_dt FROM tbl_surplus_issued_details as a 
                inner join kn_order_enquiry as b on a.enqId=b.id 
                inner join ".KN_MASTER_BRANDS." as c on b.brandId=c.id 
                inner join tbl_request as d on a.reqId=d.request_id 
                inner join tbl_purchase_indent as e on a.pId=e.purchase_indent_id
                WHERE a.draft_status=0 and d.subscriberid = " . $this->db->escape($this->subscriberId)." GROUP BY a.enqId ORDER BY e.log DESC";
        $result = $this->db->query($sql)->result_array();
        foreach ($result as $key => $res) {
            $stm_ref_no = $stm_date = $trans_category = $status = $log = [] ;
            $enq_id = $res['enqId'];
            $sql1 = "SELECT *,SUM(issued_qty) as issued_qtys ,pid as purchase_indent_id FROM tbl_surplus_issued_details WHERE enqId=".$enq_id." AND draft_status=0 GROUP BY stm_ref_no ";
            $result1 = $this->db->query($sql1)->result_array();
            $tot_qty = [] ;
            $tot_qtys = 0;
            foreach($result1 as $key1 => $val) {
                ...
                WHERE stm_ref_no=".$stm_ref." AND draft_status=0  ";
                $result2 = $this->db->query($sql2)->result_array();
                //print_r($val['issued_qtys']);
                //die;
                $this->db->select_sum('issued_qty');
                $this->db->where('pid', $val['purchase_indent_id']);
                $query1 = $this->db->get('tbl_surplus_issued_details');
                $result1 = $query1->row_array();
                $qtybomsum1 = $result1['issued_qty'];

                $this->db->select_sum('plan_bom_qty');
                $this->db->where('purchase_indent_id', $val['purchase_indent_id']);
                $query = $this->db->get('tbl_request_bom');
                $result = $query->row_array();
                $qtybomsum = $result['plan_bom_qty'];
                //print_r($qtybomsum); 
                //die;
                if($qtybomsum1 == 0 || $qtybomsum1 == null) {
                    $status[] = '<span style="font-size:8px;" class="text-light knOrangeColor bg-dark"><strong> S.T. PENDING</strong></span>';
           
                }else{
                   if($qtybomsum == $qtybomsum1) {
                    $status[] = '<span style="font-size:8px;" class="text-light knGreenColor bg-dark"><strong>ISSUED - FULL</strong></span>';
     
                }else{
                    $status[] = '<span style="font-size:8px;" class="text-light knOrangeColor bg-dark"><strong>ISSUED - PART</strong></span>';
                }
                ...
                $trans_category[] = $val['transfer_category'];
                //$status[] = '<span class="text-light knGreenColor bg-dark"><strong>ISSUED - PART</strong></span>';
                $log[] = date('d/m/ h:i A',strtotime($val['log']));
            }  
            
             $data[$key]['purchase_indent_id'] = $res['purchase_indent_id'];
            $data[$key]['isriorcode'] = $res['isriorcode'];
            $data[$key]['brandname'] = $res['brandname'];
            $data[$key]['requirement'] = 'BOM (Art-1)';
            $data[$key]['pi_ref_no'] = $res['pi_ref_no'];
            $data[$key]['pi_dt'] = $res['pi_dt'];
            $data[$key]['cutoff_date'] = $res['cutoff_date'];
           $data[$key]['stm_ref_no'] = implode('<div style=" padding-top: 10px; padding-bottom: 10px;"></div>', $stm_ref_no);
           $data[$key]['stm_date_time'] = implode('<div style=" padding-top: 10px; padding-bottom: 10px;"></div>', $stm_date);
          $data[$key]['transfer_category'] = implode('<div style="padding-top: 10px; padding-bottom: 10px;"></div>', $trans_category);
          $data[$key]['status'] = implode('<div style="padding-top: 6px; padding-bottom: 8px;"></div>', $status);
          $data[$key]['log'] = implode('<div style="padding-top: 12px; padding-bottom: 8px;"></div>', $log);
           $data[$key]['flag'] = $res['flag'];
    ...
    public function getsurplusData($stm_ref_no)
    {
        ...
        $sql = "SELECT a.*,b.ref_queue_no,c.req_status as pi_req_status, d.contactname as req_name,e.contactname as auth_name, f.contactname as pi_req_name, g.appr_status, h.contactname as pi_appr_name 
                ...
                INNER JOIN tbl_request_status as g ON a.purchase_indent_id=g.purchase_indent_id
                INNER JOIN ".KN_USERS." h on g.appr_by=h.id
                WHERE a.purchase_indent_id='$pidd'";
                //print_r($sql); exit;
        $data = $this->db->query($sql)->result_array();
        return $data;
    ...
    public function getsurplusDataspi($stm_ref_no)
    {
        $stm_ref_no = "'".$stm_ref_no."'";
        $sql = "SELECT a.*,c.cutoff_date,c.ref_queue_no,c.purchase_req_type,b.mode FROM tbl_surplus_issued_details as a
                INNER JOIN tbl_purchase_indent as b on a.pi_ref_no=b.pi_ref_queue_no
     ...
     public function getorderclosure($enq_id)
    {
     ...
     public function getorderclosuremoved($enq_id, $req_id,$itemCode)
    {
        $itemCodes = "'".$itemCode."'";
        $sql = "SELECT b.* FROM tbl_bom_in_house b WHERE b.request_id = " . $req_id . " AND b.enquiryId = " . $enq_id . "  AND b.appr_item_code = " . $itemCodes . "  and (b.order_closure_status1 = 1 and b.order_closure_status_moved1 = 0)   ";
        $req_data = $this->db->query($sql)->result_array();
     ...
     public function getMIDraftData3($req_id,$draf_id)
    {
     ...
     public function getmIDraftsaveData($req_id)
    {
        $result = [];
        $sql = "SELECT * FROM tbl_mi_issued_details as a
                INNER JOIN tbl_mi_bom_details as b on a.mi_bom_details_id=b.mi_bom_details_id
                INNER JOIN tbl_mi_bom as c on b.mi_bom_id=c.mi_bom_id
                WHERE c.request_id = ".$req_id."  AND a.save_status = 0 AND a.dc_no = '' ";
        $result = $this->db->query($sql)->result_array();
     ...
     public function getsavestatus($enq_id, $draf_id, $req_id)
    {
        $result = [];
       $miserialno = 'SAMPLE-' . ($draf_id + 1);
        $sql = "SELECT * FROM tbl_mi_issued_details as a
                INNER JOIN tbl_mi_bom_details as b on a.mi_bom_details_id=b.mi_bom_details_id
                INNER JOIN tbl_mi_bom as c on b.mi_bom_id=c.mi_bom_id
                WHERE c.request_id = ".$req_id." AND a.enquiryId = ".$enq_id." AND  a.mi_serial_no = '".$miserialno."'  AND a.save_status = 0   and a.dc_no = ''  ";
        $result = $this->db->query($sql)->result_array();
     ...
     public function getsavestatus1($enq_id, $draf_id, $req_id)
    {
        $result = [];
            $miserialno = 'SAMPLE-' . ($draf_id + 1);
        $sql = "SELECT * FROM tbl_mi_issued_details as a
                INNER JOIN tbl_mi_bom_details as b on a.mi_bom_details_id=b.mi_bom_details_id
                INNER JOIN tbl_mi_bom as c on b.mi_bom_id=c.mi_bom_id
                WHERE c.request_id = ".$req_id." AND a.enquiryId = ".$enq_id." AND  a.mi_serial_no = '".$miserialno."' AND a.save_status = 1 and a.dc_no = ''  ";
        $result = $this->db->query($sql)->result_array();
