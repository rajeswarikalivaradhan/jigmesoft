...
class Detailmodel extends CI_Model {
    function saveInfo($ArrSaveData) {
        ...
        if ($VarId == "") {
            ...
            if (empty($ArrCheckExist)) {
                ...
                $this->db->insert(KN_MASTER_DETAILS, $ArrSaveData);
            ...
            if (empty($ArrCheckExist)) {
                if ($this->db->update(KN_MASTER_DETAILS, $ArrSaveData, array('id' => $VarId))) {
                    $ArrResult['errcode'] = 1;
                    $ArrResult['msg'] = '';
                    $ArrResult['id'] = $VarId;
    ...
    function fnGetInfo($VarData, $VarStatus, $VarId = '',$VarCompanyId='') {
        $this->db->select('id,description,status')->from(KN_MASTER_DETAILS);
        if ($VarData <> '') {
    ...
    function fnCheck($VarData = '', $VarId = '',$VarCompanyId='') {
        $this->db->from(KN_MASTER_DETAILS);
    ...
    function fnCount($Vardescription = '', $VarStatus = '') {
        ...
        $VarSqlLab = "SELECT count(1) as trec  FROM " . KN_MASTER_DETAILS . " AS b " . $VarWhere;
    ...
    function fnList($Vardescription = '', $VarStatus = '', $VarLimit = 10, $offset = 0, $VarSortBy, $VarSortOrder) {
        ...
        //               FROM " . KN_MASTER_DETAILS . " AS b " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder . " limit " . $VarLimitInfo;
        ...
        $VarSqlLab = "SELECT b.id,b.description,b.datecreated,DATE_FORMAT(b.log,\"%d/%m/%Y %h:%i %p\") as formattedDateUpdated,b.status,contactname
                      FROM " . KN_MASTER_DETAILS . " AS b LEFT JOIN " . KN_USERS . " AS u ON b.updatedby = u.id " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder;
    ...
    function fnChangeComStat($checkboxids, $optvalue) {
        ...
        if ($this->db->update(KN_MASTER_DETAILS, array('status' => $optvalue))) {
            return true;
        }
        return false;
    }
