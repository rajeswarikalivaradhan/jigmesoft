...
class UnitRatemodel extends CI_Model {
    function saveInfo($ArrSaveData) {
        ...
        if ($VarId == "") {
            ...
            if (empty($ArrCheckExist)) {
                ...
                $this->db->insert(KN_MASTER_UNITRATE, $ArrSaveData);
            ...
            if (empty($ArrCheckExist)) {
                if ($this->db->update(KN_MASTER_UNITRATE, $ArrSaveData, array('id' => $VarId))) {
                    $ArrResult['errcode'] = 1;
                    $ArrResult['msg'] = '';
                    $ArrResult['id'] = $VarId;
    ...
    function fnGetInfo($VarData, $VarStatus, $VarId = '',$VarCompanyId='') {
        $this->db->select('id,description,status')->from(KN_MASTER_UNITRATE);
        if ($VarData <> '') {
    ...
    function fnCheck($VarData = '', $VarId = '',$VarCompanyId='') {
        $this->db->from(KN_MASTER_UNITRATE);
        if ($VarId <> '') {
    ...
    function fnCount($Vardescription = '', $VarStatus = '') {
        ...
        $VarSqlLab = "SELECT count(1) as trec  FROM " . KN_MASTER_UNITRATE . " AS b " . $VarWhere;
    ...
    function fnList($Vardescription = '', $VarStatus = '', $VarLimit = 10, $offset = 0, $VarSortBy, $VarSortOrder) {
        ...
        $VarSqlLab = "SELECT b.id,b.description,b.datecreated,DATE_FORMAT(b.log,\"%d/%m/%Y %h:%i %p\") as formattedDateUpdated,b.status,contactname
                      FROM " . KN_MASTER_UNITRATE . " AS b LEFT JOIN " . KN_USERS . " AS u ON b.updatedby = u.id " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder;
    ...
    function fnChangeComStat($checkboxids, $optvalue) {
        ...
        if ($this->db->update(KN_MASTER_UNITRATE, array('status' => $optvalue))) {
            return true;
        }
        return false;
    }
