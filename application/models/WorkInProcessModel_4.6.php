...
class WorkInProcessModel extends CI_Model
{
    ...
    public function getComboColourDetailss($id) {
        $sql = "SELECT * FROM tbl_oe_combo_color WHERE enquiry_id = " . $id . " AND flag=1";
    ...
    {
        $sql = "SELECT * FROM tbl_oe_po_wise as a INNER JOIN tbl_oe_combo_color as b ON a.combo_color_id = b.combo_color_id WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
    ...
    function _getCombo($id)
    {
        ...
        $ArrRes = $this->db->get_where('tbl_oe_combo_color', ['enquiry_id' => $id, 'flag' => 1]);
...
//      function getSizeChart_sample($id)
// {
//      $this->db->select('size_name');
//         $ArrRes = $this->db->get_where('tbl_size_master', array('id' => $id));
//         return $ArrRes->row()->size_ids;
...
// }
    ...
    function getSizeMaster($size_ids = '')
    {
     ...
     function getSizeMasterss($size_ids = '')
    {
        $userInfoQry = "SELECT size_name FROM tbl_size_master sm WHERE sm.id ='$size_ids' ";
        $data        = $this->db->query($userInfoQry)->result_array();
    ...
    public function getOrderEntryComponentItemizedd($id)
    {
        ...
        $component_sql = "SELECT * from tbl_oe_component_intake_wise WHERE enquiry_id = '$id' AND flag=1 ORDER BY component_intake_wise_id ASC";
        $component_data = $this->db->query($component_sql)->result_array();

         $sizeChart    = $this->getSizeChart($id);
         $sizeMaster   = $this->getSizeMaster($sizeChart);
         $sizeMaster_data[]=$sizeMaster;
...
////////////////    get concade of price to size
           $size_price = [];

        foreach ($sizeMaster_data[0] as $index => $sizeInfo) {
     $sizeName = $sizeInfo['size_name'];
     $value = isset($split_sizewise_data[$index]) ? $split_sizewise_data[$index] : null;
...
/////////// get concade of price to size
            ...
            $combineValue = array_merge($combineValue, $size_price);
    ...
    public function updateOrderEntryComponentItemizedd($req_data, $id) {
        ...
        $ArrRes = $this->db->get_where('tbl_oe_component_intake_wise', ['enquiry_id' => $id, 'flag' => 1]);
    ...
    public function getOrderEntryCompleteProcesss($id) {
        // $sql = "SELECT *, b.combo as po_combo FROM tbl_oe_po_wise as a INNER JOIN tbl_oe_combo_color as b ON a.combo_color_id = b.combo_color_id WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $sql = "SELECT * FROM tbl_oe_cmplt_garment_process as a WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
        $result = [];
        foreach ($data as $key => $value)
        {
    ...
    public function getCADRequirementt($id) {
        //$sql = "SELECT * FROM tbl_cad_requirement as a WHERE a.enquiry_id = " . $id . " AND a.flag=1";
         $sql = "SELECT * FROM tbl_cad_requirement as a WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
        $result = [];
    ...
    public function getSampleDetailss($id) {
        // $sql = "SELECT *, b.combo as po_combo FROM tbl_oe_po_wise as a INNER JOIN tbl_oe_combo_color as b ON a.combo_color_id = b.combo_color_id WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $sql = "SELECT * FROM tbl_sample_requirement as a WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
        $result = [];
        foreach ($data as $key => $value)
        {
    ...
    public function getEmbellishmentDetailss($id) {
        $sql = "SELECT * FROM tbl_embl_details as a WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
        $result = [];
...
$poEnqRefNo[] = (object)[
    ...
    public function updateEmbellishmentDetailss_backup($req_data, $id) { // myself
        foreach ($req_data as $key => $value)
        {
            $comboColor["tbl_embl_detail_id"] = $value[1];
            $comboColor["enquiry_id"] = $id;
            ...
            // $comboColor["spec_code"] = $value[6];
            $comboColor["artwork_name"] = $value[6];
            $comboColor["type_medium"] = $value[7];
            $comboColor["grading_details"] = $value[8];
            $comboColor["size_group"] = $value[9];
            $comboColor["aproval_status"] = $value[10];
            $comboColor["approved_by"] = $value[11];
            $comboColor["approved_date"] = $value[12];
            $comboColor["app_samp_ref_no"] = $value[13];
    ...
    // ********** EMBELLISHMENT VENDOR DETAILS STARTS HERE *********** /
      ...
      public function getEmbellishmentVendorDetailss($id) {
        $sql = "SELECT * FROM tbl_embl_details as a WHERE 
                a.enquiry_id = ".$id." AND a.aproval_status !='PENDING' and a.aproval_status !=''  AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
    ...
    public function getBOMSamplingApprovalDetailss($id) {
        $sql = "SELECT * FROM tbl_bom_article_1 as a WHERE a.enquiry_id = " . $id . " AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
        $result = [];
        foreach ($data as $key => $value)
        {
    ...
    public function getBOM1RequirementDetailss($id) {
        $sql = "SELECT * FROM tbl_bom_article_1_requirement as a WHERE a.enquiry_id='$id' AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
...
//              $filter = 'L'; // or 'all'
// $filter = strtolower(trim($filter)); // Clean and lowercase
// $total = 0;
// $matched = [];
//  foreach ($amountCalculation[$i] as $item) {
//     if (is_array($item) && isset($item['size_name'], $item['value'])) {
//         $size = strtolower($item['size_name']);
//         if ($filter === 'all' || $size === $filter) {
//             $matched[] = $item;
//             $total += $item['value'];
//         }
//     }
        ...
        $Amount = [];
            foreach ($req_data as $key => $value) 
            {
                //echo("ppppppppppp");
                   ...
                   $filterssss = $value['garment_size'];
                   $filter;
                 $size = $this->getSizeMasterss($filterssss);
                 if (empty($size)) {
              $filter = "All"; // Default value when no size data exists
                 } else {
   
                  $filter = $size[0]['size_name'];
                  }


                 $total = 0;
                 $matched = [];
                foreach ($amountCalculation[$key] as $item) {
    if (is_array($item) && isset($item['size_name'], $item['value'])) {
        $size = strtolower($item['size_name']);
      
      if (strtolower($filter) === 'all' || strtolower($size) === strtolower($filter)) {
           //echo($tiem);
            $matched[] = $item;
            $total += $item['value'];
        }
            ...
            $AmountValue['po_ref'] = $amountCalculation[$key][2];
            $AmountValue['combo'] = $amountCalculation[$key][3];
            $AmountValue['component'] = $amountCalculation[$key][4];
            $AmountValue['colour'] = $amountCalculation[$key][5];
            $AmountValue['spec'] = $amountCalculation[$key][7];
            $AmountValue['amount'] = $total;
            array_push($Amount, $AmountValue);
...
//echo("duplicate removed");
    ...
    public function updateBOM1RequirementDetailss($req_data, $id) {
        foreach ($req_data as $key => $value)
        {
            ...
            // $get_req = "SELECT * FROM tbl_bom_article_1_requirement WHERE enquiry_id='$id' AND flag=1 and bom_article_id='$bom_id'";
    ...
    public function getBOM1ConsolidatedReqq($id) {
        ...
        $sql1 = "SELECT * FROM tbl_bom_article_1_req_consld as a WHERE a.enquiry_id='$id' AND a.flag=1 ORDER BY a.bom_1_req_consld_id";
    ...
    public function getBOM1Sourcingg($id) {
        // $sql = "SELECT * FROM tbl_bom_article_1_requirement as a 
        //         where a.enquiry_id='$id' AND a.flag=1
        ...
        // $sql1 = "SELECT * FROM tbl_bom_1_sourcing_details as a WHERE a.enquiry_id='$id' AND a.flag=1";
        // $data1 = $this->db->query($sql1)->result_array();

        // $sql2 = "SELECT *  FROM `tbl_bom_article_1_requirement` WHERE `enquiry_id` = '$id' GROUP BY item_desc,appr_item_code";
        // $data2 = $this->db->query($sql2)->result_array();


        $sql3 = "SELECT * FROM tbl_bom_article_1_requirement b LEFT JOIN tbl_bom_1_sourcing_details c on b.appr_item_code=c.item_code WHERE b.enquiry_id = ".$id." AND b.flag=1 GROUP BY item_desc,appr_item_code";
        $data3 = $this->db->query($sql3)->result_array();
        //kn_master_bom_vendor
        // print_r($data3);
         // exit();
        ...
        for($i=0; $i < sizeof($data3); $i++)
        {
            ...
            if( isset($data3[$i]['appr_item_code'])) {
                @$appr_item_code = $data3[$i]['appr_item_code'];
            }

            if( isset($data3[$i]['appr_item_col_code'])) {
                @$appr_item_col_code = $data3[$i]['appr_item_col_code'];
            }
            ...
            if( isset($data3[$i]['vendor_name_address'])) {
                //$add = $this->db->where('id',$data3[$i]['vendor_name_address'])->get('kn_master_bom_vendor')->row();
                //@$vendor_name_address = $add->vendorname.','.$add->contactpersonname.','.$add->address;
            ...
            $result[$i] = [$type, $bom_1_req_id, $data3[$i]['item_desc'],
                             $appr_item_code, $appr_item_col_code, $sourcing_advice, $vendor_location, $vendor_name_address, $contact_email,
       ...
       // $materialData = $this->db->query('SELECT id as id, bomitemdesc as name, content, material, bomblend FROM '. KN_MASTER_BOM)->result();
        ...
        foreach($bomVendor as $key => $value) {
            ...
            $bom_ven[$key] = [ 'id'=> $value['id'], 'name'=> $value['name'].','.$value['contactpersonname'].','.$value['address'] ];
            $bom_email[$key] = [ 'id'=> $value['emailid'], 'item_id'=> $value['id'], 'name'=> $value['emailid'].','.$value['phone'].','.$value['mobile'] ];
            $bom_gst[$key] = [ 'id'=> $value['gstno'], 'item_id'=> $value['id'], 'name'=> $value['gstno'].','.$value['iecode'] ];
    ...
    public function updateBom1Sourcingg($req_data, $id) {
        ...
        foreach ($req_data as $key => $value)
        {
            ...
            $comboColor["item_code"] = $value[3];
            $comboColor["sourcing_advice"] = $value[5];
            $comboColor["vendor_location"] = $value[6];
            $comboColor["vendor_name_address"] = $value[7];
            $comboColor["contact_email"] = $value[8];
            $comboColor["gst"] = $value[9];
            $comboColor["online_order_sys"] = $value[10];
            $comboColor["pass_expiry_date"] = $value[11];
    ...
    public function get_bom1_sampling_despatchh($id) {
        ...
        $sql = "SELECT * FROM tbl_bom_article_1 as a WHERE a.enquiry_id ='$id' AND a.flag=1 and a.despatch_approval=1";
        $data = $this->db->query($sql)->result_array();
    ...
    public function getBOM2RequirementDetailss($id) {
        $sql = "SELECT * FROM tbl_bom_article_2_requirement as a WHERE a.enquiry_id='$id' AND a.flag=1";
        $data = $this->db->query($sql)->result_array();
        $result = [];
        foreach ($data as $key => $value)
        {
       ...
       // $materialData = $this->db->query('SELECT id as id, description as name FROM '.KN_BOM_MASTER.' where type=1')->result();
        ...
        foreach ($req_data as $key => $value) 
        {
           ...
           // $item_description_name = "";
            ...
            // for($i = 0; $i < sizeof($materialData); $i++) {
            //     if($materialData[$i]->id == $item_description) {
            //         $item_description_name = $materialData[$i]->name;
            //         break;
            //     }
            // }
    ...
    public function getBOM2ConsolidatedReqq($id) {
        ...
        $sql1 = "SELECT * FROM tbl_bom_article_2_req_consld as a WHERE a.enquiry_id='$id' AND a.flag=1 ORDER BY a.bom_2_req_consld_id";
        $data1 = $this->db->query($sql1)->result_array();
        // print_r($data1);
        // exit();
    ...
    public function getBOM2Sourcingg($id) {
        ...
        for($i=0; $i < sizeof($data); $i++)
        {
            ...
            $result[$i] = ['edit', $bom_2_source_id, $data[$i]['item_desc'],
    ...
    public function get_bom2_sampling_despatchh($id) {
        ...
        $sql = "SELECT * FROM tbl_bom_article_2 as a WHERE a.enquiry_id ='$id' AND a.flag=1 and a.despatch_approval=1";
        $data = $this->db->query($sql)->result_array();
    ...
    public function getAssortmentType() {
        $sql = "SELECT * FROM tbl_assortment_type WHERE flag=1";
        $data = $this->db->query($sql)->result_array();
        return $data;
    }
    
    public function getAssortmentDetails($id) {
        $sql = "SELECT * FROM tbl_wip_packing twp WHERE twp.flag=1 and twp.enquiry_id='$id'";
        $data = $this->db->query($sql)->result_array();
        if(sizeof($data) == 0) 
        {
            $mode = "add";
            $getsql = "SELECT a.po_size_wise_id, a.enquiry_id, a.pono_enq_refno FROM tbl_oe_po_wise as a
                       WHERE a.enquiry_id = ".$id." AND a.flag=1 group by a.pono_enq_refno";
            ...
            for($i = 0; $i < sizeof($data); $i++) 
            {
                ...
                if($type == "5" || $type == "6" || $type == "7" || $type == "8") 
                {
                    ...
                    $getTypeCombo = "SELECT pck_combo_color_id, pck_id, table_unique, combo_color, selected_size, 
                                     ...
                                     from tbl_pck_combo_color where pck_id='$pck_id' and enquiry_id='$id' and flag=1 group by table_unique";
    ...
    public function getPackingComboColor($id, $item) {
        $sql = "SELECT a.pono_enq_refno, b.combo_color_id, b.colour, b.combo as po_combo 
                       ...
                       WHERE a.enquiry_id = '$id' AND a.flag=1 and a.pono_enq_refno='$item'";
        $data = $this->db->query($sql)->result_array();
        return $data;
    }
    ...
    public function updateAssortmentDetailss($data) {
        ...
        for($i=0;$i<sizeof($packingData);$i++)
        {
            ...
            if($mode == "add") {
                ...
                $this->db->select('*');
                $ArrRes = $this->db->get_where('tbl_wip_packing', ['enquiry_id' => $enquiry_id, 'po_enq' => $po_enq, 'flag' => 1]);
    ...
    public function getAssortmentColumn($type, $id, $po_enq, $unique) {
        ...
        if($type == "1" || $type == "2" || $type == "3" || $type == "4") {
            ...
            $sql = "SELECT b.colour, b.combo as name, b.combo as id FROM tbl_oe_po_wise as a INNER JOIN tbl_oe_combo_color as b 
                        ON a.combo_color_id = b.combo_color_id WHERE a.enquiry_id = '$id' AND a.flag=1 and a.pono_enq_refno='$po_enq'";
            ...
            $getsql = "SELECT * FROM tbl_wip_packing as a inner join tbl_pck_combo_color as b on a.pck_id=b.pck_id 
                               WHERE a.enquiry_id = '$id' AND a.flag=1 and b.flag=1 and a.assortment_type='$type' and b.table_unique ='$unique'";
    ...
    public function getAssortmentData($type, $id, $po_enq, $assortData) {
        ...
        if($type == "1" || $type == "2" || $type == "3" || $type == "4") {
            ...
            $sql = "SELECT * FROM tbl_wip_packing as a inner join tbl_packing_assortment_details as b 
                    on a.pck_id=b.pck_id WHERE a.enquiry_id = '$id' AND a.flag=1 and b.flag=1 and a.assortment_type='$type'
                    and a.po_enq = '$po_enq'";
            $data = $this->db->query($sql)->result_array();
            ...
            foreach ($data as $key => $value)
            {
                ...
                on a.pck_id=b.pck_id WHERE a.enquiry_id = '$id' AND a.flag=1 and b.flag=1 and assortment_type='$type' group by b.table_unique";
                $data = $this->db->query($sql)->result_array();
                ...
                for($i=0; $i<sizeof($data); $i++) 
                {
                    ...
                    $getsql = "SELECT * FROM tbl_wip_packing as a inner join tbl_pck_combo_color as b on a.pck_id=b.pck_id 
                               WHERE a.enquiry_id = '$id' AND a.flag=1 and b.flag=1 and assortment_type='$type' and b.table_unique ='$unique'";
                    $getdata = $this->db->query($getsql)->result_array();
                    // print_r($getdata);
                    $uniqueResullt = [];
    ...
    public function getManagementChecklistDetailss($id) {
        ...
        for ($i=0; $i < sizeof($checkList); $i++) { 
            ...
            $sql2 = "SELECT * FROM tbl_management_checklist WHERE enquiry_id='$id' AND checklist_id='$checkListId' AND flag=1";
            ...
            if(sizeof($list) == 1)
            {
                if($list[0]['NA'] == '0' && $list[0]['NR'] == '0' && $list[0]['PR'] == '0' && $list[0]['record'] == '0' && $list[0]['req_sent_for_missing_details'] == '0') {
                    ...
                    $list[0]['record'], $list[0]['req_sent_for_missing_details'], $list[0]['req_date'], $list[0]['receive_date'], '',$list[0]['remarks']];
                }
                else {
                    $list = ['edit', $checkList[$i]['checklist_id'], $checkList[$i]['description'], $list[0]['NA'], $list[0]['NR'], $list[0]['PR'], 
                    $list[0]['record'], $list[0]['req_sent_for_missing_details'], $list[0]['req_date'], $list[0]['receive_date'], $list[0]['log'],$list[0]['remarks']];
                }
    ...
    public function getFinalInspectionStandardDetailss($id) {
        ...
        for ($i=0; $i < sizeof($poWise); $i++) { 
            ...
            $sql2 = "SELECT * FROM tbl_final_inspection_standard WHERE enquiry_id='$id' AND po_size_wise_id='$poSizeWiseId' AND flag=1";
            $list = $this->db->query($sql2)->result_array();
            // return $list;
            if(sizeof($list) == 1)
            {
    ...
    public function updateFinalInspectionStandardd($req_data, $id) {        
        foreach ($req_data as $key => $value)
        {
            ...
            $finalInspection["fi_offered_date"] = $value[15];
    ...
    public function getDetailsOfConsigneeLogisticss($id) {
        ...
        for ($i=0; $i < sizeof($poWise); $i++) { 
            ...
            $poSizeWiseId = $poWise[$i]['po_size_wise_id'];
            $sql2 = "SELECT * FROM tbl_documentation_consignee WHERE enquiry_id='$id' AND po_size_wise_id='$poSizeWiseId' AND flag=1";
            $list = $this->db->query($sql2)->result_array();
            if(sizeof($list) == 1)
            {
    ...
    public function getColourWiseGarmentPartsDetailss($id) {
        ...
        FROM tbl_fab_color_wise_garment_parts WHERE enquiry_id='$id' AND flag=1
    ...
    public function getGarmentPartsWiseQtyDetailss($id) {
        ...
        $color_wise_sql = "SELECT a.*,b.gpdname FROM tbl_fab_color_wise_garment_parts a INNER JOIN kn_master_garment_part_desc b ON a.gar_parts=b.id WHERE a.enquiry_id='$id' AND a.flag=1 ORDER BY a.fab_color_wise_garment_part_id ASC";
        $color_wise_data = $this->db->query($color_wise_sql)->result_array();
    ...
    public function getSizeWiseGarmentPartsDetailss($id) {
        ...
        $consl_sql = "SELECT * FROM tbl_fab_garment_piece_weight WHERE enquiry_id='$id' AND flag=1";
    ...
    public function getFabricConsumptionCalcDetailss($id) {
        ...
        for($i=0; $i < sizeof($color_wise_data); $i++) {
            ...
            $getData = "SELECT * FROM tbl_fab_garment_piece_weight WHERE enquiry_id = $id AND flag=1";
    ...
    public function get_sizewise_dia_dimensionn($id) {
        ...
        $consl_sql = "SELECT * FROM tbl_fab_final_dia_dim WHERE enquiry_id='$id' AND flag=1 ORDER BY fab_final_dia_dim_id asc";
        $consl_data = $this->db->query($consl_sql)->result_array();
    ...
    public function getYarnProgrammeDetailss($id) {
        ...
        $sql2 = "SELECT yarn_spe_req, yarn_purchase_type FROM tbl_fab_itemized_fabric_requirement WHERE enquiry_id = '$id' ";
        $yarnData = $this->db->query($sql2)->result_array();
    ...
    public function getYarnRequirementDetailss($id) {
        ...
        $sql2 = "SELECT * FROM tbl_fab_yarn_requirement WHERE enquiry_id = '$id' AND flag = 1 ";
        $yarnData = $this->db->query($sql2)->result_array();
    ...
    public function getKnittingProgrammeDetailss($id) {
        ...
        $sql2 = "SELECT * FROM tbl_fab_knitting_programme WHERE enquiry_id = '$id' AND flag = 1";
    ...
    public function getLabTestingAcceptanceInternalDetailss($id) {
        ...
        $lab_sql = "SELECT * FROM tbl_lab_testing_acceptance_internal WHERE enquiry_id = " . $id . " AND flag=1";
    ...
    public function getLabTestingAcceptanceExternalDetailss($id) {
        ...
        $lab_sql = "SELECT * FROM tbl_lab_testing_acceptance_external WHERE enquiry_id = " . $id . " AND flag=1";
        $lab_data = $this->db->query($lab_sql)->result_array();
    ...
    public function getExternalLabTestingAuthorityDetailss($id) {
        ...
        $sql = "SELECT * FROM tbl_external_lab_testing_authority telta INNER JOIN ".KN_MASTER_TESTINGAUTHORITY." kmel ON telta.lab_testing_authority_id=kmel.id WHERE telta.enquiry_id = " . $id . " AND telta.flag=1";
        $data = $this->db->query($sql)->result_array();
        $result = [];
        foreach ($data as $key => $value)
        {
    ...
    public function getCADCommonTableDetailss($enqId) {
        $sql = "SELECT *,a.cutoff_date as req_cutoff_date, b.ref_queue_no as cad_ref_no, a.ref_queue_no as cad_ref_queue_no ,a.log as qno_assign_dt,b.log as job_status_upd_dt FROM tbl_request a
                ...
                WHERE a.enquiry_id = " . $enqId . " AND a.flag=1 AND a.type=1";
        ...
        foreach ($data as $key => $value)
        {
            ...
            $reqdetails[$key] = [ $value['request_id'], $value['po_enq_ref_id'], $value['combo_id'],
                            ...
                            $authStatus[$value['auth_status']], $reqStatus[$value['req_status']], $value['cad_ref_queue_no'], $value['qno_assign_dt']
                        ];
    ...
    public function getSampleCommonTableDetailss($enqId) {
        ...
        $sql = "SELECT *,a.cutoff_date as req_cutoff_date, b.ref_queue_no as sample_ref_no 
                FROM tbl_request a
                ...
                WHERE a.enquiry_id = " . $enqId . " AND a.flag=1 AND a.type=2";
        $query = $this->db->query($sql);
        $data = $query->result_array();
        ...
        foreach ($data as $key => $value)
        {
            ...
            $reqdetails[$key] = [ $value['request_id'], $value['po_enq_ref_id'], $value['combo_id'],
                            ...
                            $authStatus[$value['auth_status']], $reqStatus[$value['req_status']], $value['ref_queue_no'], $value['qno_assign_dt']
                        ];
    ...
    public function getBomCommonTableDetailss($enqId) {
        $sql = "SELECT b.*,c.*,d.pi_ref_queue_no,d.exp_dod as exp_dods,d.pi_dt,e.qty as pi_qty,e.uom as pi_uom FROM tbl_request_bom b
                ...
                WHERE c.enquiry_id = " . $enqId . " AND b.flag=1";
        $data = $this->db->query($sql)->result_array();
        ...
        foreach ($data as $key => $value)
        {
            ...
            $pidetails[$key] = [ $value['request_bom_id'], $value['item_desc'], $value['garment_size'], $value['appr_item_code'],
                            $value['appr_item_col_code'], $value['size_dim'], $value['uom'], $value['pi_dt'], $value['pi_ref_queue_no'], $value['pi_qty'], $value['pi_uom'], $value['pi_appl_status'], $value['pi_appl_req_date_time'],$value['exp_dods'], $value['purchase_req_type']
                        ];
        ...
        $sql1 = "SELECT b.*,c.plan_bom_qty,e.pi_ref_queue_no FROM tbl_bom_in_house b
                ...
                WHERE d.enquiry_id = " . $enqId . " AND b.flag=1 ";
        ...
        $sql2 = "SELECT c.*, b.order_stock_status,b.received_uom, b.supply_closure_status,b.supply_closure_date, SUM(b.received_qty) as received_qtys, c.plan_bom_qty FROM tbl_request_bom c
                ...
                LEFT JOIN tbl_request d on c.request_id=d.request_id
                WHERE d.enquiry_id = " . $enqId . " AND c.flag=1 GROUP BY c.item_desc,c.garment_size,c.appr_item_code,c.appr_item_col_code,c.size_dim,c.uom ";
