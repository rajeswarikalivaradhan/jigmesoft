...
class Bominstrumenttypemodel extends CI_Model {
    function saveInfo($ArrSaveData) {
        ...
        if ($VarId == "") {
            ...
            if (empty($ArrCheckExist)) {
                ...
                $this->db->insert(KN_BOM_MASTER, $ArrSaveData);
            ...
            if (empty($ArrCheckExist)) {
                if ($this->db->update(KN_BOM_MASTER, $ArrSaveData, array('id' => $VarId))) {
                    $ArrResult['errcode'] = 1;
                    $ArrResult['msg'] = '';
                    $ArrResult['id'] = $VarId;
    ...
    function fnGetInfo($VarData, $VarStatus, $VarId = '',$VarCompanyId='') {
        $this->db->select('id,description,status')->from(KN_BOM_MASTER);
        if ($VarData <> '') {
    ...
    function fnCount($Vardescription = '', $VarStatus = '') {
        ...
        $VarSqlLab = "SELECT count(1) as trec  FROM " . KN_BOM_MASTER . " AS b " . $VarWhere;
    ...
    function fnList($Vardescription = '', $VarStatus = '', $VarLimit = 10, $offset = 0, $VarSortBy, $VarSortOrder) {
        ...
        //               FROM " . KN_BOM_MASTER . " AS b " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder . " limit " . $VarLimitInfo;
        ...
        $VarSqlLab = "SELECT b.id,b.description,b.datecreated,DATE_FORMAT(b.dateupdated,\"%d-%m-%Y\ %H:%i %p\") as formattedDateUpdated,b.status,contactname
                      FROM " . KN_BOM_MASTER . " AS b LEFT JOIN " . KN_USERS . " AS u ON b.updatedby = u.id " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder;
    ...
    function fnChangeComStat($checkboxids, $optvalue) {
        ...
        if ($this->db->update(KN_BOM_MASTER, array('status' => $optvalue))) {
            return true;
        }
        return false;
    }
