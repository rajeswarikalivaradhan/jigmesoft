...
class membellishmentvendormodel extends CI_Model {
    function fnGetInfo($VarData = '', $VarStatus = '', $VarId = '')
    {
        ...
        return $this->db->from(KN_MASTER_EMBELLISHMENT_VENDOR)->get()->result_array();
    }
    function fnCheck($VarData = '', $VarId = '') {
        $this->db->from(KN_MASTER_EMBELLISHMENT_VENDOR);
    ...
    function saveInfo($ArrUpdateData) {
        ...
        if ($VarId == "") {
            ...
            if (empty($ArrCheckExist)) {
                ...
                $this->db->insert(KN_MASTER_EMBELLISHMENT_VENDOR, $ArrUpdateData);
            ...
            if (empty($ArrCheckExist)) {
                if ($this->db->update(KN_MASTER_EMBELLISHMENT_VENDOR, $ArrUpdateData, array('id' => $VarId))) {
                    $ArrResult['errcode'] = 1;
                    $ArrResult['msg'] = '';
                    $ArrResult['id'] = $VarId;
    ...
    function fnCount($VarVendor = '', $VarContatname = '',$VarEmailId='', $VarStatus = '', $VarAfilter = '') {
        ...
        $VarSqlLab = "SELECT count(1) as trec  FROM " . KN_MASTER_EMBELLISHMENT_VENDOR . " AS b INNER JOIN ".KN_USERS." AS u ON b.updatedby = u.id WHERE " . $VarWhere;
...
FROM " . KN_MASTER_EMBELLISHMENT_VENDOR . " AS b INNER JOIN ".KN_USERS." AS u ON b.updatedby = u.id WHERE " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder . " limit " . $VarLimitInfo;
    ...
    function fnChangeComStat($checkboxids, $optvalue) {
        ...
        if ($this->db->update(KN_MASTER_EMBELLISHMENT_VENDOR, array('status' => $optvalue))) {
            return true;
        }
        return false;
    }
