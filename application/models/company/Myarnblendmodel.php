...
class myarnblendmodel extends CI_Model {
    ...
    function saveInfo_backup($ArrSaveData) {
        $VarId = $ArrSaveData['id'];
        $ArrResult = [];
        if ($VarId == "") {
    ...
    function fnCount($Varmisc_name = '', $VarStatus = '') {
        ...
        $VarSqlLab = "SELECT count(1) as trec  FROM " . KN_MASTER_YARN_MISC . " AS b " . $VarWhere;
...
FROM " . KN_MASTER_YARN_MISC . " AS b LEFT JOIN " . KN_USERS . " AS u ON b.updatedby = u.id " . $VarWhere . " order by " . $VarSortBy . " " . $VarSortOrder;
