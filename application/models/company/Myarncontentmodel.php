...
class myarncontentmodel extends CI_Model {
    ...
    function saveInfobackup($ArrSaveData) {
        $VarCompanyId = $ArrSaveData['companyid'];
        $VarId = $ArrSaveData['id'];
        $ArrResult = [];
        if ($VarId == "") {
            ...
            if (empty($ArrCheckExist)) {
                ...
                $ArrResult['errcode'] = -1;
                $ArrResult['msg'] = "This yarn content already exists";
            }
        } else {
    ...
    function fnList($Varmisc_name = '', $VarStatus = '', $VarLimit = 10, $offset = 0, $VarSortBy, $VarSortOrder, $VarAfilter = '') {
        ...
        $ArrWhere = array();
        if ($Varmisc_name <> '') {
            $ArrWhere[] = "y.misc_name like '%" . $Varmisc_name . "%'";
        }
        if ($VarAfilter <> '') {
            $ArrWhere[] = "y.misc_name LIKE '" . $VarAfilter . "%'";
        }
        if ($VarStatus <> '') {
            $ArrWhere[] = "y.status=" . $VarStatus;
        } else {
        ...
        // $VarSqlLab = "SELECT id,misc_name,datecreated,DATE_FORMAT(dateupdated,\"%d-%m-%Y\ %H:%i %p\") as formattedDateUpdated,status FROM " . KN_MASTER_YARN_MISC . " AS y $VarWhere order by " . $VarSortBy . " " . $VarSortOrder;
        // $ObjResult = $this->db->query($VarSqlLab);
        return $ObjResult;
    }
