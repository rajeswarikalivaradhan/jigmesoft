$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/mqausers/getSampleQAQueueList',
			type:'POST',
		...
		$("#QaQueueListTbl").dataTable({
			...
			"aoColumns": [			
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						if(data.type == 1) {
							return '<a class="bold" href="' + base_path +'request/Cadrequest/queuelist' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/cadId/' + encodeURIComponent(btoa(data.cad_requirement_id)) + '">' + data.isriorcode + '</a>';
						}
						else if(data.type == 2) {
							return '<a class="bold" href="' + base_path +'request/Samplerequest/queuelist' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/samId/' + encodeURIComponent(btoa(data.sample_requirement_id)) + '">' + data.isriorcode + '</a>';
						}
					}
				},
				{ "mDataProp": "brandname" },	
				{ "mDataProp": "sam_queue_no" },	
				{
					"mDataProp": function ( data, type, full, meta) {
						if(data.type == 1)
						{
