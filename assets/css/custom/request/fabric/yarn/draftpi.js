$(document).ready(function () {
    ...
    function getDraftPIRequest() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Fabricrequest/getDraftPIRequestDetails',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                if(yarn_requirement_data.fullData.length > 0) {
                    req_yarn_id = yarn_requirement_data.fullData[0].request_yarn_id;
                }
                ...
                appendAddressField(yarn_requirement_data.vendor_data);
            },
            error: function () {
                console.log("Error");
            }
    ...
    function append_purchase_request(data, tblData = []) {
        ...
        let list = {
            ...
            columns: [
                ...
                { type: 'text', title: 'Proforma No.', width: '6%', align: 'left'},
                { type: 'calendar', title: 'Proforma\n Date', width: '6%', align: 'left'},
                { type: 'text', title: 'Proforma\n Value', width: '6%', align: 'left'},
    ...
    function updateFunction(data, purchaseData) {
        ...
        dataform.append('supply_lead_time', $('#supply_lead_time').val());
        dataform.append('payment_terms', $('#payment_terms').val());

        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Fabricrequest/updatePurchaseIndent',
            data: dataform,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                setTimeout(() => {
                    window.location.href = base_path + 'request/Fabricrequest/reqsentlist';
                }, 1000);
            },
            error: function () {
                console.log("Error");
    ...
    function appendAddressField(vendorData) {
        ...
        for(let i=0; i < vendorData.length; i++) {
            vendorOption = vendorOption + "<option value='"+vendorData[i].id+"' data-index='"+i+"'>"+vendorData[i].yarnvendor+"</option>";
        }
    ...
    $("#vendorOption").change(function() {
        ...
        if(optionValue !="") {
            ...
            append_purchase_request(yarn_requirement_data, purchaseVendorData);
