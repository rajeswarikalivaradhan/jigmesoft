$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'request/Fabricrequest/getAllRequestReceivedList',
			type:'POST',
			...
			"aoColumns": [
				{ 
					"mDataProp": function ( data, type, full, meta) {
                        if(data.type == 1)
						return '<a class="bold" href="' + base_path +'request/Cadrequest/reqreceiveddetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
                        else if(data.type == 2)
						return '<a class="bold" href="' + base_path +'request/Samplerequest/reqreceiveddetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
                        else if(data.type == 3)
						return '<a class="bold" href="' + base_path +'request/Bomrequest/reqreceiveddetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
                        else if(data.type == 4)
						return '<a class="bold" href="' + base_path +'request/Bom2request/reqreceiveddetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
                        else if(data.type == 5)
						return '<a class="bold" href="' + base_path +'request/Fabricrequest/reqreceiveddetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
                        else
