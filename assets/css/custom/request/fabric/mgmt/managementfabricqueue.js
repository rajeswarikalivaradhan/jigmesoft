$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/mqausers/getfabricQueueList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#merchantBomQueueTbl").dataTable({
			...
			"aoColumns": [
				...
				{
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Fabricrequest/managementqueue' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
					}
				},
				{ "mDataProp": "brandname" },	
				{
