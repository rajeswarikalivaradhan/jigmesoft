$(document).ready(function() {
    ...
    var reqRequestJSON;
	$.when(getOrderStockList()).done(function(){
	...
	function getOrderStockList()
	{
		return $.ajax({
			url: base_path+'request/myarnstore/getGeneralStockList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/myarnstote/generalstockdetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '"> Item - View Details </a>';
					}
				},		
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return data.blend+' / '+data.content+' / '+data.material;
					}
				},				
				{ "mDataProp": "garment_size" },
				{ "mDataProp": "appr_item_code" },
				{ "mDataProp": "appr_item_col_code" },
				{ "mDataProp": "size_dim" },
				{ "mDataProp": "uom" },	
				{ "mDataProp": "plan_bom_qty" },
				{
					"mDataProp": function ( data, type, full, meta) {
