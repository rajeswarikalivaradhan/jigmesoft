	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Bomrequest/managementpurchaseindentapprovaldetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '"> P.I.APPROVAL </a>';
					}
				},	
				{ 
					"mDataProp": function ( data, type, full, meta) {
