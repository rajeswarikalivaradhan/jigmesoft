$(document).ready(function() {
    ...
    var reqRequestJSON;
	$.when(getBom2QueueList()).done(function(){
	...
	function getBom2QueueList()
	{
		return $.ajax({
			url: base_path+'company/mqausers/getBom2QueueList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#merchantBomQueueTbl").dataTable({
			...
			"aoColumns": [		
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Bomrequest/merchantqueue' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
					}
				},		
				{ "mDataProp": "brandname" },	
				{ 
