$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/Mstoreuser/getOrderClosurelist',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				// { 
				// 	"mDataProp": function ( data, type, full, meta) {
				// 		return '<a class="bold" href="' + base_path +'request/Bomrequest/orderstockdetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/itemCode/' + encodeURIComponent(btoa(data.appr_item_code)) + '/pId/' + encodeURIComponent(btoa(data.purchase_indent_id)) + '">'+data.isriorcode+'</a>';
				// 	}
				// },
