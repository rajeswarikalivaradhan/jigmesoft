$(document).ready(function () {
    ...
    function _call_to_bom_artcle_2() {
        ...
        get_bom2_sampling_despatch(); // BOM 1 sample despatch
    }
    ...
    function append_bom_sampling_approval_details(data) {
        ...
        let bom_sampling_details_wise = {
            ...
            columns: [
                ...
                { title: 'Sample Despatch \nFor Appl.', align: 'center', type: 'checkbox' }
            ],
            minDimensions: [4, 1],
            allowDeleteColumn: false,
            allowInsertRow: true,
    ...
    function append_bom_requirement_details(data) {
        ...
        let bom1_requirement_details = {
            ...
            onchange: function(instance, cell, col, row, val, label, cellName) {
                ...
                //     changeStatus = false;
                //     updatedRow = row;
                //     txt = bom1_requirement_details.data[row][14];
                //     dd = data.appendData.calculatedAmmount;
                //     if(txt != '')
                //     {
                //         let po_ref = bom1_requirement_details.data[row][2];
                ...
                //                 item.colour == colour && item.spec == spec ) return true;
                //         })
                //         let qty = bom1_requirement_details.data[row][14];
                //         let count = bom1_requirement_details.data[row][15];
                //         reqQty = parseInt(qty) * parseInt(count);
                //         instance.jexcel.options.data[row][16] = reqQty;
                //     }
                //     else
                //     {
                    ...
                    let options = {
                        ...
                        updateTable: function(instance, cell, col, row, val, label, cellName) {
                            ...
                            if(assortment_type == '7') {
                                ...
                                if (col >= 3 && col < tablelength - 3 && row != table_data.length - 1) 
                                {
                                    let a = instance.jexcel.options.data[row][col];
                                    if(a == '') { 
                                        a = 0;
                                ...
                                if(col == tablelength - 3 && row != table_data.length - 1) {
                                    instance.jexcel.options.data[row][col] = num_of_item;
                                    $(cell).text(num_of_item);
                                }
                                if(col == tablelength - 2 && row != table_data.length - 1) {
                                    prevVal = instance.jexcel.options.data[row][col];
                                }
                                if(col == tablelength - 1 && row != table_data.length - 1) 
                                {   
                                    console.log(row, table_data.length)
                                    if(prevVal == '') { 
                                        prevVal = 0;
                                    }
                                    total_item = parseInt(num_of_item) * parseInt(prevVal);
                                ...
                                if(row == table_data.length - 1 && (col == table_column.length - 1) || 
                                   row == table_data.length - 1 && (col == table_column.length - 2)) {
                                    let calVall = 0;
                                    for(let k=0; k < table_data.length; k++) {
                                        let tbRow = table_data[k];
                                        if(tbRow == "") { tbRow = 0}
                                        if(k < table_data.length - 1) {
                                            if(tbRow[col] == "") { tbRow[col] = 0}
                                            calVall += parseInt(tbRow[col]);
                                        }
                                    }
                                    instance.jexcel.options.data[row][col] = calVall;
                            ...
                            if(assortment_type == '8') {
                                ...
                                if (col >= 3 && col < tablelength - 1 && row != table_data.length - 1) 
                                {
                                    let a = instance.jexcel.options.data[row][col];
                                    if(a == '') { a = 0; }
    ...
    var GlbApprovingauthority = ['Buyer', 'Buyer Agent', 'Buyer Office', 'Buyer Office Agent', 'Third Party'];
        ...
        let final_inspection_standard = {
            ...
            columns: [
                ...
                { title: 'Sample Size\n Code Letter', width: '8%', align: 'left', readOnly: true },
                { title: 'Sample Size', width: '8%', align: 'right', readOnly: true },
                { title: 'Critical\n AQL', width: '8%', align: 'center', type: 'dropdown', source: Aql },
                { title: 'Major\n AQL', width: '8%', align: 'center', type: 'dropdown', source: Aql },
                { title: 'Minor\n AQL', width: '8%', align: 'center', type: 'dropdown', source: Aql },
                { title: 'Inspection\n Authority', width: '8%', align: 'center', type: 'dropdown', source: GlbApprovingauthority },
    ...
    function append_details_of_consignee_logistics(data) {
        ...
        let consignee_details_list = {
            ...
            columns: [
                ...
                { title: 'Consignor / Shipper / Exporter', width: '8%', align: 'center', type: 'dropdown', source: data.consignorAgent},
                { title: 'Clearing Agent: Name / Address /\n Contact Person  / Mobile No.', width: '8%', align: 'center', type: 'dropdown', source: data.clearingAgent},
                { title: 'Forwarding Agent: Name / Address /\n Contact Person  / Mobile No.', width: '8%', align: 'center', type: 'dropdown', source: data.forwadingAgent},
                { title: 'Importer - If other than Consignee: Name /\n Address / Contact Person  / Mobile No.', width: '8%', align: 'center', type: 'dropdown', source: data.importerAgent},
                { title: 'Consignee: Name / Address / Contact\n Person  / Mobile No.', width: '8%', align: 'center', type: 'dropdown', source: data.consigneeAgent}
            ],
            minDimensions: [4, 1],
            allowDeleteColumn: false,
            allowInsertRow: true,
    ...
    function append_lab_testing_acceptance_internal_details(data) {
        ...
        let lab_testing_acceptance_internal_list = {
            ...
            columns: [
                ...
                { title: 'Req. GSM', width: '8%', align: 'left', readOnly: true },
                { title: 'Req. DIA /\n DIM (Inches)', width: '8%', align: 'left', readOnly: true },
                { title: 'Shrink. Acc.\n Level (L)', width: '8%', align: 'center', type: 'dropdown', source: data.perData},
                { title: 'Shrink. Acc.\n Level (W)', width: '8%', align: 'center', type: 'dropdown', source: data.perData},
                { title: 'Spirality Acc.\n Level', width: '8%', align: 'center', type: 'dropdown', source: data.perData},
                { title: 'Crocking Acc.\n Level (Dry)', width: '8%', align: 'center', type: 'dropdown', source: data.gradeData},
                { title: 'Crocking Acc.\n Level (Wet)', width: '8%', align: 'center', type: 'dropdown', source: data.gradeData},
                { title: 'Fastness Acc.\n Level (Shade)', width: '8%', align: 'center', type: 'dropdown', source: data.gradeData},
                { title: 'Fastness Acc.\n Level (Stain)', width: '8%', align: 'center', type: 'dropdown', source: data.gradeData},
                { title: 'Testing\n Authority', width: '8%', align: 'center', type: 'dropdown', source: data.testingAuthority},
                { title: 'Approving\n Authority', width: '8%', align: 'center', type: 'dropdown', source: data.approvingAuthority}
            ],
            minDimensions: [4, 1],
            allowDeleteColumn: false,
            allowInsertRow: true,
    ...
    function append_lab_testing_acceptance_external_details(data) {
        ...
        let lab_testing_acceptance_external_list = {
            ...
            columns: [
                { title:'mode', width:'0%',align:'center',type:'hidden'},
                { title:'id', width:'0%',align:'center',type:'hidden'},
                { title: 'Combo / Colour', width: '8%', align: 'left', type: 'dropdown', source: [] },
                { title: 'Item Description', width: '8%', align: 'left', type: 'dropdown', source: [] },
