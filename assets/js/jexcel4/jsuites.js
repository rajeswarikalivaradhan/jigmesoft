    ...
    obj.getDays = function() {
        ...
        for (var i = j; i < (Math.ceil((nd + fd) / 7) * 7); i++) {
            ...
            if (d) {
                ...
                // Disabled
                if (obj.options.between) {
                    if (! obj.options.between[0] || current >= obj.options.between[0]) {
                        var test1 = true;
                    } else {
                        var test1 = false;
                    }

                    if (! obj.options.between[1] || current <= obj.options.between[1]) {
                        var test2 = true;
                    } else {
                        var test2 = false;
                    }
            ...
            // Sundays
            if (! (i % 7)) {
                cell.classList.add('jcalendar-sunday')
            }
...
jSuites.tabs = (function(el, options) {
    ...
    if (obj.options.border == true) {
        ...
        var setBorder = function(index) {
            ...
            border.style.top = rect.top + rect.height - 1 + 'px';
