...
function fnSearchCadReq() {
    var WipRefNo     					        = $("#frmSrchWipRefNo").val();
    var IsrIortype     					        = $("#frmSrchIsrIortype").val();
    ...
    GlbSearchParam				= "rfrom=1&wip="+WipRefNo+"&isriortype="+IsrIortype+"&cutfrom="+CutOffFrom+"&cutto="+CutOffTo+"&cs="+CStatus+"&afilter="+
        GlbFilterAlpha+"&columnId="+GlbColumnId+"&sortorder="+GlbSortOrder+"&reqtype="+ReqType+"&merid="+Merchant+"&bb="+BB;
    $("#DivTotalCntResult").html('');
    MakePostRequest(base_path+'management/managemgmtcadrequest',GlbSearchParam,'json',fnListCadRequestRes);
}

function fnListCadRequest() {
    $("#DivTotalCntResult").text('');
    GlbSearchParam = 'rfrom=1';
    MakeAsynPostRequest(base_path+'management/managemgmtcadrequest',GlbSearchParam,'json',fnListCadRequestRes);
}
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><input type="checkbox" class="allcbox" id="'+value.id+'"></td>' +
                                ...
                                '<td>'+value.at+'</td>' +
                                '<td>'+value.m+'</td>' +
                                '<td><a href="'+base_path+'management/mgmtcadauthorizing/'+encodeURIComponent(base64_encode(value.id))+'">'+value.mgmts+'</a></td>'+
                                '<td>'+value.ru+'</td>'+
                                '<td>'+value.s+'</td>';
...
function fnChangeStatusRes(data) {
    ...
    $('#mBomSrcList').on('click', 'th.sortable', function () {
        ...
        MakePostRequest(base_path+'merchant/managecadrequest',GlbSearchParam,'json',fnListRes);
    });
...
function fnPaginationMgmtCadReqAuth(VarURL) {
