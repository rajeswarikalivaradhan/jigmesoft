...
function fnList() {
    ...
    MakeAsynPostRequest(base_path+'management/approvallist',GlbSearchParam,'json',fnListRes);
}
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value) {
                            PageContent=PageContent+'<tr><td><input type="checkbox" class="allcbox" id="'+value.id+'"></td>' +
                                ...
                                '<td>'+value.ven+'</td>' +
                                '<td><a href="'+base_path+'management/bomPurchasePayemntReq/'+value.id+'">'+value.pino+'</a></td>' +
                                '<td>'+value.req+'</td>' +
                                '<td>'+value.reqdt+'</td>' +
                                '<td>'+value.cutoff+'</td>' +
