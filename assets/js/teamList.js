function fnSearch() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'merteam/teamList', GlbSearchParam, 'json', fnListRes);
}
function fnList() {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="'+base_path+GlbCompanyFdr+'merteam/addeditteammates/'+encodeURIComponent(base64_encode(value.id))+'">' +
                                '<i class="fa fa-edit"></i>&nbsp;Team Members</a>' +
                                '</td>';
