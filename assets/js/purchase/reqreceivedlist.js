...
function fnSearchSamReceivedList() {
    ...
    MakePostRequest(base_path+'purchaseuser/purchasereceivedlist',GlbSearchParam,'json',fnRecdListRes);
}

function fnRecdList() {
    $("#DivTotalCntResult").html('');
    GlbSearchParam = 'rfrom=1';
    MakeAsynPostRequest(base_path+'purchaseuser/purchasereceivedlist',GlbSearchParam,'json',fnRecdListRes);
}
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><input type="checkbox" class="allcbox" id="'+value.id+'"></td>' +
                                ...
                                '<td><a href="'+base_path+'purchaseuser/queuenoassign/'+encodeURIComponent(base64_encode(value.id))+'"> '+value.wip+'</a></td>' +
                                '<td>'+value.bb+'</td>' +
                                '<td>'+value.r+'</td>' +
                                '<td>'+value.reqtype+'</td>' +
                                '<td>'+value.reqdt+'</td>' +
...
function fnChangeStatusRes(data) {
    ...
    $('#SamReceivedListTbl').on('click', 'th.sortable', function () {
        ...
        MakePostRequest(base_path+'purchaseuser/purchasereceivedlist',Param,'json',fnRecdListRes);
    });
