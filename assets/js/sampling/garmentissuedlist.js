...
$(document).ready(function() {
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#sampleQueueListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Samplerequest/garmentissueddetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/samId/' + encodeURIComponent(btoa(data.sample_requirement_id)) + '">' + data.dc_ref_queue_no + '</a>';
					}
				},
				{
					"mDataProp": function ( data, type, full, meta) {
    ...
    if (Status1) { // only if a value is selected
        table.column(10).search('^' + Status1, true, false); // true = regex, false = smart
    } else {
...
//   }
	...
	var gdc_no = $('#gdc_no').val().toLowerCase();
    var sample_ref_no   = $('#sample_ref_no').val().toLowerCase();
   var material_issue = $('#material_issue').val().toLowerCase();
    var material_recived   = $('#material_recived').val().toLowerCase();
   

    table
      .column(1).search(wip_ref_no)
      .column(2).search(brandId)
      .column(4).search(gdc_no)
      .column(6).search(sample_ref_no)
       .column(7).search(material_issue)
      .column(8).search(material_recived)
