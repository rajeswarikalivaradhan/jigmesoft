...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/msamplinguser/getReqRequestList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#SamReceivedListTbl").dataTable({
			...
			"aoColumns": [
				...
				{
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Samplerequest/department' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.isriorcode + '</a>';
					}
				},
                { "mDataProp": "brandname" },
                { 
				...
				{ "mDataProp": "auth_name" },
				{
					"mDataProp": function ( data, type, full, meta) {
                        ...
                        // return '<span class="text-light knRedColor bg-dark"><strong>REQ. REJECTED</strong></span>';
					}
				},
				{
					"mDataProp": function ( data, type, full, meta) {
    ...
    //             var indentTbls = ['tbl_request','tbl_request_sample'];
            ...
            if (checkBoxLength >= 1) {
                ...
                swalWithBootstrapButtons.fire(
					...
					if (result.value) {
                        ...
                        '&tblname=tbl_request_sample'+'&idName=request_id', 'json', function (data) {
                            $.when(getReqRequestList()).done(function(){
