...
$(document).ready(function() {
    ...
    var reqRequestJSON;
	$.when(getReqSentList()).done(function(){
	...
	function getReqSentList()
	{
		return $.ajax({
			url: base_path+'company/msamplinguser/getReqSentList',
			type:'POST',
	...
	{
		if ( $.fn.DataTable.isDataTable('#SamReceivedListTbl') ) {
		  $('#SamReceivedListTbl').DataTable().destroy();
		}

        var i = 1;

		$('#SamReceivedListTbl tbody').empty();	
		$("#SamReceivedListTbl").dataTable({
            "aaData": reqRequestJSON,
            "aaSorting": [],
			"aoColumns": [
                {
                    "mDataProp": function(data, type, full, meta) {
                        return '<input type="checkbox" class="allcbox" id="'+data.qa_req_id+'">';
                    }
                },
				{
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Samplerequest/samplesentlist' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/samId/' + encodeURIComponent(btoa(data.qa_req_ids)) + '">' + data.isriorcode + '</a>';
					}
				},
                { "mDataProp": "brandname" },
                { 
            ...
            if (checkBoxLength >= 1) {
                ...
                swalWithBootstrapButtons.fire(
					...
					if (result.value) {
                        ...
                        '&tblname=tbl_sample_requirement'+'&idName=qa_req_id', 'json', function (data) {
                            $.when(getReqSentList()).done(function(){
                                dispDetails(reqRequestJSON);  
  ...
  if(cutfromDate!=='' && cuttoDate!==''){
    ...
    var table = $('#SamReceivedListTbl').DataTable();
