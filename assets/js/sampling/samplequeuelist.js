...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/msamplinguser/getQueueList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#sampleQueueListTbl").dataTable({
			...
			"aoColumns": [		
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Samplerequest/qa' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.ref_queue_no + '</a>';
					}
				},
