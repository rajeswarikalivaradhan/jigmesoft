...
function fnSearchConsignee() {
    var frmBasicMobileNo = $("#frmSrchMobileNo").val();
    var frmSrchVendorConsigneeName = $("#frmSrchConsigneeName").val();
    var frmSrchEmailid = $("#frmSrchEmailid").val();
    var Status = $("#frmSrchBomStatus").val();
    GlbSearchParam = "rfrom=1&m=" + frmBasicMobileNo + "&cn=" + frmSrchVendorConsigneeName + "&e=" + frmSrchEmailid + "&s=" + Status + "&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mconsignee/manageconsignee', GlbSearchParam, 'json', fnListRes);
}
    ...
    if (data != '') {
        if (data.errcode != undefined) {
            if (data.errcode == '404') {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'mconsignee/addedit/' + encodeURIComponent(base64_encode(value.id)) + '">' + value.vcn + '</a></td>' +
                                '<td>' + value.addr + '</td>' +
                                '<td>' + value.e + '</td>' +
                                '<td>' + value.p + '</td>' +
                                '<td>' + value.m + '</td>' +
                                ...
                                '<td>' + value.s + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'mconsignee/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '"><i class="fa fa-edit"></i>&nbsp;Edit</a></td>';
                ...
                $('tbody').empty();
                $('#mconsigneeList').append(PageContent);
            }
        }
    }
...
function fnSaveembellishmentvendorInfo() {
    ...
    $.ajax({
        url: base_path + GlbCompanyFdr + 'mconsignee/updateInfo',
        data: MyFormData,
        contentType: false,
        processData: false,
        dataType: "json",
...
function fnSaveBomRes(data) {
    ...
    if (data != '') {
        if (data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'mconsignee/addedit/' + data.eid);
        }
    }
}
...
$('#mconsigneeList').on('click', 'th.sortable', function () {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mconsignee/manageconsignee', GlbSearchParam, 'json', fnChangeStatusRes);
});
    ...
    if (parseInt(StatusOptSelVal) > 0) {
        ...
        if (parseInt(ArrItemCheckBoxSel[1]) >= 1) {
            if (confirm('Do you want to ' + GlbStatusForMaster[StatusOptSelVal] + ' this records?')) {
                ...
                MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mconsignee/changemStatus', GlbSearchParam, 'json', fnSearchBomVendor);
            }
        }
    } else {
