...
function fnSearchBomVendor() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'membellishmentvendor/manageembellishmentvendor', GlbSearchParam, 'json', fnListRes);
}
    ...
    if (data != '') {
        if (data.errcode != undefined) {
            if (data.errcode == '404') {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'membellishmentvendor/addedit/' + encodeURIComponent(base64_encode(value.id)) + '">' + value.vn + '</a></td>' +
                                '<td>' + value.vcn + '</td>' +
                                '<td>' + value.addr + '</td>' +
                                '<td>' + value.e + '</td>' +
                                '<td>' + value.p + '</td>' +
                                ...
                                '<td>' + value.s + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'membellishmentvendor/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '"><i class="fa fa-edit"></i>&nbsp;Edit</a></td>';
                ...
                $('tbody').empty();
                $('#membellishmentvendorList').append(PageContent);
            }
        }
    }
...
function fnSaveembellishmentvendorInfo() {
    ...
    $.ajax({
        url: base_path + GlbCompanyFdr + 'membellishmentvendor/updateInfo',
        data: MyFormData,
        contentType: false,
        processData: false,
        dataType: "json",
...
function fnSaveBomRes(data) {
    ...
    if (data != '') {
        if (data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'membellishmentvendor/addedit/' + data.eid);
        }
    }
}
...
$('#membellishmentvendorList').on('click', 'th.sortable', function () {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'membellishmentvendor/manageembellishmentvendor', GlbSearchParam, 'json', fnChangeStatusRes);
});
    ...
    if (parseInt(StatusOptSelVal) > 0) {
        ...
        if (parseInt(ArrItemCheckBoxSel[1]) >= 1) {
            if (confirm('Do you want to ' + GlbStatusForMaster[StatusOptSelVal] + ' this records?')) {
                ...
                MakeAsynPostRequest(base_path + GlbCompanyFdr + 'membellishmentvendor/changemStatus', GlbSearchParam, 'json', fnSearchBomVendor);
            }
        }
    } else {
