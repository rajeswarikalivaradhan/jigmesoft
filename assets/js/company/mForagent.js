...
function fnSearch() {
    ...
    GlbSearchParam = "rfrom=1&fa=" + FAgent + "&s=" + Status + "&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mlogistics/forwardingAgent', GlbSearchParam, 'json', fnListRes);
}
function fnList() {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'mlogistics/addEditForagent/' + encodeURIComponent(base64_encode(value.id)) + '">' + value.fa + '</a>' +
                                '</td>' +
                                '<td>' + value.s + '</td><td>' + value.ub + '</td><td>' + value.du + '</td>' +
                                '<td>' +
                                '<a href="' + base_path + GlbCompanyFdr + 'mlogistics/addEditForagent/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">' +
...
function fnPaginationForwardingAgent(VarURL) {
...
function fnSaveInfo() {
    try {
        ...
        if (jsTrim(FAgent) == "") {
            $('#ErrfrmBasicFAgent').text("Please fill the Forwarding Agent");
            $('#frmBasicFAgent').focus();
            $('#frmBasicFAgent').css("border", "1px solid #B94A48");
            return false;
        }
        if (jsTrim(Status) == "") {
        ...
        $.ajax({
            url: base_path + GlbCompanyFdr + 'mlogistics/updateForAgent',
...
function fnSaveInfoRes(data) {
    ...
    if (data != '') {
        if (data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'mlogistics/addEditForagent/' + data.eid);
        }
    }
}
