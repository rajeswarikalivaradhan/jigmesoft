...
function fnSearch() {
    var frmBasicYarnType     					        = $("#frmSrchYarnPurchaseType").val();
    var Status        							= $("#frmSrchYarnStatus").val();
    GlbSearchParam = "rfrom=1&ypt=" + frmBasicYarnType + "&s=" + Status + "&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder;
    $("#DivTotalCntResult").html('');
    MakePostRequest(base_path+GlbCompanyFdr+'myarn/manageyarnpurchasetype',GlbSearchParam,'json',fnListRes);
}
    ...
    if(data!=''){
        if(data.errcode!=undefined) {
            if(data.errcode == '404') {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><input type="checkbox" class="allcbox" id="'+value.id+'"></td>' +
                                '<td><a href="'+base_path+GlbCompanyFdr+'myarn/addedityarnpurchasetype/'+encodeURIComponent(base64_encode(value.id))+'/">'+value.ypt+'</a></td>' +
                ...
                $('tbody').empty();
                $('#mYarnPurchaseTypeList').append(PageContent);
            }
        }
    }
}

function fnPaginationYarn(VarURL) {
...
function fnSaveYarnPurchaseTypeInfo() {
    try{
        $('.form-control').css("border", "1px solid #cccccc");
        $('div.herr').text('');
        var ProfileFormData							= false;
        var frmBasicYarnPurchaseType     			= $("#frmBasicYarnPurchaseType").val();
        var Status        							= $("#frmBasicStatus").val();
        if(jsTrim(frmBasicYarnPurchaseType) == ""){
            $('#ErrfrmBasicYarnPurchaseType').text("Please fill the yarn purchase type");
            $('#frmBasicYarnPurchaseType').focus();
            $('#frmBasicYarnPurchaseType').css("border", "1px solid #B94A48");
            return false;
        }
        if(jsTrim(Status) == ""){
        ...
        if (window.FormData){
            ...
            ProfileFormData.append("ypt",frmBasicYarnPurchaseType);
            ProfileFormData.append("s",Status);
            ProfileFormData.append("id",GlbId);
        }
        //console.log(ProfileFormData);
        $.ajax({
            url 		: base_path+GlbCompanyFdr+'myarn/updateYarnPurchaseTypeInfo',
...
function fnSaveYarnRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            $("#divSuccessBasicInfoMsg").text("Yarn Purchase Type has updated at successfully!");
            fnRedirectPageTimeOut(base_path+GlbCompanyFdr+'myarn/addedityarnpurchasetype/'+data.eid);
        }
    }
}
...
function fnChangeStatusRes(data) {
    ...
    $('#mYarnPurchaseTypeList').on('click', 'th.sortable', function () {
        ...
        var frmBasicYarnType     					        = $("#frmSrchYarnPurchaseType").val();
        var Status        							= $("#frmSrchYarnStatus").val();
        var param = "rfrom=1&ypt=" + frmBasicYarnType + "&s=" + Status + "&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder;
        MakePostRequest(base_path+GlbCompanyFdr+'myarn/manageyarnpurchasetype',param,'json',fnListRes);
            ...
            if (checkBoxLength >= 1) {
                ...
                if (dropdownOpt == '1') { //Activate
                    if(confirm('Do you want to activate this yarn?')) {
                        var param = "rfrom=1&ypt=" + frmBasicYarnType + "&s=" + Status + "&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder + "&actdeactFabType=" + dropdownOpt + "&cid=" + companyid_json;
                        MakePostRequest(base_path+GlbCompanyFdr+'myarn/changemStatus',param,'json',fnChangeStatusRes);
                    }
                }
