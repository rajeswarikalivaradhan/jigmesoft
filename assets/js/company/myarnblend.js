...
function fnSave() {
    ...
    if (jsTrim(Blend) == "") {
        $('#ErrfrmBasicBlend').text("Enter Yarn Blend (%)");
        $('#frmBasicBlend').focus();
    ...
    swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    MakePostRequest(base_path + GlbCompanyFdr + "myarnblend/updateInfo", 
                            ...
                            if (data.errcode == '404') {
                               ...
                               // $('#AnyErrElse').text(data.msg);
                                ...
                                swalWithBootstrapButtons.fire({
                                        ...
                                        let redirectpath = base_path + GlbCompanyFdr + 'myarnblend/manage';
                                        window.location.href = redirectpath;
                                });
                                // $("#divSuccessBasicInfoMsg").removeClass('hide');
                                // $("#divSuccessBasicInfoMsg").text("Updated successfully");
                                // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'myarnblend/addedit/' + data.eid);
                            }
                        }
                    });
				}
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'myarnblend/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'myarnblend/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'myarnblend/manage', Param, 'json', fnListRes);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                     ...
                     MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarnblend/changemStatus', Param, 'json', fnChangeStatusRes);
				}
                });
