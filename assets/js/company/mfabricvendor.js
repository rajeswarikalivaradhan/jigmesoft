...
function fnSave() {
    ...
    if (jsTrim(Vendor_name) == "") {
        $('#Errvendor_name').text("Enter Fabric Vendor Name");
        $('#vendor_name').focus();
    ...
    if (jsTrim(VendorCategory) == "") {
        $('#Errvendor_categoryid').text("Select Vendor Category");
        $('#vendor_categoryid').focus();
        $('#vendor_categoryid').css("border", "1px solid #B94A48");
        return false;
    }
    if (jsTrim(primaryline) == "") {
        $('#Errprimary_pdtline').text("Enter Primary Product Line");
        ...
        return false;
            ...
            swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + "mfabvendor/updateInfo",
                    ...
                    "&swift=" + swiftcode + "&s=" + Status + "&id=" + GlbId, "json",function (data) {
                        ...
                        if (data.errcode == '404') {
                           ...
                           // $('#AnyErrElse').text(data.msg);
                            ...
                            // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'mfabvendor/addedit/' + data.eid);
                            ...
                            swalWithBootstrapButtons.fire({
                                            ...
                                            customClass: {'confirmButton': 'btn btn-info'}
                                                ...
                                                let redirectpath = base_path + GlbCompanyFdr + 'mfabvendor/manage';
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mfabvendor/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'mfabvendor/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mfabvendor/changeStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                });
            ...
            //     MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mfabvendor/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
            // }
        }
    }
    else {
