...
function fnSave() {
    ...
    var AgentCategory = $("#agent_categoryid").val();
    ...
    if (jsTrim(Agent) == "") {
        $('#Erragentname').text("Enter Agent Name");
        $('#agentname').focus();
        $('#agentname').css("border", "1px solid #B94A48");
        return false;
    }
    if (jsTrim(Address) == "") {
    ...
    if (jsTrim(AgentCategory) == "") {
        $('#Erragent_categoryid').text("Select Forwarding Agent Category");
        $('#agent_categoryid').focus();
        $('#agent_categoryid').css("border", "1px solid #B94A48");
        return false;
    }
    if (jsTrim(Brand_id) == "") {
        $('#Errbrand_id').text("Select Brand");
        $('#brand_id').focus();
        $('#brand_id').css("border", "1px solid #B94A48");
        return false;
    }
    if (jsTrim(Gstno) == "") {
        ...
        return false;
            ...
            swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + "Magent/updateFwdingAgentInfo",
                    "rfrom=1&agent=" + Agent + "&addr=" + Address + "&cty=" + City + "&st=" + State + "&cy=" + Country + "&pin=" + Pincode +
                    "&em=" + EmailId + "&phno=" + PhoneNo + "&mbno=" + MobileNo + "&cp=" + Contactperson + "&ac=" + AgentCategory + "&brnid=" + Brand_id + "&gst=" + Gstno + 
                    ...
                    "&s=" + Status + "&id=" + GlbId, "json",function (data) {
                        ...
                        if (data.errcode == '404') {
                            ...
                            // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'Magent/addedit/' + data.eid);
                            ...
                            swalWithBootstrapButtons.fire({
                                            ...
                                            customClass: {'confirmButton': 'btn btn-info'}
                                                ...
                                                let redirectpath = base_path + GlbCompanyFdr + 'Magent/forwarding';
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'Magent/forwarding', GlbSearchParam, 'json', fnListAgentRes);
}
function fnListAgentRes(data) {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'Magent/fwdaddedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
var GlbSortOrder = '';
var GlbColumnId = '';
function fnSearchFwdAgent() {
    var frmSrchagentname = $("#frmSrchagentname").val();
    var frmSrchemailid = $("#frmSrchemailid").val();
    var frmSrchmobno = $("#frmSrchmobno").val();
    var frmSrchcontperson = $("#frmSrchcontperson").val();
    var frmSrchagentcategory = $("#frmSrchagent_category").val();
    var frmSrchbrand_id = $("#frmSrchbrand").val();
    var frmSrchstatus = $("#frmSrchstatus").val();
    
    GlbSearchParam = "rfrom=1&agent=" + frmSrchagentname + "&em=" + frmSrchemailid + "&mno=" + frmSrchmobno + "&cp=" + frmSrchcontperson + "&ac=" + frmSrchagentcategory + "&brand_id=" + frmSrchbrand_id + "&s=" + frmSrchstatus;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'Magent/forwarding', GlbSearchParam, 'json', fnListAgentRes);
}
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'Magent/changeStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                });
            ...
            //     MakeAsynPostRequest(base_path + GlbCompanyFdr + 'Magent/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
            // }
        }
    }
    else {
...
function fnPaginationBrand(VarURL) {
    ...
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(VarURL, Parameters, 'json', fnListAgentRes);
}
...
$("#brand_id").change(function () {
    ...
    var BrnId = $(this).val();
    MakeAsynPostRequest(base_path + GlbCompanyFdr +"Magent/getBuyerInfoByBrandId", "rFrom=1&id=" + BrnId, "json", function (data) {
