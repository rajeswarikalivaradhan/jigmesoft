...
function fnSearchGPD() {
    var GPD          				            = $("#frmSrchGPDName").val();
    var Status          						= $("#frmSrchGPDStatus").val();
    GlbSearchParam							    = "rfrom=1&gn="+GPD+"&s="+Status;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(base_path + GlbCompanyFdr + '/mgpd/managegpd', GlbSearchParam, 'json', fnListGPDRes);
}
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                ...
                                '<td>' +
                                '<a href="' + base_path + GlbCompanyFdr + 'mgpd/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a>' +
                                '</td>';
...
function fnPaginationGPD(VarURL) {
    var Parameters = GlbSearchParam;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(VarURL, Parameters, 'json', fnListGPDRes);
}

function fnSaveGPDInfo() {
    try{
        $('.form-control').css("border", "1px solid #cccccc");
        $('div.herr').html('');
        var ProfileFormData							= false;
        var GPD     						        = $("#frmBasicGPDName").val();
        var Status        							= $("#frmBasicStatus").val();
        if(jsTrim(GPD)== ""){
            $('#ErrBasicGPDName').text("EnterGarment Parts");
            $('#frmBasicGPDName').focus();
            $('#frmBasicGPDName').css("border", "1px solid #B94A48");
            return false;
        }
        if(jsTrim(Status)== ""){
        ...
        swalWithBootstrapButtons.fire(
        {
    	   ...
    	   if (result.value) {
                $.ajax({
                    url: base_path + GlbCompanyFdr + 'mgpd/updateGPDInfo',
        ...
        if(data.errcode == '404') {
           ...
           swalWithBootstrapButtons.fire({
            ...
            swalWithBootstrapButtons.fire({
                ...
                let redirectpath = base_path + GlbCompanyFdr + 'mgpd/managegpd';
                window.location.href = redirectpath;
            });
            // $("#divSuccessBasicInfoMsg").removeClass('hide');
            // $("#divSuccessBasicInfoMsg").text("Updated successfully!");
            // fnRedirectPageTimeOut(base_path+GlbCompanyFdr+'mgpd/addedit/'+data.eid);
        }
    }
}
$( document ).ready(function() {
    $('#tableList').on('click', 'th.sortable', function () {
        ...
        MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mgpd/managegpd', param, 'json', fnListGPDRes);
    });
});
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mgpd/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
            });
