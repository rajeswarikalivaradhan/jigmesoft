...
function fnSave() {
    ...
    var login_id = ($("#login_prefix").val()!='')? $("#login_prefix").val()+$("#login_suffix").val(): $("#login_prefix").val(); // loginid 
    var password = $("#password").val();
    var pin = $("#merchantpin").val();
    ...
    if (jsTrim(password) == "") {
     ...
     if (jsTrim(pin) == "") {
        ...
        $('#merchantpin').focus();
        $('#merchantpin').css("border", "1px solid #B94A48");
        return false;
    } 
    if (!/^\d{4}$/.test(pin)) {
      $('#Errpin').text("Please enter a 4-digit numeric pin");
      $('#merchantpin').focus();
      $('#merchantpin').css("border", "1px solid #B94A48");
    return false;
    }
            ...
            swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    ...
                    "rfrom=1&dept=" + dept + "&deptcount=" + deptcount + "&dsgn=" + designation + "&username=" + contactname + "&addr=" + address + "&subscriber_id=" + subscriber_id + "&proforma_id=" + proforma_id +
                    "&loginid=" + login_id +  "&pwd=" + password +"&pin=" + pin + "&em=" + emailId + "&mbno=" + mobileno + "&doj=" + doj +
                    "&curr_salpckg=" + curr_salpckg + "&bnk=" + bankname + "&actn=" + accountname + "&actno=" + accountno + 
                    "&ifsc=" + ifsccode + "&swift=" + swiftcode + "&s=" + status + "&badms=" + badminstatus + "&id=" + GlbId, "json",function (data) {
                        if (data != '') {
                        if (data.errcode == '404') {
                            fnCallSessionExpire();
                            return false;
...
$('#editEnable').on('click', function() {
    ...
    $("#login_suffix").prop("disabled", true);
...
$("#department_id").change(function () {
    ...
    var Proforma_id = $('#proforma_id').val();
    if(Dept_id!=''){
        MakeAsynPostRequest(base_path + GlbCompanyFdr +"muser/getsubuserDeptcount", "rFrom=1&dept_id=" + Dept_id + "&proforma_id=" + Proforma_id + "&subscriber_id=" + Subscriber_id, "json", function (data) {
        // console.log(data.usercnt);
        if(data.usercnt<data.totuser){
          $("#dept_usercount").val(parseInt(data.usercnt)+1);   
        }else{
            $("#dept_usercount").val('');   
           // $(this).val('');
            swalWithBootstrapButtons.fire({
                title: 'Total number of users allowed for this department created already!',
                type: 'warning',
                icon: 'warning',
                 width:460,
                customClass: {'confirmButton': 'btn btn-info'}
            ...
            // alert('Total number of users for this department created already');
