...
function fnSave() {
    ...
    $('div.herr').text('');
    var Fabric = $("#frmBasicFabric").val();
    var Status = $("#frmBasicStatus").val();

    if (jsTrim(Fabric) == "") {
        $('#ErrfrmBasicFabric').text("Enter Fabric Name");
        $('#frmBasicFabric').focus();
        $('#frmBasicFabric').css("border", "1px solid #B94A48");
        return false;
    }
    if (Status == "") {
    ...
    swalWithBootstrapButtons.fire(
    {
	   ...
	   if (result.value) {
            MakeAsynPostRequest(base_path + GlbCompanyFdr + "fabricname/updateInfo", "rfrom=1&f=" + Fabric + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
                if (data != '') {
                    if (data.errcode == '404') {
                        fnCallSessionExpire();
                        return false;
                        ...
                        swalWithBootstrapButtons.fire({
                            ...
                            let redirectpath = base_path + GlbCompanyFdr + 'fabricname/manage';
                            window.location.href = redirectpath;
                        });
                        // $("#divSuccessBasicInfoMsg").removeClass('hide');
                        // $("#divSuccessBasicInfoMsg").text("Updated successfully");
                        // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'fabricname/addedit/' + data.eid);
                    }
                }
            });
		}
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'fabricname/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'fabricname/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
function fnSearch() {
    ...
    GlbSearchParam = "rfrom=1&f=" + frmSrchFabric + "&s=" + Status;
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'fabricname/manage', GlbSearchParam, 'json', fnListRes);
}
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'fabricname/manage', Param, 'json', fnListRes);
    ...
    if (dropdownOpt > 0) {
        ...
        if (checkBoxLength >= 1) {
            ...
            var Status = $("#frmSrchBomStatus").val();
				...
				if (result.value) {
				    var Param = "actDeact=" + dropdownOpt + "&cid=" + companyid_json;
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'fabricname/changemStatus', Param, 'json', fnChangeStatusRes);
				}
                });
...
function fnPaginationFabricname(VarURL) {
