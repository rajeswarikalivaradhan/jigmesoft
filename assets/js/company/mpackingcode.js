                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent = PageContent + '<tr><td><input type="checkbox" id="' + value.id + '" class="allcbox"></td>' +
                                ...
                                '<td>' +
                                '<a href="' + base_path + GlbCompanyFdr + 'mpackingcode/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
...
function fnSavePackingCodeInfo() {
    try {
        ...
        if(jsTrim(PackingCode)== ""){
            $('#ErrBasicPackingCode').text("Enter Packing Code");
    ...
    if(data!=''){
        if(data.errcode == '404') {
            ...
            swalWithBootstrapButtons.fire({
                ...
                let redirectpath = base_path + GlbCompanyFdr + 'mpackingcode/managepackingcode';
