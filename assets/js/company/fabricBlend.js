...
function fnSave() {
    ...
    if (jsTrim(Blend) == "") {
        $('#ErrfrmBasicBlend').text("Enter Fabric Blend (%)");
        $('#frmBasicBlend').focus();
   ...
   swalWithBootstrapButtons.fire(
		...
		if (result.value) {
            MakePostRequest(base_path + GlbCompanyFdr + "fabricblend/updateInfo", "rfrom=1&b=" + Blend + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
            ...
            if (data != '') {
                if (data.errcode == '404') {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        let redirectpath = base_path + GlbCompanyFdr + 'fabricblend/manage';
                        window.location.href = redirectpath;
                    });
                    // $("#divSuccessBasicInfoMsg").removeClass('hide');
                    // $("#divSuccessBasicInfoMsg").text("updated successfully!");
                    // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'fabricblend/addedit/' + data.eid);
                }
            }
            });
		}
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'fabricblend/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'fabricblend/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'fabricblend/manage', Param, 'json', fnListRes);
    ...
    if (dropdownOpt > 0) {
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
				    GlbSearchParam = "rfrom=1&type=" + dropdownOpt + "&cid=" + companyid_json;
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'fabricblend/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                });
...
function fnPaginationBlend(VarURL) {
