...
function fnSave() {
    ...
    if (jsTrim(Blend) == "") {
        $('#ErrfrmBasicBlend').text("Enter BOM (Art-1) - Blend (%)");
        $('#frmBasicBlend').focus();
    ...
    //console.log(Blend, 'Blend');
            swalWithBootstrapButtons.fire(
				 ...
				 if (result.value) {
                    MakePostRequest(base_path + GlbCompanyFdr + "bom1/Bomblend/updateInfo", 
                      ...
                      //  console.log(data, 'data');
                        if (data != '') {
                            if (data.errcode == '404') {
                                ...
                                // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'bom1/Bomblend/addedit/' + data.eid);
                                ...
                                let redirectpath = base_path + GlbCompanyFdr + 'bom1/Bomblend/manage';
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom1/Bomblend/manage/', GlbSearchParam, 'json', fnListRes);
}
    ...
    if (data != '') {
        if (data.errcode != undefined) {
            if (data.errcode == '404') {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'bom1/Bomblend/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
                ...
                //commented by myself $('tbody').empty();
                $('#tableId tbody').empty();
               //commented by myself  $('#tableId').append(PageContent);
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom1/Bomblend/manage/', Param, 'json', fnListRes);
            ...
            var companyid_json = JSON.stringify(cboxObj);
                        swalWithBootstrapButtons.fire(
    						...
    						if (result.value) {
                                ...
                                MakeAsynPostRequest(base_path + GlbCompanyFdr + 'bom1/Bomblend/changemStatus', Param, 'json', fnChangeStatusRes);
    						}
                            });
