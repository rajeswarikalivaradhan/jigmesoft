...
function fnSave() {
    ...
    if (jsTrim(Blend) == "") {
        ...
        $('#ErrfrmBasicBlend').text("Enter BOM (Art-1) - Item Description");
        $('#frmBasicBlend').focus();
    ...
    if (result.value) {
    MakePostRequest(base_path + GlbCompanyFdr + "bom1/Bominstrumenttype/updateInfo", "rfrom=1&b=" + encodeURIComponent(Blend) + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
        ...
        if (data != '') {
            if (data.errcode == '404') {
                ...
                // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'bom1/Bominstrumenttype/addedit/' + data.eid);
                ...
                swalWithBootstrapButtons.fire({
                            ...
                            let redirectpath = base_path + GlbCompanyFdr + 'bom1/Bominstrumenttype/manage';
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom1/Bominstrumenttype/manage/', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'bom1/Bominstrumenttype/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom1/Bominstrumenttype/manage/', Param, 'json', fnListRes);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
    						...
    						if (result.value) {
                               ...
                               MakeAsynPostRequest(base_path + GlbCompanyFdr + 'bom1/Bominstrumenttype/changemStatus', Param, 'json', fnChangeStatusRes);
    						}
                            });
