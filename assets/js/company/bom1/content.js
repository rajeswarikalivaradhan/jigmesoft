...
function fnSave() {
    ...
    if (jsTrim(Blend) == "") {
        ...
        $('#ErrfrmBasicBlend').text("Enter BOM (Art-1) - Content");
        $('#frmBasicBlend').focus();
    ...
    //console.log(Blend, 'Blend');
            swalWithBootstrapButtons.fire(
        				...
        				if (result.value) {
                             MakePostRequest(base_path + GlbCompanyFdr + "bom1/Bomcontent/updateInfo",
                             ...
                             // console.log(data, 'data');
                                if (data != '') {
                                    if (data.errcode == '404') {
                                        ...
                                        // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'bom1/Bomcontent/addedit/' + data.eid);
                                        ...
                                        swalWithBootstrapButtons.fire({
                                            ...
                                            let redirectpath = base_path + GlbCompanyFdr + 'bom1/Bomcontent/manage';
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom1/Bomcontent/manage/', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'bom1/Bomcontent/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom1/Bomcontent/manage/', Param, 'json', fnListRes);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
    						...
    						if (result.value) {
                                 ...
                                 MakeAsynPostRequest(base_path + GlbCompanyFdr + 'bom1/Bomcontent/changemStatus', Param, 'json', fnChangeStatusRes);
    						}
                            });
