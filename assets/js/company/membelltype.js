...
function fnSearch() {
    var Data = $("#frmSrchEmbellName").val();
    var Status = $("#frmSrchStatus").val();
    GlbSearchParam = "rfrom=1&em=" + Data + "&s=" + Status + "&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'membellishment/manageType', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                ...
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'membellishment/addeditembellishmenttype/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
...
function fnPaginationEmbellType(VarURL) {
...
function fnSaveInfo() {
    try {
        ...
        let frmBasicEmbellType = $("#frmBasicEmbellType").val();
        let Status = $("#frmBasicStatus").val();
        if (jsTrim(frmBasicEmbellType) == "") {
            $('#ErrfrmBasicEmbellType').text("Enter Embellishment Type");
            $('#frmBasicEmbellType').focus();
            $('#frmBasicEmbellType').css("border", "1px solid #B94A48");
            return false;
        }
        if (jsTrim(Status) == "") {
        ...
        if (window.FormData) {
            ...
            ProfileFormData.append("en", frmBasicEmbellType);
        ...
        swalWithBootstrapButtons.fire(
            {
				...
				if (result.value) {
                    $.ajax({
                        url: base_path + GlbCompanyFdr + 'membellishment/updateEmbellishmentInfo',
    ...
    if (data != '') {
        if (data.errcode == '404') {
            ...
            swalWithBootstrapButtons.fire({
                ...
                let redirectpath = base_path + GlbCompanyFdr + 'membellishment/manageType';
                window.location.href = redirectpath;
            });
            
            // $("#divSuccessBasicInfoMsg").removeClass('hide');
            // $("#divSuccessBasicInfoMsg").text("Updated successfully!");
            // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'membellishment/addeditembellishmenttype/' + data.eid);
        }
    }
}
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'membellishment/changeEmbellishmentType', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                });
