...
function fnSave() {
    ...
    if (jsTrim(Content) == "") {
        $('#ErrfrmBasicContent').text("Enter Yarn Content");
    ...
    swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + "myarncontent/updateInfo",
                    "rfrom=1&c=" + encodeURIComponent(Content) + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
                        ...
                        if (data.errcode == '404') {
                            ...
                            swalWithBootstrapButtons.fire({
                                ...
                                customClass: {'confirmButton': 'btn btn-info'}
                                    ...
                                    let redirectpath = base_path + GlbCompanyFdr + 'myarncontent/manage';
                                    window.location.href = redirectpath;
                            });
                            // $("#divSuccessBasicInfoMsg").removeClass('hide');
                            // $("#divSuccessBasicInfoMsg").text("Updated successfully");
                            // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'myarncontent/addedit/' + data.eid);
                        }
                    }
                });
				}
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'myarncontent/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'myarncontent/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
var GlbColumnId = '';
function fnSearch() {
    var frmSrchContent = $("#frmSrchContent").val();
    var Status = $("#frmSrchStatus").val();
    GlbSearchParam = "rfrom=1&c=" + frmSrchContent + "&s=" + Status;
    MakePostRequest(base_path + GlbCompanyFdr + 'myarncontent/manage', GlbSearchParam, 'json', fnListRes);
}
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'myarncontent/manage', Param, 'json', fnListRes);
});
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                     ...
                     MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarncontent/changeStatus', Param, 'json', fnChangeStatusRes);
				}
                });
...
function fnPaginationYarnContent(VarURL) {
