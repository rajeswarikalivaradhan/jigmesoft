...
function fnSave() {
    ...
    $('div.herr').text('');
    var Jobwrkname = $("#jobwrkname").val();
    var Contactperson = $("#contactperson").val();
    var Address = $("#address").val();
    var EmailId = $("#emailid").val();
    var MobileNo = $("#mobile").val();
    var PhoneNo = $("#phone").val();
    var JobwrkCategory = $("#wrk_categoryid").val();
    var primaryline = $("#primaryjob_wrkline").val();
    var secondaryline = $("#secondaryjob_wrkline").val();
    var Gstno = $("#gstno").val();
    ...
    if (jsTrim(Jobwrkname) == "") {
        $('#Errjobwrkname').text("Enter Embellishment Job Work Name");
        $('#jobwrkname').focus();
        $('#jobwrkname').css("border", "1px solid #B94A48");
        return false;
    }
    if (jsTrim(Address) == "") {
    ...
    if (jsTrim(JobwrkCategory) == "") {
        $('#Errwrk_categoryid').text("Select Job Work Category");
        $('#wrk_categoryid').focus();
        $('#wrk_categoryid').css("border", "1px solid #B94A48");
        return false;
    }
    if (jsTrim(primaryline) == "") {
        $('#Errprimary_jobline').text("Enter Primary Job Work Line");
        $('#primaryjob_wrkline').focus();
        $('#primaryjob_wrkline').css("border", "1px solid #B94A48");
        return false;
    }
    if (jsTrim(Gstno) == "") {
        ...
        return false;
            ...
            swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + "mjobwrk/updateemblInfo",
                    "rfrom=1&jbwrkname=" + Jobwrkname + "&cp=" + Contactperson + "&addr=" + Address + 
                    "&em=" + EmailId + "&phno=" + PhoneNo + "&mbno=" + MobileNo + "&jbwrkc=" + JobwrkCategory +
                    "&pmln=" + primaryline + "&scln=" + secondaryline + "&gst=" + Gstno +  "&iec=" + IECode + 
                    ...
                    "&swift=" + swiftcode + "&s=" + Status + "&id=" + GlbId, "json",function (data) {
                        ...
                        if (data.errcode == '404') {
                           ...
                           // $('#AnyErrElse').text(data.msg);
                            ...
                            // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'mjobwrk/addedit/' + data.eid);
                            ...
                            swalWithBootstrapButtons.fire({
                                            ...
                                            customClass: {'confirmButton': 'btn btn-info'}
                                                ...
                                                let redirectpath = base_path + GlbCompanyFdr + 'mjobwrk/membelish';
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mjobwrk/membelish', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'mjobwrk/addeditembl/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
var GlbColumnId = '';
function fnSearch() {
    var frmSrchjobwrkname = $("#frmSrchjobwrkname").val();
    var frmSrchcontperson = $("#frmSrchcontperson").val();
    var frmSrchemailid = $("#frmSrchemailid").val();
    var frmSrchmobno = $("#frmSrchmobno").val();
    var frmSrchjbwrkcategory = $("#frmSrchjobwrk_category").val();
    var frmSrchprimaryline = $("#frmSrch_primaryjobwrkline").val();
    var frmSrchstatus = $("#frmSrchstatus").val();
    
    GlbSearchParam = "rfrom=1&jbwrkname=" + frmSrchjobwrkname + "&cp=" + frmSrchcontperson + "&em=" + frmSrchemailid + "&mno=" + frmSrchmobno +  "&jbwrkc=" + frmSrchjbwrkcategory + "&pmryln=" + frmSrchprimaryline + "&s=" + frmSrchstatus;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mjobwrk/membelish', GlbSearchParam, 'json', fnListRes);
}
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mjobwrk/changeStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                });
            ...
            //     MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mjobwrk/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
            // }
        }
    }
    else {
