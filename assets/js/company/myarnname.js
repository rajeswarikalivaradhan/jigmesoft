function fnSave() {
    ...
    if (jsTrim(Yarn) == "") {
        $('#ErrfrmBasicYarn').text("Please fill Yarn");
        $('#frmBasicYarn').focus();
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + "myarnname/updateInfo", "rfrom=1&y=" + Yarn + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
        if (data != '') {
            if (data.errcode == '404') {
                fnCallSessionExpire();
                return false;
                ...
                fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'myarnname/addedit/' + data.eid);
            }
        }
    });
}
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarnname/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td><a href="' + base_path + GlbCompanyFdr + 'myarnname/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/">' + value.y + '</a></td>' +
                                '<td>' + value.s + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'myarnname/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">' +
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarnname/manage', Param, 'json', fnListRes);
    ...
    if (dropdownOpt > 0) {
        ...
        if (checkBoxLength >= 1) {
            ...
            if (confirm('Do you want to ' + GlbStatusForMaster[dropdownOpt] + ' this record?')) {
                ...
                MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarnname/changemStatus', Param, 'json', fnChangeStatusRes);
            }
        }
    }
    else {
...
function fnPaginationYarnname(VarURL) {
