...
function fnSave() {
    ...
    if (jsTrim(Yarn) == "") {
        $('#ErrfrmBasicYarn').text("Enter Yarn Count");
        $('#frmBasicYarn').focus();
    ...
    swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + "myarncount/updateInfo",
                    "rfrom=1&y=" + Yarn + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
                        if (data != '') {
                            if (data.errcode == '404') {
                                fnCallSessionExpire();
                                return false;
                                ...
                                swalWithBootstrapButtons.fire({
                                    ...
                                    customClass: {'confirmButton': 'btn btn-info'}
                                            ...
                                            let redirectpath = base_path + GlbCompanyFdr + 'myarncount/manage';
                                            window.location.href = redirectpath;
                                });
                                // $("#divSuccessBasicInfoMsg").removeClass('hide');
                                // $("#divSuccessBasicInfoMsg").text("Updated successfully");
                                // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'myarncount/addedit/' + data.eid);
                            }
                        }
                    });
				}
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarncount/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'myarncount/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">' +
                                'View</a></td>';
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarncount/manage', Param, 'json', fnListRes);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                     ...
                     MakeAsynPostRequest(base_path + GlbCompanyFdr + 'myarncount/changemStatus', Param, 'json', fnChangeStatusRes);
				}
                });
