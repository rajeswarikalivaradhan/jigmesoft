...
function fnSave() {
    ...
    if (jsTrim(Content) == "") {
        $('#ErrfrmBasicContent').text("Enter Fabric Content");
    ...
    swalWithBootstrapButtons.fire(
		...
		if (result.value) {
            MakePostRequest(base_path + GlbCompanyFdr + "fabriccontent/updateInfo", "rfrom=1&c=" + Content + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
            ...
            if (data.errcode == '404') {
                ...
                swalWithBootstrapButtons.fire({
                        ...
                        let redirectpath = base_path + GlbCompanyFdr + 'fabriccontent/manage';
                        window.location.href = redirectpath;
                    });
                // $("#divSuccessBasicInfoMsg").removeClass('hide');
                // $("#divSuccessBasicInfoMsg").text("Updated successfully");
                // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'fabriccontent/addedit/' + data.eid);
            }
        }
            });
		}
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'fabriccontent/manage', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'fabriccontent/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'fabriccontent/manage', Param, 'json', fnListRes);
});
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'fabriccontent/changeStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                });
