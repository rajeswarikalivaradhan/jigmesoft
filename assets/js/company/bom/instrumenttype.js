...
function fnSave() {
    ...
    if (jsTrim(Blend) == "") {
        ...
        $('#ErrfrmBasicBlend').text("Enter BOM (Art-2) - Item Description");
        $('#frmBasicBlend').focus();
    ...
    if (result.value) {
    MakePostRequest(base_path + GlbCompanyFdr + "bom/Bominstrumenttype/updateInfo", "rfrom=1&b=" + encodeURIComponent(Blend) + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
        ...
        if (data != '') {
            if (data.errcode == '404') {
               ...
               // $('#AnyErrElse').text(data.msg);
                ...
                // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'bom/Bominstrumenttype/addedit/' + data.eid);
                ...
                swalWithBootstrapButtons.fire({
                            ...
                            let redirectpath = base_path + GlbCompanyFdr + 'bom/Bominstrumenttype/manage';
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom/Bominstrumenttype/manage/', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'bom/Bominstrumenttype/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom/Bominstrumenttype/manage/', Param, 'json', fnListRes);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
    						...
    						if (result.value) {
                               ...
                               MakeAsynPostRequest(base_path + GlbCompanyFdr + 'bom/Bominstrumenttype/changemStatus', Param, 'json', fnChangeStatusRes);
    						}
                            });
