...
function fnSave() {
    ...
    if (jsTrim(Blend) == "") {
        ...
        $('#ErrfrmBasicBlend').text("Enter BOM (Art-2) - Material");
        $('#frmBasicBlend').focus();
   ...
   // console.log(Blend, 'Blend');
                 swalWithBootstrapButtons.fire(
        				...
        				if (result.value) {
                            MakePostRequest(base_path + GlbCompanyFdr + "bom/Bommaterial/updateInfo", 
                            ...
                            if (data != '') {
                                if (data.errcode == '404') {
                                    ...
                                    // fnRedirectPageTimeOut(base_path + GlbCompanyFdr + 'bom/Bommaterial/addedit/' + data.eid);
                                        ...
                                        swalWithBootstrapButtons.fire({
                                                ...
                                                let redirectpath = base_path + GlbCompanyFdr + 'bom/Bommaterial/manage';
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom/Bommaterial/manage/', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="' + base_path + GlbCompanyFdr + 'bom/Bommaterial/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbCompanyFdr + 'bom/Bommaterial/manage/', Param, 'json', fnListRes);
            ...
            var companyid_json = JSON.stringify(cboxObj);
             swalWithBootstrapButtons.fire(
    						...
    						if (result.value) {
                                 ...
                                 MakeAsynPostRequest(base_path + GlbCompanyFdr + 'bom/Bommaterial/changemStatus', Param, 'json', fnChangeStatusRes);
    					    }
                            });
