function fnSearch() {
    var frmSrchUserType = $("#frmSrchUserType").val();
    var Status = $("#frmSrchMStatus").val();
    GlbSearchParam = "rfrom=1&ut=" + frmSrchUserType + "&s=" + Status;
    $("#DivTotalCntResult").html('');
    MakeAsynPostRequest(base_path + 'dashboard/manageDesignations', GlbSearchParam, 'json', fnListRes);
}
function fnList() {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                '<td>' +
                                '<a href="' + base_path +  'dashboard/addedituserdesgn/' + encodeURIComponent(base64_encode(value.id)) + '/">' + value.ut + '' +
                                '</a></td>' +
                                '<td>' + value.ds + '</td>' +
                                '<td>' + value.s + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="'+base_path+'dashboard/addedituserdesgn/'+encodeURIComponent(base64_encode(value.id))+'/edit'+'">' +
...
function fnPaginationUser(VarURL) {
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    GlbSearchParam = "rfrom=1&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder;
    MakeAsynPostRequest(base_path +  'dashboard/manageDesignations', GlbSearchParam, 'json', fnListRes);
});
    ...
    if (dropdownOpt > 0) {
        ...
        if (checkBoxLength >= 1) {
            ...
            if (confirm('Do you want to ' + GlbStatusForMaster[dropdownOpt] + ' this record?')) {
                ...
                MakeAsynPostRequest(base_path +  'dashboard/changeDesignationStatus', GlbSearchParam, 'json', fnChangeStatusRes);
            }
        }
    }
    else {
