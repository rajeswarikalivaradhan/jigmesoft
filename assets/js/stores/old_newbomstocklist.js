...
function fnList() {
    ...
    MakeAsynPostRequest(base_path+'storesuser/newbomstocklist/'+GlbBomreqInvId+'/'+GlbOrderId,GlbSearchParam,'json',fnListRes);
}
    ...
    if(data!='') {
        if(data.errcode!=undefined) {
            if(data.errcode == '404') {
                ...
                var PageContent=''; var pinourl = ''; var idesc=''; var icode = '';
                if(data.cn>0) {
                    ListCount	= '<div style="font-weight:bold;">Number of Record(s) : '+data.cn+'</div>';
                    if(data.ct>0) {
                        $.each(data.re,function(index,value) {
                            $.each(value.griddata,function (i,v) {
                                console.log(i,'i');
                                console.log(v,'v');
                                idesc=v.itemdesc;
                                icode=v.itemcode;
                            });
                            PageContent=PageContent+'<tr><td>'+value.sno+'</td>' +
                                '<td>'+value.wip+'</td>'+
                            '<td>'+value.bb+'</td>' +
                            '<td>'+value.pino+'</td>' +
                            '<td><a href="'+base_path+'storesuser/bomitemissuedreturn/'+value.pinoonly+'/'+encodeURIComponent(base64_encode(idesc))+'/'+encodeURIComponent(base64_encode(icode))+'">'+idesc+'</a></td>'+
                            '<td>'+icode+'</td>'+
                            '<td>'+value.indrefno+'</td>'+
                            '<td>'+value.cutoffdt+'</td>'+
                            '<td>'+value.authtype+'</td>';
                            /*'<td>'+value.cs+'</td>'+
                            '<td>'+value.cs+'</td>'+
                            '<td>'+value.ru+'</td>'+*/
                ...
                $('tbody').empty();
                $('#mBomNewStockList').append(PageContent);
            }
        }
    }
}
