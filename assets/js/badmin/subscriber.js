        ...
        return false;
            ...
            swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    MakeAsynPostRequest(base_path + GlbBAdminFdr + "msubscriber/updateInfo",
                            ...
                            if(data.mode == 'inserted'){
                                swalWithBootstrapButtons.fire({
                                            ...
                                            customClass: {'confirmButton': 'btn btn-info'}
                                                    ...
                                                    let redirectpath = base_path + GlbBAdminFdr + 'msubscriber/manage';
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbBAdminFdr + 'msubscriber/manage', GlbSearchParam, 'json', fnListRes);
}
                     ...
                     if ($('#btn-active').hasClass('active')) {
                        ...
                        $.each(data.re, function (index, value) {
                            ...
                            PageContent = PageContent + '<tr>' +
                                '<td><input type="checkbox" style="margin:0px!important" class="allcbox" id="' + value.id + '"></td>' +
                                '<td style="width:14%">' + value.cmpny + '</td>' +
                                '<td style="width:10%">' + value.cp + '</td>' +
                                '<td style="width:14%">' + value.em + '</td>' +
                                '<td style="width:7%">' + value.mbno + '</td>' +
                                '<td style="width:9%">' + value.cty + '</td>' +
                                '<td style="width:9%">' + value.st + '</td>' +
                                '<td style="width:9%">' + value.pckdet + '</td>' +
                                '<td style="width:10%;font-weight:bold;" class="knOrangeColor">' + value.reqstatus + '</td>' +
                                '<td style="width:11%">' + value.formateddate + '</td>' +
                                '<td style="width:3%">' + value.s + '</td>' +
                                '<td style="width:3%"><a href="' + base_path + GlbBAdminFdr + 'msubscriber/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                            }
...
//     var frmSrchCntPerson = $("#frmSrchCntPerson").val();
...
//     GlbSearchParam = "rfrom=1&cmpny=" + frmSrchCmpny + "&cp=" + frmSrchCntPerson + "&mbno=" + frmSrchMobno + "&cty=" + frmSrchCity + "&pckdetid=" + frmSrchPckgDet + "&reqstatus=" + frmSrchReqStatus;
//     $("#DivTotalCntResult").html('');
//     MakeAsynPostRequest(base_path + GlbBAdminFdr + 'msubscriber/manage', GlbSearchParam, 'json', fnListRes);
// }
...
function fnSearch() {
 ...
 var table1 = $('#tableId').DataTable();
    ...
    table1.column(2).search(frmSrchCntPerson, false, true);
    table1.column(4).search(frmSrchMobno, false, true);
    table1.column(5).search(frmSrchCity, false, true);
     table1.column(7).search(frmSrchPckgDet, false, true);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbBAdminFdr + 'msubscriber/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                if (result.value) {
                    location.reload();
				}
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
				    let idValue = $('#subscriber_id').val();
                                let reqstatus = 1;
                                let status = 1;
                    MakeAsynPostRequest(base_path + GlbBAdminFdr + "msubscriber/proformareq", "rFrom=1&id="+idValue+"&reqstatus="+reqstatus+"&status="+status, "json", function (data) {
                                    // console.log(data);
                                    if(data.statusCode == "200") {
                                        swalWithBootstrapButtons.fire({
                                            ...
                                            let enquiryListPath = GlbBAdminFdr +"msubscriber/manage";
      ...
      swalWithBootstrapButtons.fire(
                            {
                               ...
                               // text: "If you save You won't be able to revert this!",
								...
								if (result.value) {
								MakeAsynPostRequest(base_path + GlbBAdminFdr + "msubscriber/getcleardraftstatus", "id=" + val, "json", function (data) {
