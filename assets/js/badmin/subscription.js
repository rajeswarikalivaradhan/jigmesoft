...
function fnSave() {
    ...
    if (jsTrim(Iecode) == "") {
        $('#Erriecode_no').text("Enter IE Code");
        $('#iecode_no').focus();
            ...
            swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                    ...
                    "&remarks=" + Remarks + "&proforma_id=" + proforma_id + "&id=" + GlbId, "json",function (data) {
                        if (data != '') {
                        if (data.errcode == '404') {
                            fnCallSessionExpire();
                            return false;
...
$('#editEnable').on('click', function() {
    ...
    $("#package_id").prop("disabled", true);  // newly included to disable these fields in subscriber info
    $("#purchasetype").prop("disabled", true);
    $("#subscription_category").prop("disabled", true);
    $("#additional_users").prop("disabled", true);
    $("#data_storage_limit").prop("disabled", true);
    $("#file_storage_limit").prop("disabled", true); ////////////////
});
function fnList() {
    $("#DivTotalCntResult").html('');
    GlbSearchParam = "rfrom=1";
    MakeAsynPostRequest(base_path + GlbBAdminFdr + 'msubscription/manage', GlbSearchParam, 'json', fnListRes);
}
                ...
                if (data.cn > 0) {
                    ...
                    if (data.ct > 0) {
                        ...
                        $.each(data.re, function (index, value) {
                            // if(value.pckg_saved_status==1 && value.dept_saved_status==1){
                            //    refrow= "<td style='width:10%'>"+'<a href="' + base_path + GlbBAdminFdr  + 'msubscription/subaccnt/'+  encodeURIComponent(base64_encode(value.subscriber_id))+'">' + value.subsrptnrefno + "</a></td>"; 
                            // }else{
                            //     refrow= "<td style='width:10%'><a href='javascript:void(0)' onclick='alertmsg(\"Login account is inactive , please fill user count and user role details !\")'>" + value.subsrptnrefno + "</a></td>";
                            // }
                                refrow= "<td style='width:10%'>"+'<a href="' + base_path + GlbBAdminFdr  + 'msubscription/subaccnt/'+  encodeURIComponent(base64_encode(value.subscriber_id))+'">' + value.subsrptnrefno + "</a></td>"; 
                            ...
                            PageContent = PageContent + '<tr>' +
                                '<td><input style="margin:0px!important" type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                 refrow +
                                '<td style="width:25%">' + value.cmpny + '</td>' +
                                '<td style="width:5%">' + value.city + '</td>' +
                                '<td style="width:19%">' + value.email_id + '</td>' +
                                '<td style="width:9%">' + value.mobile_no + '</td>' +
                                '<td style="width:5%">' +value.subscription_category + '</td>' +
                                '<td style="width:5%">' + value.pckdet + '</td>' +
                                '<td style="width:5%;">' + value.startdate+ '</td>' +
                                '<td style="width:5%">' + value.enddate + '</td>' +
                                '<td style="width:10%">' + value.renewal_daysleft + '</td>' +
                                '<td style="width:3%">' + value.s + '</td>' +
                                '<td style="width:3%"><a href="' + base_path + GlbBAdminFdr  + 'msubscription/detview/' + encodeURIComponent(base64_encode(value.subscriber_id)) + '">View</a></td>';
                            PageContent = PageContent + '</tr>';
                            }
...
//     GlbSearchParam = "rfrom=1&cmpny=" + frmSrchCmpny + "&fromdate=" + frmSrchfromdate + "&city=" + frmSrchCity + "&emailid=" + frmSrchEmailid + "&mobno=" + frmSrchMobno + "&todate=" + frmSrchtodate;
//     $("#DivTotalCntResult").html('');
//     MakeAsynPostRequest(base_path + GlbBAdminFdr +  'msubscription/manage', GlbSearchParam, 'json', fnListRes);
// }
$.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
    var min = parseFloat($('#dayleftmin').val());
    var max = parseFloat($('#dayleftmax').val());

    if (isNaN(min)) min = -Infinity;
    if (isNaN(max)) max = Infinity;

    // Get the value from Unit Rate column (column 1)
    var unitRateText = data[10] || "";
...
$.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
    ...
    var requestDate = parseTableDate(data[9]); // column index 6
...
function fnSearch() {
 ...
 var table1 = $('#tableId').DataTable();
    ...
    table1.column(2).search(frmSrchCmpny, false, true); // false = regex, true = smart
    table1.column(3).search(frmSrchCity, false, true);
    table1.column(4).search(frmSrchEmailid, false, true);
    table1.column(5).search(frmSrchMobno, false, true);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
				...
				if (result.value) {
                    ...
                    MakeAsynPostRequest(base_path + GlbBAdminFdr +  'msubscription/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
				}
                if (result.value) {
                    location.reload();
				}
    ...
    swalWithBootstrapButtons.fire(
            {
               // title: 'Are you sure want to save the details ?',
               // text: "If you save You won't be able to revert this!",
                title: 'Do you want to generate a new subscription ?',
                type: 'warning',
                showCancelButton: true,
                scrollbarPadding: false,
                confirmButtonText: 'Yes',
				...
				if (result.value) {
                    ...
                    "rfrom=1&subscriber_id=" + subscriber_id + "&subscription_period=" + subscription_period +  "&subscriber_refno=" + subscriber_refno + "&id=" + GlbId, "json",function (data) {
                        if (data != '') {
                        if (data.errcode == '404') {
                            fnCallSessionExpire();
                            return false;
