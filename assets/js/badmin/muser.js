        ...
        return false;
            ...
            swalWithBootstrapButtons.fire(
            {
               ...
               // text: "If you save You won't be able to revert this!",
				...
				if (result.value) {
                     MakeAsynPostRequest(base_path + GlbBAdminFdr + "muser/updateusertInfo",
                    ...
                    "&ifsc=" + ifsccode + "&swift=" + swiftcode + "&s=" + status + "&id=" + GlbId, "json",function (data) {
                        ...
                        if (data.errcode == '404') {
                           ...
                           // $('#AnyErrElse').text(data.msg);
                            ...
                            // fnRedirectPageTimeOut(base_path + GlbBAdminFdr + 'muser/addedit/' + data.eid);
...
function fnList() {
    ...
    MakeAsynPostRequest(base_path + GlbBAdminFdr + 'muser/manage', GlbSearchParam, 'json', fnListRes);
}
                     ...
                     if ($('#btn-active').hasClass('active')) {
                        ...
                        $.each(data.re, function (index, value) {
                            ...
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td><a href="' + base_path + GlbBAdminFdr + 'muser/addedit_user/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">' + value.dept + '</a></td>' +
                                ...
                                '<td>' + value.du + '</td>' +
                                 '<td>' + value.s + '</td>' +
                                '<td><a href="' + base_path + GlbBAdminFdr + 'muser/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                            }
                        });
...
function fnSearch() {
 ...
 var table = $('#tableId').DataTable();
    ...
    table.column(1).search(frmSrchDept, false, true); // false = regex, true = smart
    table.column(3).search(frmSrchDesignation, false, true);
    table.column(4).search(frmSrchUserName, false, true);
    table.column(5).search(frmSrchLoginid, false, true);
    table.column(6).search(frmSrchEmailid, false, true);
    table.column(7).search(frmSrchMobNo, false, true);

    // Column 5: prefix search using regex
    if (Status) { // only if a value is selected
        table.column(10).search('^' + Status, true, false); // true = regex, false = smart
    } else {
        table.column(10).search(''); // reset
    }
...
$('#btnChangeStatus').on('click', function () {
    ...
    if (dropdownOpt > 0) {
        ...
        if (checkBoxLength >= 1) {
            ...
            //     MakeAsynPostRequest(base_path + GlbBAdminFdr + 'muser/changemStatus', GlbSearchParam, 'json', fnChangeStatusRes);
            // }
        }
    }
    else {
...
$("#department_id").change(function () {
    ...
    MakeAsynPostRequest(base_path + GlbBAdminFdr +"muser/getDeptcount", "rFrom=1&dept_id=" + Dept_id, "json", function (data) {
