...
function fnSave() {
    ...
    if (jsTrim(Descp) == "") {
        $('#ErrfrmDescp').text("Enter Details");
        $('#frmDescp').focus();
    ...
    if (result.value) {
    MakePostRequest(base_path + GlbBAdminFdr + "mdetail/updateInfo", "rfrom=1&d=" + encodeURIComponent(Descp) + "&s=" + Status + "&id=" + GlbId, "json", function (data) {
            ...
            if (data.errcode == '404') {
               ...
               // $('#AnyErrElse').text(data.msg);
                ...
                swalWithBootstrapButtons.fire({
                            ...
                            let redirectpath = base_path + GlbBAdminFdr + 'mdetail/manage';
...
function fnList() {
    ...
    MakePostRequest(base_path + GlbBAdminFdr + 'mdetail/manage/', GlbSearchParam, 'json', fnListRes);
}
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr>' +
                                ...
                                '<td>' + value.du + '</td>' +
                                '<td>' + value.s + '</td>' +
                                '<td><a href="' + base_path + GlbBAdminFdr + 'mdetail/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/edit' + '">View</a></td>';
                            ;
                            PageContent = PageContent + '</tr>';
                        });
                    }
...
function fnSearch() {
 ...
 var table = $('#tableId').DataTable();
    ...
    if (Status) { // only if a value is selected
        table.column(4).search('^' + Status, true, false); // true = regex, false = smart
    } else {
        table.column(4).search(''); // reset
    }
...
$('#tableId').on('click', 'th.sortable', function () {
    ...
    MakePostRequest(base_path + GlbBAdminFdr + 'mdetail/manage/', Param, 'json', fnListRes);
        ...
        if (checkBoxLength >= 1) {
            ...
            swalWithBootstrapButtons.fire(
    						...
    						if (result.value) {
                               ...
                               MakeAsynPostRequest(base_path + GlbBAdminFdr + 'mdetail/changemStatus', Param, 'json', fnChangeStatusRes);
    						}
                            if (result.value) {
                                location.reload();
