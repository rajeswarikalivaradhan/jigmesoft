function fnSaveEnquiry() {
    ...
    //try {
        ...
        var merchantremarks=$('#frmBasicMnote').val();
        var orderstatus=$("#order_status").val();
        //var chkupload=$('#uploadedfile').val();
         $("#uploadedfile").val('');
         if($('.ajax-file-upload-container .ajax-file-upload-filename').length>=1){
              $("#uploadedfile").val('2').trigger('change');
          }else{
              $("#uploadedfile").val('1').trigger('change');
          }
...
//        }
        ...
        if(frmBasicCurrency == 0 ||  frmBasicCurrency === null) {
            ...
            // return false;
            swalWithBootstrapButtons.fire({
                title: 'Select Currency in Enquiry Form',
               // text: 'Cancelled successfully.',
                type: 'warning',
                icon: 'warning',
                width:460,
        ...
        if(jsTrim(frmBasicBprice) == "" ||  frmBasicBprice === null || frmBasicBprice == "0.00") {
            // $('#ErrfrmBasicBprice').html("Enter Buyer's Price");
            // $('#frmBasicBprice').focus();
            // $('#frmBasicBprice').css("border", "1px solid #B94A48");
            // return false;
            swalWithBootstrapButtons.fire({
                title: "Enter Buyer's Price in Enquiry Form",
               // text: 'Cancelled successfully.',
                type: 'warning',
                icon: 'warning',
                width:460,
        ...
        if(jsTrim(frmBasicCprice) == "" ||  frmBasicCprice === null || frmBasicCprice=='0.00') {
            // $('#ErrfrmBasicCprice').html("Enter Confirm Price in Enquiry Form");
            // $('#frmBasicCprice').focus();
            // $('#frmBasicCprice').css("border", "1px solid #B94A48");
            // return false;
            swalWithBootstrapButtons.fire({
                title: "Enter Confirm Price in Enquiry Form",
               // text: 'Cancelled successfully.',
                type: 'warning',
                icon: 'warning',
                width:460,
       ...
       // var match = /\r|\n/.exec(chkupload);
        if(chkupload==1){
             $('#Errupload-list-view').html("Select File To Upload");
            $('.ajax-file-upload').focus();
            //$('.ajax-file-upload').css("border", "1px solid #B94A48");
            return false;
        }else{
             $('#Errupload-list-view').html("");
        }
        var Parameters = "rfrom=1&draftstatus=2&sd="+encodeURIComponent(StyDesc)+"&styref="+encodeURIComponent(Styref)+"&mt="+encodeURIComponent(merchantremarks)+"&enqdt="+
            ...
            && frmBasicPs && frmcombo && frmComponents && frmShipmentDate && PriceQuotedFor
            && frmBasicQprice && frmBasicCurrency && frmBasicBprice && frmBasicCprice && frmBasicRType && merchantremarks && chkupload) {
             swalWithBootstrapButtons.fire(
                            {
...
function fnSaveEnquiryRes(data) {
    if(data != '') {
        if(data.errcode == '404') {
            ...
            const fileCount = $('.ajax-file-upload-filename').length;
            if (fileCount > 0) {
                extraObj.startUpload(); // Upload files
            } 
...
$(document).ready(function() {
    var enquiSubid = document.getElementById('enqui_subid').value;
    extraObj     = $("#uploadBusinessImg").uploadFile({
        ...
        afterUploadAll: function () {
            ...
            Swal.fire({
                ...
                // let redirectUrl = "preCosting/index" + '/' + data.eid;
                // window.location.href = redirectUrl;
                location.reload();
            });
        }
    });
    console.log(extraObj.length,'extraObj INI');


$("#uploadBusinessImg").change(function () {
          $("#submitAuthRequest").prop("disabled", true);
          $("#fnSaveEnquiry").prop("disabled",false);
          console.log('test'+$('.ajax-file-upload-container .ajax-file-upload-filename').length);
          if($('.ajax-file-upload-container .ajax-file-upload-filename .ajax-file-upload-cancel').length>=1){
              $("#uploadedfile").val('2');
          }else{
              $("#uploadedfile").val('1');
          }
        ...
        $("#frmBasicBrand").change(function () {
            ...
            MakeAsynPostRequest(base_path + "merchant/getBuyerInfoByBrandId", "rFrom=1&id=" + BrnId, "json", function (data) {
                ...
                if(data.companyId != ''){
                   $("#frmBasicCountry").val(data.companyId); 
                }    
...
$(document).ready(function() {
    $("#submitAuthRequest").click(function () {
        var chkupload=$('#uploadedfile').val();
        if($("#frmBasicMnote").val()!='' && chkupload==2){
            $('#myModal').modal('show');
        }else if($("#frmBasicMnote").val()=='' && chkupload==1){
            alert('Please Save Merchant Remarks & File Upload');
        }else if($("#frmBasicMnote").val()=='' && chkupload==2){
            alert('Please Save Merchant Remarks');
            $('#ErrfrmBasicMnote').html("Enter Merchant Remarks");
            $('#frmBasicMnote').focus();
            $('#frmBasicMnote').css("border", "1px solid #B94A48");
            return false;
        }else if($("#frmBasicMnote").val()!='' && chkupload==1){
            alert('Choose File & Save');
            $('#Errupload-list-view').html("Select File To Upload");
            $('.ajax-file-upload').focus();
            return false;
        }
            ...
            function fnAuthRes(data) {
                if (data != '') {
                    if (data.errcode == '404') {
                        ...
                        swalWithBootstrapButtons.fire(
                            {
                                title: 'Are you sure you want to send the Authorization Request ?',
                                type: 'warning',
                                showCancelButton: true,
                                scrollbarPadding: false,
                                confirmButtonText: 'Yes',
    ...
    if(lastSegment != 'addenquiry' && segment == 'addenquiry')
    {
		...
		return $.ajax({
			...
			success:function(data){
				...
				$('#frmManagementRemarks').val(enquiryJSON[0].merchantnote).trigger("change");
				
				let createauth_iso = (enquiryJSON[0].dateauthorized!==null)? new Date(enquiryJSON[0].dateauthorized):'';
                ...
                var authampm =(hours!=='')? hours >= 12 ? "PM":"AM":'';
         ...
         let swalWithBootstrapButtons = Swal.mixin({buttonsStyling: false});
            swalWithBootstrapButtons.fire(
                ...
                if (result.value) {
                    ...
                    document.getElementById("temp_list_" + id).remove();
                    $("#submitAuthRequest").prop("disabled", true);
                    $("#fnSaveEnquiry").prop("disabled",false);
                } 
...
$('#frmBasicMnote').on('keyup', function() {
   $("#submitAuthRequest").prop("disabled", true);
   $("#fnSaveEnquiry").prop("disabled",false);
});
