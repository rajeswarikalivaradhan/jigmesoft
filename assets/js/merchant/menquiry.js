...
function fnSearchEn() {
    ...
    MakePostRequest(base_path +  'merchant/manageenquiry', GlbSearchParam, 'json', fnListRes);
}

function fnEnList() {
    GlbSearchParam = "rfrom=1";
    $("#DivTotalCntResult").html('');
    MakePostRequest(base_path + 'merchant/manageenquiry', GlbSearchParam+"&ut="+4, 'json', fnListRes);
}
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><input type="checkbox" id="'+value.id+'" class="allcbox"></td>'+
                                ...
                                '<td>'+value.bb+'</td>' +
                                '<td><a href="'+base_path+'merchant/entry/'+encodeURIComponent(base64_encode(value.id))+'">'+value.isrior+'</a></td>' +
                                '<td>'+value.enqt+'</td>' +
                                '<td>'+value.styref+'</td>' +
                                '<td>'+value.cfmpr+'</td>' +
                                '<td>'+value.curr+'</td>' +
...
$("span[class^='alpha_']").on('click', function () {
    ...
    if ($(this).hasClass('alpha_all_filter')) { //All accounts
        ...
        MakePostRequest(base_path + 'merchant/manageenquiry', param, 'json', fnListRes);
    }
});
        ...
        if (checkBoxLength >= 1) {
            ...
            if (dropdownOpt == '1') { //Activate
                if (confirm('Do you want to Activate this enquiry?')) {
                    ...
                    MakePostRequest(base_path + 'merchant/changemStatus', param, 'json', fnChangeStatusRes);
                }
            }
