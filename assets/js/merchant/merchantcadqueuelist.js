...
function fnCadQListSearch() {
    ...
    GlbSearchParam				= "rfrom=1&wip="+WipRefNo+"&cutfrom="+CutOffFrom+"&cutto="+CutOffTo+"&rf="+ReqDateFrom+"&rt="+ReqDateTo+"&req="+Requirement+
        "&cs=" + CStatus + "&columnId=" + GlbColumnId + "&sortorder=" + GlbSortOrder + "&at=" + approvaltype + "&mer=" + merchant;
    $("#DivTotalCntResult").html('');
    MakePostRequest(base_path+'merchant/cadqueuelist',GlbSearchParam,'json',fnListCadQListRes);
}

function fnListCadQList() {
    $("#DivTotalCntResult").html('');
    GlbSearchParam = 'rfrom=1';
    MakeAsynPostRequest(base_path+'merchant/cadqueuelist',GlbSearchParam,'json',fnListCadQListRes);
}
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value) {
                            PageContent=PageContent+'<tr><td><input type="checkbox" class="allcbox" id="'+value.id+'"></td>' +
                                ...
                                '<td>'+value.wip+'</td>' +
                                '<td>'+value.bb+'</td>' +
                                '<td><a href="'+base_path+'merchant/cadqueuelistdetail/'+encodeURIComponent(base64_encode(value.id))+'">'+value.cq+'</a></td>' +
                                '<td>'+value.allreq+'</td>' +
                                '<td>'+value.r+'</td>' +
                                '<td>'+value.reqdt+'</td>' +
                                '<td>'+value.cutoff+'</td>' +
...
function fnPaginationCadQueueList(VarURL) {
    var Parameters = GlbSearchParam;
    $("#DivTotalCntResult").html('');
    MakePostRequest(VarURL,Parameters,'json',fnListCadQListRes);
}