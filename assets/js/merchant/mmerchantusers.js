function fnSearch() {
    ...
    MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mmerchantuser/manage', GlbSearchParam, 'json', fnListRes);
}
function fnList() {
                    ...
                    if (data.ct > 0) {
                        $.each(data.re, function (index, value) {
                            PageContent = PageContent + '<tr><td><input type="checkbox" class="allcbox" id="' + value.id + '"></td>' +
                                '<td>' +
                                '<a href="' + base_path + GlbCompanyFdr + 'mmerchantuser/addedit/' + encodeURIComponent(base64_encode(value.id)) + '/">' + value.n + '' +
                                '</a>' +
                                '</td>' +
                                '<td>' + value.ds + '</td>' +
                                '<td>' + value.e + '</td>' +
                                ...
                                '<td>' + value.ub + '</td>' +
                                '<td>' + value.du + '</td>' +
                                '<td><a href="'+base_path+GlbCompanyFdr+'mmerchantuser/addedit/'+encodeURIComponent(base64_encode(value.id))+'/edit'+'">' +
...
function fnPaginationMerUser(VarURL) {
    ...
    if (dropdownOpt > 0) {
        ...
        if (checkBoxLength >= 1) {
            ...
            if (confirm('Do you want to ' + GlbStatusForMaster[dropdownOpt] + ' this record?')) {
                ...
                MakeAsynPostRequest(base_path + GlbCompanyFdr + 'mmerchantuser/changeStatus', GlbSearchParam, 'json', fnChangeStatusRes);
            }
        }
    }
    else {
