$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'request/myarnstore/getSurplusStockList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/myarnstote/surplusstockdetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '"> Item - View Details </a>';
					}
				},		
				{ 
					"mDataProp": function ( data, type, full, meta) {
