$(document).ready(function () {
    ...
    function append_amount_paid_request(data) {
        ...
        let list = {
            ...
            columns: [
                { title:'id', width:'0%',align:'center',type:'hidden'},
                { title: 'Paid in Favour of', width: '7%', align: 'left', readOnly: true, type: 'dropdown', source: data.vendor_data },
                { title: 'Bank Name', width: '12%', align: 'left', readOnly: true },
                { title: 'Account Number', width: '10%', align: 'center', readOnly: true },
                { title: 'Mode of\n Payment', width: '6%', align: 'left', type: 'dropdown' , source: ['ON-LINE', 'CHEQUE'], readOnly: true},
                { title: 'Transaction ID / Code', width: '6%', align: 'left' , readOnly: true},
                { title: 'Transaction\n Date', width: '6%', align: 'center', type: 'calendar' , readOnly: true},
                { title: 'Cheque No.', width: '8%', align: 'left' , readOnly: true},
                { title: 'Cheque Date', width: '6%', align: 'center', type: 'calendar' , readOnly: true},
                { title: "Paid Towards", width: '6%', align: 'left', type: 'dropdown' , source: ['ADVANCE', 'BILL/INVOICE'], readOnly: true},
                ...
                { title: "Paid in\n Full / Part", width: '6%', align: 'center', type: 'dropdown' , source: ['PART', 'FULL'] , readOnly: true},
            ],
            minDimensions: [4, 1],
            allowDeleteColumn: false,
            allowInsertRow: false,
    ...
    $('#getValues').click(function () {
        swalWithBootstrapButtons.fire(
            ...
            if (result.value) {
                let data = sourceDetailsReference_vm.getData();
                updateFunction(data);
            } 
    ...
    function updateFunction(data) {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Fabricrequest/updateYarnPaymentPaidDetails',
            data: dataform,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                setTimeout(() => {
                    window.location.href = base_path + 'request/Fabricrequest/yarnpaymentpaidlist';
                }, 1000);
            },
            error: function () {
                console.log("Error");
