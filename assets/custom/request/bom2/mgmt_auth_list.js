...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'management/getBOM2List',
			type:'POST',
	...
	function dispDetails(enquiryJSON)
	{
		...
		$("#MerAllReqSentAllList").dataTable({
			...
			"aoColumns": [			
                ...
                {
					"mDataProp": function ( data, type, full, meta) {
                        return '<a class="bold" href="' + base_path +'management/bomr2equestlist/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id))+ '">'+data.isriorcode+'</a>';
					}
				},
                { "mDataProp": "brandname" },
                {
