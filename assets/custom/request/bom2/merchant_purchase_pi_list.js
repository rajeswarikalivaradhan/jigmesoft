...
$(document).ready(function() {
	...
	function getReqRequestList()
	{
		...
		return $.ajax({
			url: base_path+'request/bom2request/getMerchantPIList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
                        if(data.request_for == "P.I.APPROVAL" || data.request_for == "P.I. APPROVAL")
						{
							if(data.type == 3)
							{
								return 'BOM (A1)';
...
let swalWithBootstrapButtons = Swal.mixin({buttonsStyling: false});
 $('#btnChangeStatus').on('click', function () {
        ...
        if (dropdownOpt > 0) {
            if (checkBoxLength >= 1) {
                ...
                swalWithBootstrapButtons.fire(
					...
					// if (result.value) {
					// 	 MakeAsynPostRequest(base_path + 'dashboard/fnChangeAllListStatus', 'request_id=' + idJson + '&cs=' + dropdownOpt,'tbl_purchase_indent','flag', 'json', function (data) {
                    //     $.when(getReqRequestList()).done(function(){
                    //         dispDetails(reqRequestJSON);		
                    //     });
                    // });
  ...
  if(cutfromDate!=='' && cuttoDate!==''){
	...
	var vedorname = $('#vedorname').val().toLowerCase();
    var piref_no   = $('#piref_no').val().toLowerCase();
   

    table
      .column(1).search(wip_ref_no)
      .column(2).search(brandId)
      .column(5).search(vedorname)
      .column(6).search(piref_no)
