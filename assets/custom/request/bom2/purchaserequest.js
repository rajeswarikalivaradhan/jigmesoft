$(document).ready(function () {
    ...
    function get_draft_value() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bom2request/get_draft_value',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                if(draft_val.type == "BULK") {
                    ...
                    getPurchaseRequest();
                    if(draft_val.type == 'DISCREPANCY' ){
                         
                     var radio = $('input[name="purchase_req_type"][value="DISCREPANCY"]');
                     radio.prop('checked', true);
                    }
                     if(draft_val.type == 'SHORTAGE' ){
                        
                     var radio = $('input[name="purchase_req_type"][value="SHORTAGE"]');
                     radio.prop('checked', true);
                    }
                }
        ...
        let request = $.ajax({
            ...
            success: function (data) {
                ...
                sample_requirement_data = JSON.parse(data);
                  console.log(sample_requirement_data);
    ...
    function getPurchaseRequest_bulk() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bom2request/getPurchaseRequestDetails_bulk',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
    ...
    function getReferenceValue(data, status, row) {
        ...
        if(status == true) {
            ...
            table_data = table_data.filter(function(e) { if(e[1] !== data['bom_2_req_consld_id']) return e });
            //console.log(table_data);
            selectCount = selectCount-1;
            append_attach_reference(req_sel_data);
        }
    ...
    $('#saveasdraft').click(function () {
          ...
          const container = document.querySelector('.ajax-file-upload-statusbar');

if (container && container.querySelector('.ajax-file-upload-filename')) {
      hasFilename = 1
} else {
     hasFilename = 0;
        ...
        if(selectCount <= 0 && sourcing_result.length == 0) {
            ...
            if(hasFilename == 0 ){
              swalWithBootstrapButtons.fire({
            title: 'No file has been uploaded ', text: "Are you sure you want to proceed without uploading?", type: 'warning', showCancelButton: true, scrollbarPadding: false, confirmButtonText: 'Yes, proceed!', cancelButtonText: 'No, cancel!', reverseButtons: true, customClass: { 'confirmButton': 'btn btn-green mx-2 px-3', 'cancelButton': 'btn btn-red mx-2 px-3' }
    ...
    $('#getValues').click(function () {
           const container = document.querySelector('.ajax-file-upload-statusbar');
const containerview = document.querySelector('.upload-list-view');

        

if((container && container.querySelector('.ajax-file-upload-filename')) || (containerview && containerview.querySelector('.ajax-file-upload-filename'))){  
      hasFilename = 1
} else {
     hasFilename = 0;
                        ...
                        swalWithBootstrapButtons.fire(
                            ...
                            if (okay) {
                                if(types == "draft") {
                                    ...
                                    window.location.href = base_path + 'request/bom2request/index/' + encodeURIComponent(btoa(enquiry_id));
                                }
                                else {
                                    window.location.href = base_path + 'MerchantRequestSent/bom2';
                                }
                            }
                        });
                    }
    ...
    function clearFunction(type) {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bom2request/clearPurchaseRequest',
            data: dataform,
            processData: false,
            contentType: false,
            cache: false,
    ...
    function getBomRequestImages()
    {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/bom2request/getbomrequestImages',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
