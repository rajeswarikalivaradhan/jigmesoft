$(document).ready(function () {
    ...
    function refFilter(instance, cell, c, r, source)
    {
        ...
        if (size_id != "" && component_id != "" && combo_id != "" ) {
            return source.filter(function (item) {
    ...
    function getReferenceValue(data, status) {
        ...
        if(status === true) {
            ...
            {
            // console.log(requirementData[i]);
            if(data[0]==requirementData[i][0])
            {
                
                requirementData.splice(i,1);

            }
        }
        }
        
        append_attach_reference(requirementData);
    ...
    {
        // console.log(row);
        var duplicate_req_sel_data=requirementData;
        for(var i=0;i<requirementData.length;i++)
        {
            //console.log(req_sel_data[i]);
            if(row[0]==requirementData[i][0])
            {
        ...
        requirementData = duplicate_req_sel_data;
    ...
    function append_attach_reference(requirementData) {
    ...
    function remove_attach_reference(requirementData) {
        let tblData = requirementData;
        tblData = tblData.reduce((unique, o) => {
            if(!unique.some(obj => obj[0] === o[0])) {
              unique.push(o);
            }
            return unique;
        },[]);
