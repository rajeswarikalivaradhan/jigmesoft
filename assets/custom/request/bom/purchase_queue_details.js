$(document).ready(function () {
    ...
    function getQABomRequest() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bomrequest/getPurchaseBomQueueDetails',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
    ...
    function append_in_house_status(data) {
        ...
        let list = {
            ...
            columns: [
                { title:'id', width:'0%',align:'center',type:'hidden'},
                { type: 'text', title: 'Item Description', width: '8%', align: 'left', readOnly: true },
                { type: 'text', title: 'Garment\n Size', width: '7%', align: 'left', readOnly: true },
                { type: 'text', title: 'Approved\n Item Code', width: '8%', align: 'left', readOnly: true },
                { type: 'text', title: 'Approved Item\n Colour Code', width: '8%', align: 'left', readOnly: true },
                { title: 'Size / Dim.\n (L*W*H)', width: '7%', align: 'center', readOnly: true },
                ...
                { title: 'D.C. Date', type:'calendar', width: '8%', align: 'right', readOnly: true, options: { format:'DD/MM/YYYY' } },
                { title: 'D.C. Qty.', width: '8%', align: 'right', readOnly: true },
                { title: 'Invoice No.', width: '8%', align: 'right', readOnly: true },
                { title: 'Invoice Date', type:'calendar', width: '8%', align: 'right', readOnly: true, readOnly: true, options: { format:'DD/MM/YYYY' } },
                { title: 'Invoice Qty.', width: '8%', align: 'right', readOnly: true },
                { title: 'Received Date', type:'calendar', width: '6%', align: 'center', readOnly: true, options: { format:'DD/MM/YYYY' } },
        ...
        let list = {
            ...
            updateTable: function(instance, cell, col, row, val, label, cellName) {
                 ...
                 if(col == 13) 
                {
                    ...
                    mgmt_status = data.itemacceptstatus[row][13];
                   // console.log('mgmt_status', mgmt_status);
                     //mStatus1 = data.itemacceptstatus[row][9];
    ...
    $('#getValues').click(function () {
        ...
        if($('#dep_remarks').val() == "" || $('#dep_remarks').val() == null ) {
            $('#err_dep_remarks').html("Fill purchase department remarks");
    ...
    function updateFunction() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bomrequest/updateDeptNoteequest',
            data: dataform,
            processData: false,
            contentType: false,
            cache: false,
