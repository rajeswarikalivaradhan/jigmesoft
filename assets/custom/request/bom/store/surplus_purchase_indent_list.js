...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/Mstoreuser/getSurplusPurchaseIndentList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Bomrequest/surpluspurchaseindentdetailswip' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/' + encodeURIComponent(btoa(data.purchase_indent_id)) + '"> '+data.isriorcode+' </a>';
					}
				},	
				{ "mDataProp": "brandname" },	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Bomrequest/surpluspurchaseindentdetailspiref' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/' + encodeURIComponent(btoa(data.purchase_indent_id)) +'/' + encodeURIComponent(btoa(data.isriorcode)) + '">'+data.pi_ref_queue_no+'</a>';
					 // return '<a class="bold" href="' + base_path +'request/Bomrequest/surpluspurchaseindentdetailspiref' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/' + encodeURIComponent(btoa(data.purchase_indent_id)) + '"> '+data.isriorcode+' </a>';
					}
				},	
				{ 
					"mDataProp": function ( data, type, full, meta) {
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
                         if(data.appr_statuss == '0' || data.mgmt_approval == '')
            ...
            if (checkBoxLength >= 1) {
                ...
                swalWithBootstrapButtons.fire(
					...
					if (result.value) {
                        ...
                        '&tblname=tbl_purchase_indent'+'&idName=request_id ', 'json', function (data) {
                            $.when(getReqRequestList()).done(function(){
                                dispDetails(reqRequestJSON)
