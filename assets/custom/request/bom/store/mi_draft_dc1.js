...
const itemDescSource1 = group.map(row => ({
  ...
  name: row[3]     // col 3
            ...
            let count = 0;
            let list = {
                data: BOMMaterialIndent[i],  // Use BOMMaterialIndent from the response
                columns: [
                    ...
                    { type: 'dropdown', title: 'Item Description', width: '8%', align: 'left', source:itemDescSource1},
                    ...
                    { type: 'dropdown', title: 'Item Code', width: '8%', align: 'left' , readOnly: true },
                    ...
                    { type: 'dropdown', title: 'UOM', width: '8%', align: 'left', readOnly: true },
                    { type: 'dropdown', title: 'M.I. Wise \n Pending Qty.', width: '8%', align: 'right',  readOnly: true },
                    { type: 'dropdown', title: 'UOM', width: '8%', align: 'left', readOnly: true},
                    { type: 'dropdown', title: 'Item - Lot / \n Batch Ref. No.', width: '8%', align: 'left'},
  ...
  if(item_id >=1) {
                        ...
                        const resultRow = group.find(r => String(r[0]) === String(item_id));

                         if (resultRow) {
                         const bcm = resultRow[4];  
                     ...
                     if(col == 5) 
                    {
                        ...
                        if (resultRow) {
                     const itemcode = resultRow[6];  // get column 4
                       item_cod = itemcode;
                     ...
                     if(col == 6) 
                    {
                       ...
                       if (resultRow) {
                      const itemcolourcode = resultRow[7];  
   
                       $(cell).text(itemcolourcode);
                     ...
                     if(col == 8) 
                    {
                        if(item_id >=1) {
                            const resultRow = group.find(r => String(r[0]) === String(item_id));
                             ...
                             if (resultRow) {
                             const sizeuom = resultRow[9];  // get column 4
                    ...
                    if(col == 9) 
                    {
                        if(item_id >=1) {
                            const resultRow = group.find(r => String(r[0]) === String(item_id));
                               ...
                               if (resultRow) {
                         const mip = resultRow[10];  // get column 4
                    ...
                    if(col == 10) 
                    {
                        ...
                        if (resultRow) {
                       const miuom = resultRow[11];  // get column 4
                       $(cell).text(miuom);
                     ...
                     if(col == 11) 
                    {
                        //console.log(BomLotData);
                        if(item_id >=1 ){
                            let ind_uom = item_cod;
                            let obj = BomLotData.find(o => o.item === ind_uom);
                            $(cell).text(obj.name);
                     ...
                     if(col == 12) 
                    {
                        ...
                        if(item_id >=1 ){
                              //console.log(itemcodeee);
                            let ind_uom = item_cod;
                            let obj = BomRateData.find(o => o.item === ind_uom);
                            $(cell).text(obj.name);
                            //instance.jexcel.options.data[row][col] = obj.name;
                        }
                    }
...
//     }
 ...
 $('#draftDc').click(function () {
           ...
           // let issued_data = bom_mi_tbl_data.getData();
             let issued_data = bom_dynamic_mi_data.getData();
