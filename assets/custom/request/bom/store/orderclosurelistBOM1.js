...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/Mstoreuser/getOrderClosurelistBOM1',
			type:'POST',
            ...
            if (checkBoxLength >= 1) {
                ...
                swalWithBootstrapButtons.fire(
					...
					if (result.value) {
                        ...
                        '&tblname=tbl_bom_in_house'+'&idName=request_id ', 'json', function (data) {
                            $.when(getReqRequestList()).done(function(){
                                dispDetails(reqRequestJSON)
...
$.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
    ...
    var min = parseFloat($('#balence_qty_min').val());
    var max = parseFloat($('#balence_qty_max').val());
  ...
  $('#searchButton').on('click', function() {
	...
	var item_desc = $('#item_desc').val().toLowerCase();
    var garment_size   = $('#garment_size').val().toLowerCase();
    ...
    table
      ...
      .column(3).search(item_desc)
      .column(4).search(garment_size)
