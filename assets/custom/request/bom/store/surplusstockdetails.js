$(document).ready(function () {
    ...
    function rateFilter2(instance, cell, c, r, source) {
        var item_id = instance.jexcel.getValueFromCoords(c -1, r);
    ...
    function footer(grid_name)
    {
        ...
        {
            return [[ '', '', '',  'Total:', '=GPWSUMCOL(TABLE(), COLUMN(), "")',  '=GPWSUMCOL(TABLE(), COLUMN(), "")',  '=GPWSUMCOL(TABLE(), COLUMN(),"")', '=GPWSUMCOL(TABLE(), COLUMN(), "")', '', '=GPWSUMCOL(TABLE(), COLUMN(), "")',  '','=GPWSUMCOL(TABLE(), COLUMN(),"")' ,  ]];
        }
    }
    ...
    function append_material_issued_details(data) {
        ...
        let list = {
            ...
            columns: [
                ...
                { type: 'dropdown', title: 'Transfer\nCategory', width: '8%', align: 'left',source:stockDD ,},
                { type: 'text', title: 'S.T.M. Ref. No.', width: '12%', align: 'left', readOnly: true },
                // { title: 'Issued Qty.', width: '6%', align: 'right', readOnly: true },
                { type: 'text', title: 'Stock Transfer\nDate & Time', width: '12%', align: 'left', readOnly: true },
            ...
            onchange: function(instance, cell, col, row, val, label, cellName) {
                if(col == 4) {
                    console.log(enq);
                     var item_id = instance.jexcel.getValueFromCoords(col -1, row);
                     var filterData = enq
                     var filteredItem = filterData.find(item => item.id === item_id);
                     var itemName = filteredItem ? filteredItem.name : 'Not Found';
                     instance.jexcel.setValueFromCoords(14, row, itemName);
    ...
    function updateFunctionDraft(issuedData) {
        ...
        let request = $.ajax({
            ...
            error: function () {
             ...
             complete: function() {
        // Re-enable the save button after the request completes (success or failure)
    ...
    $('#stockclose').click(function () {
        swalWithBootstrapButtons.fire(
            ...
            if (result.value) {
                ...
                //let itemAccept = itemAcceptStatusReference_vm.getData();
                let inHouseConsolidate = inHouseConsolidatedReference_vm.getData();
                //console.log(inHouseConsolidate);
                updateFunctionclose( inHouseConsolidate);
            } 
            else if (result.dismiss === Swal.DismissReason.cancel) {
                // *** CANCELLED MESSAGE *** //
                swalWithBootstrapButtons.fire(
    ...
    function updateFunctionclose(inHouseConsolidate) {
        //console.log(inHouseConsolidate)
        let dataform = new FormData();
        dataform.append('inHouseConsolidate', JSON.stringify(inHouseConsolidate));
        dataform.append('enquiry_id',enquiry_id);
    ...
    //console.log(dataform.get('inHouseConsolidate'));

        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bomrequest/updateOrderClosestatus',
            data: dataform,
            processData: false,
            contentType: false,
            cache: false,
