$(document).ready(function () {
    ...
    function getQABomRequest() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bomrequest/supplyclosuredata',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
    ...
    function append_in_house_details(data) {
        ...
        let list = {
            ...
            columns: [
                ...
                { title: 'P.I. Ref. No.', width: '10%', align: 'center', type: 'text', source: data.piRefNo, filter: refNoFilter, readOnly: true },
                ...
                { title: 'Qty.', width: '6%', align: 'center', readOnly: true },
                { title: 'UOM', width: '6%', align: 'center', readOnly: true },
                { title: 'Rate Per Unit\n (Rs.)', width: '6%', align: 'center', readOnly: true },
                { title: 'Value\n (Rs.)', width: '6%', align: 'center', readOnly: true },
                { title: 'GST /\nIGST', width: '6%', align: 'center', readOnly: true },
                { title: 'Total Value\n (Rs.)', width: '6%', align: 'center', readOnly: true },
                ...
                { title: 'Order Stock Recd. \n Date & Time', width: '6%', align: 'center', type: 'calendar', options: { format: 'DD/MM/YYYY' }, readOnly: true },
                { title: 'Storage Bin / \n Rack Ref. No.', width: '7%', align: 'center', type: 'text', readOnly: true },
            ],
            minDimensions: [4, 1],
            allowDeleteColumn: false,
            allowInsertRow: true,
    ...
    function append_item_accept_status(data) {
        ...
        let list = {
            ...
            columns: [
                ...
                { title: 'M.I. Type \n Internal / External', width: '8%', align: 'center', type: 'dropdown',source:mi_type, readOnly: true },
                { title: 'Issue to Dept. / \n Vendor Name', width: '8%', align: 'center', readOnly: true },
                { title: 'M.I. Qty.', width: '8%', align: 'right', type: 'text', readOnly: true },
                { title: 'M.I. Wise \n Issued Qty.', width: '8%', align: 'right', type: 'text', readOnly: true },
                { title: 'M.I. Wise \n Pending Qty.', width: '8%', align: 'right', type: 'text', readOnly: true },
            ],
            minDimensions: [4, 1],
            allowDeleteColumn: false,
    ...
    function append_in_house_consolidated_qty(data) {
        ...
        let list = {
            ...
            columns: [
                ...
                { title:'Sample No.', width:'6%',align:'center',type:'text',readOnly: true},
            ...
            updateTable: function(instance, cell, col, row, val, label) {
                ...
                if(col == 14)
                {
                    if(mi_id != '') {
                        ...
                        dataform.append('mi_id', mi_id);
                        dataform.append('reqId', reqId);
                        dataform.append('itemCode',itemCodess);
    ...
    function append_shipment_details(data) {
        ...
        let list = {
            ...
            columns: [
                ...
                { type: 'dropdown', title: 'Order Closure \n Status', width: '6%', align: 'left', source: closureStatus, readOnly: true },
            ...
            updateTable: function(instance, cell, col, row, val, label) {
                ...
                if(col == 14)
                {
                    // if(data.shipmentorderclosuredetails[row][14] != '') {
                    //     $(cell).addClass('readonly');
                    // } else {
                    //     $(cell).removeClass('readonly');
                    // }
    ...
    function validateForm(issued_data, order_data ) {
        let errorCount = 0;
        for (let i = 0; i < issued_data.length; i++) {
            if(issued_data[i][3] != "") {
                const miId = issued_data[i][3];
                const qty = issued_data[i][9];
              
              for(let j = 0; j < order_data.length; j++) {
                  if(order_data[j][0] == miId ) {
                    //   console.log(miId);
                      const totalQty = order_data[j][12];
                      if(qty > totalQty) {
                        errorCount++;  
                      } 
                  }
    ...
    $('#getValues').click(function () {
            ...
            let validateReqCount = validateForm(issued_data, order_data);
    ...
    $('#orderClose').click(function () {
            ...
            if(validateReqCount > 0) {
                ...
                if (result.value) {
                    let itemDetails = itemDetails_vm.getData();
                    let shipment_order_details = shipmentOrderDetails_vm.getData();
                    updateCloseFunction( shipment_order_details, itemDetails);
                }
                else if (result.dismiss === Swal.DismissReason.cancel) {
                    // *** CANCELLED MESSAGE *** //
                    swalWithBootstrapButtons.fire(
    ...
    function updateCloseFunction(shipment_order_details, itemDetails) {
        let dataform = new FormData();
        dataform.append('enquiry_id', enquiry_id);
        dataform.append('reqId', reqId);
        dataform.append('pId', pId);
