			...
			"aoColumns": [	
				{ 
					...
					// }
					 // }
					 "mDataProp": function(data, type, full, meta) {
						return '<input type="checkbox" class="allcbox" id="'+data.item_code+'">';
					}
				},
				{ "mDataProp": "brandname" },	
				{ 
            ...
            if (checkBoxLength >= 1) {
                ...
                swalWithBootstrapButtons.fire(
					...
					if (result.value) {
                        ...
                        '&tblname=tbl_surplus_stock_details'+'&idName=item_code ', 'json', function (data) {
                            $.when(getReqRequestList()).done(function(){
                                dispDetails(reqRequestJSON)
...
$('#refreshBtn').on('click', function () {
    ...
    var cutoffDate  = parseTableDate(data[11]); // column index 7
...
//  }
  ...
  if(cutfromDate!=='' && cuttoDate!==''){
    ...
    var itemdescription = $('#itemdescription').val().toLowerCase();
	var GarmentSize = $('#GarmentSize').val().toLowerCase();
    var itemcode   = $('#itemcode').val().toLowerCase();
    var itemcolourcode = $('#itemcolourcode').val().toLowerCase();
    var pi_ref_no   = $('#pi_ref_no').val().toLowerCase();
   

    table
      
      .column(1).search(brandId)
      .column(2).search(itemdescription)
      .column(3).search(GarmentSize)
      .column(4).search(itemcode)
       .column(5).search(itemcolourcode)
      .column(10).search(pi_ref_no)
