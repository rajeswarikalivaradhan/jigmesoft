...
$(document).ready(function() {
    ...
    var stmInput = $('#stm_ref_no').val().trim().toLowerCase();
    if (!stmInput) return true; // no filter, show all rows

    // Get STM column (adjust index if needed)
    var cellValue = data[7] || "";
    
    // Remove HTML tags and extra whitespace
    cellValue = cellValue.replace(/<[^>]*>/g, ' ').trim();

    // Split by '/', ',', whitespace, or newlines
    var stmNumbers = cellValue.split(/\/|,|\s+|\n/).map(s => s.trim().toLowerCase()).filter(s => s);
	console.log(stmNumbers,'stmNumbers');

    // Check if any STM number includes the input
    return stmNumbers.some(stm => stm.includes(stmInput));
});
...
if(cutfromDate!==''){
  ...
  if(cutfromDate!=='' && cuttoDate!==''){
	...
	//var stm_ref_no = $('#stm_ref_no').val().toLowerCase();
