...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'company/Mstoreuser/getSupplyclosurelistBOM1',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Bomrequest/storepurchaseindentdetails1' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/' + encodeURIComponent(btoa(data.purchase_indent_id)) + '"> '+data.isriorcode+' </a>';
					}
				},	
				{ "mDataProp": "brandname" },	
