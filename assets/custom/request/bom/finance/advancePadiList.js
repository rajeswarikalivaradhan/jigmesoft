$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'request/Bomrequest/getAdvancePadiList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#paymentReceivedList").dataTable({
			...
			"aoColumns": [
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Bomrequest/advanceDetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '">' + data.orderenqrefno + '</a>';
					}
				},		
				{ "mDataProp": "brandname" },	
                { 
					"mDataProp": function ( data, type, full, meta) {
                        var d = new Date(data.pi_date); 
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
                        var d = new Date(data.cheque_date); 
                        var time = d.toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
                        var dFormat = ("0" + (d.getDate())).slice(-2) + '/' + ("0" + (d.getMonth() + 1)).slice(-2) + '/' + d.getFullYear();
						return dFormat;
					}
