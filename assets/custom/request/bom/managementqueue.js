$(document).ready(function () {
    ...
    function append_item_accept_status(data) {
        ...
        let list = {
            ...
            columns: [
                ...
                { title: 'Merchant Status \n Update Date & Time', width: '10%', align: 'center', type: 'text', options: { format: 'DD/MM/YYYY h:s A' }, readOnly: true },
                { title: 'Q.A. Status', width: '8%', align: 'center', type: 'dropdown', source: approvalStatusData, readOnly: true },
                { title: 'Q.A. Status Update\n Date & Time', width: '10%', align: 'center', type: 'text', options: { format: 'DD/MM/YYYY h:s A' }, readOnly: true },
                { title: 'Management\n Overriding Status', width: '8%', align: 'center', type: 'dropdown', source: overridingStatusData },
            ...
            updateTable: function(instance, cell, col, row, val, label, cellName) {
                if(col == 9) {
                    ...
                    if(mer_status == "1") {
                        ...
                        $(cell).css('background-color', '#fc0303ff');
                    } else if(mer_status == "0" || mer_status === "") {
                        $(cell).css('background-color', '#FFA519');
                    }
                ...
                if(col == 13) {
                     date = data.itemacceptstatus[row][14];
                    //console.log(status);
                    if(status !== 0 && date === '') {
                    ...
                    mgmt_status = data.itemacceptstatus[row][col];
                    if(mgmt_status == "1") {
                        $(cell).css('background-color', '#5DE684');
    ...
    function updateFunction(data) {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Bomrequest/updateManagementQueue',
            data: dataform,
            processData: false,
            contentType: false,
            cache: false,
