$(document).ready(function () {
    ...
    function footer(grid_name)
    {
        ...
        {
             return [[  '', '', '', '', '', '', '', '', '', '', '', '', 'Total:', '=GPWSUMCOL(TABLE(), 13)', ''  ]];
            //return [[ '', '', '', '', '', '', '', '', '', '', '', 'Total:', '=GPWSUMCOL(TABLE(), COLUMN(), "")', '',   ]];
        }
        else if(grid_name == 'bill_invoice')
        {
            return [[ '', '', '', '', '', '', '', '', '', 'Total:', '=GPWSUMCOL(TABLE(), 10)', '=GPWSUMCOL(TABLE(), 11)', '=GPWSUMCOL(TABLE(), 12)','=GPWSUMCOL(TABLE(), 13)', '=GPWSUMCOL(TABLE(), COLUMN(), "")', '',   ]];
        }
    ...
    function append_request_payment_log(data) {
        ...
        let list = {
            ...
            columns: [
                ...
                { title:'mode', width:'0%',align:'center',type:'hidden'},
                { title:'amt', width:'0%',align:'center',type:'hidden'},
            ],
            minDimensions: [4, 1],
            allowDeleteColumn: false,
            allowInsertRow: false,
            ...
            updateTable: function(instance, cell, col, row, val, label) {
                ...
                {
                    status = val;
                    if(data.paymentLog[row][14] == 'EDIT') {
