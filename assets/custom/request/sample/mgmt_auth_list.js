...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'management/getSampleList',
			type:'POST',
                ...
                { 
					"mDataProp": function ( data, type, full, meta) {
                        if(data.type == 1) {
                            ...
                            return '<a class="bold" href="' + base_path +'request/Samplerequest/managament' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id))+ '">SAMPLE</a>';
                        }
                        else if(data.type == 3) {
