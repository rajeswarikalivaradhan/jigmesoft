$(document).ready(function() {
    ...
    $('#btnChangeStatus1').on('click', function () {
 ...
 $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
    ...
    var requestDate = parseTableDate(data[3]); // column index 6
  ...
  if(cutfromDate!=='' && cuttoDate!==''){
	...
	var gdc_ref_no = $('#gdc_ref_no').val().toLowerCase();
    var samp_ref_no   = $('#samp_ref_no').val().toLowerCase();
   

    table
      .column(1).search(wip_ref_no)
      .column(2).search(brandId)
      .column(4).search(gdc_ref_no)
      .column(6).search(samp_ref_no)
