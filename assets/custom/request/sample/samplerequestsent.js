$(document).ready(function () {
    ...
    function getSampleRequest() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Samplerequest/getRequestSentdetails',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
    ...
    function append_sample_request(data) {
        ...
        for (let i = 0; i < qa_data.length; i++) {
            let combineValue = [ qa_data[i].sample_requirement_id, qa_data[i].po_enq_ref_id, qa_data[i].combo_id, qa_data[i].component_id,
                qa_data[i].spec_code_id, qa_data[i].sample_requirement, qa_data[i].purpose, qa_data[i].category, qa_data[i].if_revised,
                qa_data[i].req_size, qa_data[i].req_qty ];
                
            let refValue = [ qa_data[i].sample_requirement_id, qa_data[i].po_enq_ref_id, qa_data[i].combo_id, qa_data[i].component_id,
            qa_data[i].color_id, qa_data[i].spec_code_id, qa_data[i].grad_measure_chart , qa_data[i].artwork, qa_data[i].measure_details, 
            qa_data[i].buyer_sample, qa_data[i].buyer_comment ];

            table_data.push(combineValue);
            ref_tbl_data.push(refValue);
        }

        append_attach_reference(ref_tbl_data);
