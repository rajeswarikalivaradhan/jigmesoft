$(document).ready(function () {
    ...
    function getSampleRequest() {
        ...
        let request = $.ajax({
            ...
            success: function (data) {
                ...
                //console.log(sample_requirement_data.data[0].dep_remarks);
                if(sample_requirement_data.data[0].dep_remarks !== '') {
                    $('#sam_dept_note').text(sample_requirement_data.data[0].dep_remarks);
                }
            },
            error: function () {
                console.log("Error");
    ...
    function append_sample_request(data) {
        ...
        let filteredData = data.data.filter(row => filterIds.includes(row.sample_requirement_id.toString()));
        let qa_data = filteredData;
        ...
        for (let i = 0; i < qa_data.length; i++) {
            let combineValue = [ qa_data[i].sample_requirement_id, qa_data[i].po_enq_ref_id, qa_data[i].combo_id, qa_data[i].component_id,
                ...
                qa_data[i].req_size, qa_data[i].req_qty, qa_data[i].qa_status, qa_data[i].qa_req_id  ];
        ...
        let list = {
            ...
            columns: [
               ...
               { type: 'text', title: 'Delete', width: '5%', align: 'center', readOnly: true }, 
                ...
                { title:'QA Status', width:'0%',align:'center',type:'hidden'},
            ...
            onchange: function(instance, cell, col, row, val, label, cellName) {
             ...
             updateTable: function(instance, cell, col, row, val, label) {
                if(col==2) {
                    checkVal = val;
                    if(checkVal!="") {
                         getReferenceValue(list.data[row][0], true, data.data);
                    } 
    ...
    $('#getValues').click(function () {
        ...
        // else {
            ...
            req_data = req_data.filter(function(e) { if(e[1]  !="") return e });
    ...
    function updateFunction(finalData) {
        ...
        let request = $.ajax({
            ...
            success: function (data) {
                ...
                //     }
                //     else {
                //         alert("Please upload sample images");
                //         sampleUpload.startUpload();
                //     }
                // }
    ...
    let type = "sample_qa_request";

    let sampleUpload = $("#sampleReqImageUpload").uploadFile({
        dragDrop: true,
        multiple: true,
        url:base_path+'request/Samplerequest/uploadSAMPLEReqImages',
        returnType: "json",
        fileName: "myFile",
    ...
    $("#sampleReqImageUpload").change(function () {
        var BrnId = $(this).val();
        // console.log(BrnId);
    });
