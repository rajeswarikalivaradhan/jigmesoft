$(document).ready(function () {
    ...
    function getCadRequest() {
        ...
        let request = $.ajax({
            type: "POST",
            url: base_path + 'MerchantRequestSent/getcadqarequestDetails',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                // console.log(cad_requirement_data.cad_data[0].qa_approval)
    ...
    $('#getValues').click(function () {
        ...
        $('.herr').hide();
        if($('#qa_req_status').val() == "" || $('#qa_req_status').val() == null ) {
            $('#err_qa_req_status').html("Select Q.A. request status");
            $('#err_qa_req_status').show();
        }
        // else if($('#qa_dept_remarks').val() == "" || $('#qa_dept_remarks').val() == null ) {
        //     $('#err_qa_dept_remarks').html("Fill Q.A. dept. remarks");
    ...
    function updateFunction() {
        ...
        let tbl_data = cadRequest_vm.getData();
        dataform.append('tbl_data', JSON.stringify(tbl_data));
        dataform.append('enquiry_id', enquiry_id);
        dataform.append('request_id', reqId);
        dataform.append('qa_req_status', $('#qa_req_status').val());
        dataform.append('qa_dept_remarks', $('#qa_dept_remarks').val());

        let request = $.ajax({
            type: "POST",
            url: base_path + 'request/Cadrequest/updateCadQAReceivedList',
            data: dataform,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                swalWithBootstrapButtons.fire(
                    ...
                    if(okay) {
                       // window.location.href = base_path+"company/mqausers/cadqareceivedlist";
