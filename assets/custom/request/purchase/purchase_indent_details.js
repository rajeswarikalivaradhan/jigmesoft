        ...
        let list = {
            ...
            updateTable: function(instance, cell, col, row, val, label, cellName) {
                 ...
                 if(col == 8) 
                {
                     ...
                     if(inv_no != '') {
                          let inv_amts = data.pi_invno_total;
                          //console.log(inv_amts);
                    let obj = inv_amts.find(o => o.id === inv_no);
                         $(cell).text(obj.name);
                         instance.jexcel.options.data[row][col] = obj.name;
                         inv_amt = numeral(obj.name).format('0.00');
                     }
                    if(inv_no != '') {
                        //inv_amt = numeral(obj.name).format('0.00');
                        //$(cell).text(inv_amt);
                ...
                if(col == 9) 
                {
                      //console.log("jjjjjjjjj"+inv_amt);
                      //console.log("jjjjjjjjj"+val);
                       let numericVal = parseFloat(val); 
                    if(numericVal >= 0) {
                        if(numericVal <= inv_amt) {
...
// }
    ...
    function append_request_payment_log(data) {
        console.log("append_request_payment_log");
        console.log(data);
                console.log("append_request_payment_log");
        ...
        let list = {
            ...
            columns: [
                ...
                { type: 'dropdown', title: 'Approval Status', width: '6%', align: 'center', source: statusDD, readOnly:true },
                ...
                { type: 'text', title: "Payment Paid\n Status", width: '6%', align: 'center', source: paymentStatusDD, readOnly: true },
    ...
    $('#print_val').click(function () {
        ...
        dataform.append('pId', pId);
         ...
         //window.location.href = base_path + 'request/Bomrequest/invoicebill/'+encodeURIComponent(btoa(pId)) ;
            ...
            window.location.href = base_path + 'request/Bomrequest/invoicebill/'+encodeURIComponent(btoa(enquiry_id))+'/'+encodeURIComponent(btoa(reqId))+'/'+encodeURIComponent(btoa(pId)) ;
        
    });
    $('#pdf_val').click(function () {
        let dataform = new FormData();
        dataform.append('enquiry_id', enquiry_id);
        dataform.append('reqId', reqId);
        dataform.append('pId', pId);
        window.location.href = base_path + 'request/Bomrequest/invoicebill_pdf/'+encodeURIComponent(btoa(enquiry_id))+'/'+encodeURIComponent(btoa(reqId))+'/'+encodeURIComponent(btoa(pId)) ;
         //window.location.href = base_path + 'request/Bomrequest/invoicebill_pdf/'+encodeURIComponent(btoa(pId)) ;
