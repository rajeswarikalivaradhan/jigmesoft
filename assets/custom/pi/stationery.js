$(document).ready(function() {
    ...
    var reqRequestJSON;
	$.when(getStationeryPIList()).done(function(){
	...
	function getStationeryPIList()
	{
		return $.ajax({
			url: base_path+'merchant/getStationeryPIList',
			type:'POST',
	...
	function dispDetails(reqRequestJSON)
	{
		...
		$("#bomPurchaseReceivedListTbl").dataTable({
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						return '<a class="bold" href="' + base_path +'request/Bomrequest/merchantpurchaseindentdetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '"> P.I. Approval. </a>';
					}
				},	
