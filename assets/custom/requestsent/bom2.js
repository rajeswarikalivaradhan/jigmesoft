...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'MerchantRequestSent/getBOM2List',
			type:'POST',
	...
	function dispDetails(enquiryJSON)
	{
		...
		$("#MerAllReqSentAllList").dataTable({
			...
			"aoColumns": [			
                ...
                {
					"mDataProp": function ( data, type, full, meta) {
                        return '<a class="bold" href="' + base_path +'MerchantRequestSent/bom2requestlist' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id))+ '">'+data.isriorcode+'</a>';
					}
				},
                { "mDataProp": "brandname" },
                {
