...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'management/getWIPList',
			type:'POST',
	...
	function dispDetails(wipJSON)
	{
		...
		$("#workInProgressTbl").dataTable({
			...
			"aoColumns": [			
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						//return '<td><a href="' + base_path + 'preCosting/index/' + encodeURIComponent(btoa(data.id)) + '">' + data.isriorcode + '</a></td>';
					   return '<td><a href="' + base_path + 'Management/wipPrecosting/' + encodeURIComponent(btoa(data.id)) + '">' + data.isriorcode + '</a></td>';
                    }
				},
				{ 
					"mDataProp": function ( data, type, full, meta) {
    ...
    //     var url = base_path + "management/searchWIPList";
    //     $.ajax({
    //         url: url,
    //         method: "POST",
    //         data: data,
