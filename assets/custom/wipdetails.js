...
//         {
//             let empar = [];
//            let totalColumns = columnlength;
//            alert(totalColumns);
//            let dataColumnsToSum = 5;
//            let nonSumCols = totalColumns - dataColumnsToSum;

//      for (let i = 0; i < totalColumns; i++) {
//     if (i === nonSumCols - 1) {
//         empar.push('Total111111:'); // Insert the label before sum columns start
//     } else if (i < nonSumCols) {
//         empar.push('');
//     } else {
//         empar.push(`=SUMCOL(TABLE(), ${i})`);
//     }
//      }
...
//         }
    ...
    let fixedColsBeforeSize = 8; // Columns before size columns
    let lastHiddenColIndex = -1; // Usually 'component_intake_id'

    for (let i = 0; i < totalColumns; i++) {
        if (i === fixedColsBeforeSize) {
            empar.push('Total:'); // Add 'Total:' just before size columns
        } else if (i < fixedColsBeforeSize) {
            empar.push('');
        } else if (i === lastHiddenColIndex) {
            empar.push(''); // Skip hidden field (component_intake_id)
        } else {
            empar.push(`=SUMCOL(TABLE(), ${i})`); // Add total for size + Itemized Qty
        }
    }
    ...
    function appendComboColourDetailss(data) {
        ...
        var combo_color_wise_vm = new Vue({
            ...
            mounted: function () {
                ...
                //spreadsheet.setOverflowDrag(true);
    ...
    function updateSaveComboColour_old(data) {
            ...
            if (data.status == 'success') {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        // After the user clicks 'OK' (when result.isConfirmed is true)
                        let $active = $('.nav-tabs.order-entry li.active');
            ...
            success: function (data) {
                ...
                if(data.status == 'success')
                {
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function () {
                $('html, body').animate({
                    scrollTop: $('.nav-tabs.order-entry').offset().top - 150
        ...
        $('#oe_submit_component_intake').click(function () {
            ...
            if(validate == 0)
            {
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        // After the user clicks 'OK' (when result.isConfirmed is true)
                        let $active = $('.nav-tabs.order-entry li.active'); // Get active tab
                        let $next = $active.next('li'); // Find the next tab

                        if ($next.length) {
                            $next.find('a[data-toggle="tab"]').tab('show'); // Show the next tab
                        }
                         setTimeout(function () {
                $('html, body').animate({
                    scrollTop: $('.nav-tabs.order-entry').offset().top - 130 // adjust offset for sticky header
                }, 500);
            }, 300);
            ...
            success: function (data) {
                ...
                if(data.status == 'success')
                {
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function () {
                $('html, body').animate({
                    scrollTop: $('.nav-tabs.order-entry').offset().top - 120 // adjust offset for sticky header
                }, 500);
            }, 300);
                    }
                });
    ...
    var poEnqRefNoSource = data.poEnqRefNo;
    function comboFilter_old(instance, cell, c, r, source) {
         let firstPoEnqId = source[0].po_enq_id;
         var po_enq_id = instance.jexcel.getValueFromCoords(c - 1, r);
        if (po_enq_id !== "") {
            if(po_enq_id == "All")
            {
                 po_enq_id=firstPoEnqId
...
/**
 * Filters the source array based on the `po_enq_id` value obtained from the specified cell.
 * If the `po_enq_id` is "0", it uses the first `po_enq_id` from the source as the filter criterion.
 * Returns an array of items from the source where the `po_enq_id` matches the filter criterion.
 *
 * @param {Object} instance - The jexcel instance.
 * @param {HTMLElement} cell - The cell element.
 * @param {number} c - The column index of the cell.
 * @param {number} r - The row index of the cell.
 * @param {Array} source - The source array containing objects with `po_enq_id` properties.
 * @returns {Array} Filtered array of items with matching `po_enq_id`.
 */

    function comboFilter_bom1(instance, cell, c, r, source) {
        //poEnqRefNo
         let firstPoEnqId = source[0].po_enq_id;
    ...
    function specFilter(instance, cell, c, r, source) {
     ...
     function specFiltersample(instance, cell, c, r, source) {
        let colour_id = instance.jexcel.getValueFromCoords(c - 1, r);
        let component_id = instance.jexcel.getValueFromCoords(c - 2, r);
        let combo_id = instance.jexcel.getValueFromCoords(c - 3, r);
        let po_enq_id = instance.jexcel.getValueFromCoords(c - 4, r);
        ...
        if (colour_id != "" && component_id != "" && combo_id != "" && po_enq_id != "") {
            return source.filter(function (item) {
                if ((item.colour_id == colour_id) && (item.component_id == component_id) &&
                    (item.combo_id == combo_id) ) return true;
            })
        } else {
            return [];
        }
    }
    function orderspecFilter(instance, cell, c, r, source) {
         let colour_id = instance.jexcel.getValueFromCoords(c - 1, r);
        let component_id = instance.jexcel.getValueFromCoords(c - 2, r);
        let combo_id = instance.jexcel.getValueFromCoords(c - 3, r);
        let po_enq_id = instance.jexcel.getValueFromCoords(c - 4, r);
     ...
     if(po_enq_id == "All")
     {
        if (colour_id != "" && component_id != "" && combo_id != "" && po_enq_id != "") {
            ...
            // Only allow if this colour_id hasn't been added yet
            if (!seenColours.has(item.id)) {
                seenColours.add(item.id);
                return true;
            }
        }
        return false;
        ...
        if (colour_id != "" && component_id != "" && combo_id != "" && po_enq_id != "") {
            return source.filter(function (item) {
                if ((item.colour_id == colour_id) && (item.component_id == component_id) &&
                    (item.combo_id == combo_id) && (item.po_enq_id == po_enq_id) ) 
    ...
    function orderspecFilter_sample(instance, cell, c, r, source) {
          const seenColours = new Set();
        let colour_id = instance.jexcel.getValueFromCoords(c - 1, r);
        let component_id = instance.jexcel.getValueFromCoords(c - 2, r);
        let combo_id = instance.jexcel.getValueFromCoords(c - 3, r);
    ...
    function append_oe_complete_process(data) {
        ...
        let oe_complete_process = {
            ...
            columns: [
                ...
                { type:'dropdown',title:'Size Spec Code / Fit', width:'10%',source: data.specCode,filter: orderspecFilter,multiple: true, align:'left' },
            ...
            success: function (data) {
                ...
                if(data.status == 'success')
                {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.isConfirmed) {
    // Target the currently active tab in the nav-pills
            let $active = $('.nav-pills li.active');
             let $next = $active.next('li');

            if ($next.length) {
          $next.find('a[data-toggle="tab"]').tab('show'); // Activate next tab
           _call_to_cadDetails();

          }

    // Scroll to the <ul> nav-pills element
          setTimeout(function () {
          $('html, body').animate({
            scrollTop: $('.nav-pills').offset().top - 120 // adjust for sticky headers
          }, 500);
          }, 300);
       }
                    })
    ...
    function cadSpecFilter(instance, cell, c, r, source) {
     ...
     function cadSpecFilter_olddd(instance, cell, c, r, source) {
         let firstPoEnqId = source[0].po_enq_id;
         //alert(firstPoEnqId);
    ...
    function append_cad_requirement(data) {
        ...
        let cad_requirement = {
            ...
            columns: [
                ...
                { type: 'dropdown', title: 'Size Spec \n Code / Fit', width: '8%', align: 'left', source: data.specCode, filter: cadSpecFilter, multiple: true},
            ...
            onchange: function(instance, cell, col, row, val, label, cellName) {
                if(col == 4) 
                {
                   instance.jexcel.options.data[row][5]=''; 
                }
            },
        ...
        $('#oe_submitCADRequirement').click(function () {
            ...
            //let validatedErrorCount = validateForm(validate_filed,  statusCheck, optional_validation_field, pendingField);
             
            //alert(validatedErrorCount);
    ...
    function update_oe_cad_requirement(data) {
        ...
        if (result.isConfirmed) {
            // Activate the next tab in nav-pills
            let $active = $('.nav-pills li.active');
            let $next = $active.next('li');
            ...
            setTimeout(function () {
                $('html, body').animate({
                    scrollTop: $('.nav-pills').offset().top - 120
                }, 500);
            }, 300);
        }
    });
    ...
    function append_sample_requirement(data) {
        ...
        columns: [
           ...
           // { type: 'dropdown', title: 'Size Spec \n Code / Fit', width: '8%', align: 'left', source: data.specCode,  filter: orderspecFilter_sample, multiple: true,readOnly: true },
            { type:'dropdown',title:'Size Spec \n Code / Fit', width:'10%',source: data.specCode, filter: orderspecFilter_sample,  multiple: true, align:'left',},
            ...
            { type: 'dropdown', title: "Buyer's \nAppl. Days", width: '7%', align: 'left', source: ['Mon', 'Tue', 'Wed', 'Thur', 'Fri', 'Sat', 'Sun','All'], multiple:true},
            ...
            onchange: function(instance, cell, col, row, val, label, cellName) {
                if(col == 4) 
                {
                   instance.jexcel.options.data[row][6]='';
                }
                if(col == 6){
                //     alert("eeeee");
                //  instance.setValueFromCoords(6, row, []);
                   ...
                   //txt = $(cell).text();
                    // updatedRow = row;
                    //instance.jexcel.setValueFromCoords(col, row, value);
                }
            ...
            updateTable: function(instance, cell, col, row, val, label, cellName, des_city) {
                ...
                if(col == 6)
                {
                    ...
                    //     let obj = specCode.find(o =>o.combo_id === combo && o.component_id === component  && o.colour_id === colval );
                    //      $(cell).text(obj.name);
                    //      instance.jexcel.options.data[row][col] = obj.name;
                    // }
                    if(data.data[row][13] == 1 || data.data[row][13] == "1") {
...
let validatedErrorCount_new=0;
 
 let columnIndex = 11;
for (let i = 0; i < validate_data.length; i++) {
    let cellValue = validate_data[i][columnIndex];
    console.log(`Row ${i} - Col ${columnIndex}:`, cellValue);
    if(cellValue == "PENDING")
    {
       //return true;
    }else{
        
             let cell12 = validate_data[i][12];
              let cell13 = validate_data[i][13];
               let cell14 = validate_data[i][14];
              

               if(cell12 == '' || cell13 == '' || cell14 == '')
               {
               validatedErrorCount_new=1;
                   
               }else{
                validatedErrorCount_new=0;
               }
    }
}
            ...
            if(validatedErrorCount == 0 && validatedErrorCount_new == 0)
            {
                swalWithBootstrapButtons.fire(
                    // *** CONFIRMATION MESSAGE *** //
                    alertMessageFunction('confirmation_save')
                ...
                // *** VALIDATION ERROR MESSAGE *** //
                if(validatedErrorCount_new == 1)
                {
                     swalWithBootstrapButtons.fire({
                        title: 'Warning', text: ' select Approval Field.', type: 'error', icon: 'warning', customClass: { 'confirmButton': 'btn btn-secondary px-5' }
                    });
                }else{
           swalWithBootstrapButtons.fire(
                    alertMessageFunction('validation_error')
    ...
    function append_embellishment_vendor_details(data) {
        ...
        let embellishment_vendor_wise = {
            ...
            columns: [
                ...
                { title: 'Contact Person, e-mail\n ID & Mobile No.', width: '10%', align: 'left',  },
            ...
            updateTable: function(instance, cell, col, row, val, label, cellName, des_city) {
                ...
                if(col == 9) 
                {
                    ...
                    if(contact != '')
                    {
                     ...
                     let ddd = embellishment_vendor_wise.columns[8]['source'];
                    
                     const data = ddd.filter(item => item.id === contact);
                     
                     const contactpersonname = data[0].contactpersonname;
                     const emailid = data[0].emailid;
                     const mobile = data[0].mobile;
                     const jjj= [contactpersonname, emailid, mobile];
    ...
    function append_bom_sampling_approval_details(data) {
        ...
        let bom_sampling_details_wise = {
            ...
            columns: [
                ...
                { title: 'Appl. Recd.\n Date & Time', 'width': '8%', align: 'center', type: 'calendar' },
...
//                onchange: function(instance, cell, col, row, val, label, cellName) {
//             if(col == 15){
//       instance.setOptions({ tableOverflow: true });
...
onselection: function (inst, x1, y1) {
   if (!inst || !inst.options || !inst.options.columns) {
    ...
    const isCal = inst.options.columns[x1]?.type === 'calendar';
    const isTarget = x1 === TARGET_COL && isCal;
    document.body.classList.toggle('col15-cal-open', isTarget);
},

//   // Reposition calendar when editing starts
//   oneditionstart: function (inst, cell, x, y) {
//     // First row → force ABOVE; others → auto
//     setTimeout(() => placeCalendarAtCell(cell, y === 0 ? 'above' : undefined), 0);
//   },
...
for (let i = 0; i < validate_data.length; i++) {
    ...
    if(cellValue =="1")
    {
         ...
         validatedErrorCount_new = 0;
             ...
             let cell10 = validate_data[i][10];
              let cell11 = validate_data[i][11];
               let cell12 = validate_data[i][12];
               let cell13 = validate_data[i][13];
              let cell14 = validate_data[i][14];
               let cell15 = validate_data[i][15];
              

               if(cell10 == '' || cell11 == '' || cell12 == '' || cell13 == '' || cell14 == '' || cell15 == '')
               {
               validatedErrorCount_new=1;
            ...
            let validate_field = [2,3,4,5,6,7,8];
            let optional_validation_field = [];
            let pendingField = "";
            let statusCheck = "no";
            let validatedErrorCount = validateForm(validate_field, validate_data, statusCheck, optional_validation_field, pendingField);
    ...
    function sizeFilter(instance, cell, c, r, source) {
       //alert(JSON.stringify(source));
        ...
        if (material_name !== "") {
            ...
            //console.log(item_name);
            return source.filter(function (item) {
                 let targetItemName = String(item_name).trim(); // Sanitize target
                 let sourceItemName = String(item.item_name).trim(); // Sanitize source
                
                if (item.blend_name == blend_name && sourceItemName === targetItemName  && item.content_name == content_name
    ...
    function append_bom_requirement_details(data) {
        ...
        let dd = data.appendData.calculatedAmmount, updatedRow = "", inupdatedRow = "", filterValue = "", filterValue1 = "",itemQty = "";
        ...
        let bom1_requirement_details = {
            ...
            columns: [
                ...
                { title:'mode',width: 130,  type:'hidden', align:'center' },
                { title:'id',   width: 130, type:'hidden', align:'center' }, 
                { type: 'dropdown',title: 'P.O. No. /\nEnq. Ref. No1.',width: 130,align: 'left',source: data.appendData.poEnqRefNo,},
                { type: 'dropdown', title: 'Combo', width: 130, align: 'left',source: data.appendData.poCombo, filter: comboFilter_bom1,},
                { type: 'dropdown', title: 'Component',width: 130,align: 'left',source: data.appendData.poComponent, filter: componentFilter,},
                { type: 'dropdown', title: 'Colour', width: 130, align: 'left', source: data.appendData.poColor, filter: colorFilter, readOnly: true },
               // { type: 'dropdown', title: 'Size Spec \n Code / Fit', width: 130, align: 'left', source: data.appendData.specCode, filter: specFilter,readOnly: true },
                { type:'dropdown',title:'Size Spec \n Code / Fit', width:'10%',source: data.appendData.specCode, filter: orderspecFilter_sample,  multiple: true, align:'left',},   
                { title: 'Item Description', width: 140, align: 'left', type: 'dropdown', source: data.itemSource,},
                { title: 'Blend (%)', width: 110, align: 'center', type: 'dropdown', source: data.blendSource, filter: blendFilter ,readOnly: true },
                { title: 'Content', width: 130, align: 'left', type: 'dropdown', source: data.contentSource, filter: contentFilter,readOnly: true },
                { title: 'Material', width: 120, align: 'left', type: 'dropdown', source: data.materialSource, filter: materialFilter,readOnly: true },
                { title: 'Garment\n Size', width: 110, align: 'center', type: 'dropdown', source: data.garmentsizeSource, filter: sizeFilter },
                { title: 'Approved\n Item Code', width: 100, align: 'left', type: 'dropdown', source: data.apprItemCodeSource, filter: appr_item_Filter,readOnly: true },
                { title: 'Approved Item\n Colour Code', width: 130, align: 'left', type: 'dropdown', source: data.apprItemColourCodeSource, filter: appr_item_color_Filter,readOnly: true  },
                { title: 'Size / Dim.\n (L*W*H)', width: 100, align: 'center', type: 'dropdown', source: data.sizeDimSource, filter: size_dim_Filter ,readOnly: true },
                { title: 'UOM', width: 90, align: 'left', type: 'dropdown', source: data.uomSource, filter: uom_Filter ,readOnly: true },
                { title: 'Itemized Qty.\n (Pcs.)', width: 100, align: 'right', readOnly:true },
                { title: 'BOM\n Intake', width: '4%', align: 'center', decimal:',' },
                { title: 'Reqd.\n BOM Qty', width:'8%', align: 'right', readOnly:true },
               { type: 'dropdown', title: 'UOM', width: 110, align: 'left', source: data.UOMDetails }
               


            //   { title:'mode', width: 1,  type:'hidden', align:'center' },
            //    { title:'id',   width: 1,  type:'hidden', align:'center' }, 
            //     { type: 'dropdown',title: 'P.O. No. /\nEnq. Ref. No1.',width:'10%',align: 'left',source: data.appendData.poEnqRefNo,},
            //      { type: 'dropdown', title: 'Combo', width:'13%', align: 'left',source: data.appendData.poCombo, filter: comboFilter_bom1,},
            //      { type: 'dropdown', title: 'Component',width:'12%',align: 'left',source: data.appendData.poComponent, filter: componentFilter,},
            //      { type: 'dropdown', title: 'Colour', width:'12%', align: 'left', source: data.appendData.poColor, filter: colorFilter, readOnly: true },
            //      { type: 'dropdown', title: 'Size Spec \n Code / Fit', width:'12%', align: 'left', source: data.appendData.specCode, filter: specFilter,readOnly: true },
            //      { title: 'Item Description', width:'15%', align: 'left', type: 'dropdown', source: data.itemSource,},
            //      { title: 'Blend (%)', width:'8%', align: 'center', type: 'dropdown', source: data.blendSource, filter: blendFilter ,readOnly: true },
            //      { title: 'Content', width:'10%', align: 'left', type: 'dropdown', source: data.contentSource, filter: contentFilter,readOnly: true },
            //      { title: 'Material', width:'10%', align: 'left', type: 'dropdown', source: data.materialSource, filter: materialFilter,readOnly: true },
            //      { title: 'Garment\n Size', width:'10%', align: 'center', type: 'dropdown', source: data.garmentsizeSource, filter: sizeFilter },
            //      { title: 'Approved\n Item Code', width:'13%', align: 'left', type: 'dropdown', source: data.apprItemCodeSource, filter: appr_item_Filter,readOnly: true },
            //     { title: 'Approved Item\n Colour Code', width:'10%', align: 'left', type: 'dropdown', source: data.apprItemColourCodeSource, filter: appr_item_color_Filter,readOnly: true  },
            //      { title: 'Size / Dim.\n (L*W*H)', width:'10%', align: 'center', type: 'dropdown', source: data.sizeDimSource, filter: size_dim_Filter ,readOnly: true },
            //     { title: 'UOM', width:'6%', align: 'left', type: 'dropdown', source: data.uomSource, filter: uom_Filter ,readOnly: true },
            //   { title: 'Itemized Qty.\n (Pcs.)', width:'10%', align: 'right', readOnly:true },
            //     { title: 'BOM\n Intake', width: '8%', align: 'center', decimal:',' },
            //     { title: 'Request bomqty', width:'8%', align: 'right', readOnly:true },
            //     { title: 'uom', width: '6%', align: 'center', decimal:',' },
  ...
  onchange: function(instance, cell, col, row, val, label, cellName) {
                    ...
                    if(component != '' && combo != '') {
                        ...
                        let component = bom1_requirement_details.data[row][4];
                         filterValue = dd.findIndex(function (item) {
                            if (item.combo == combo && item.component == component) return true;
                        })
                        let colour = dd[filterValue].colour;
                        $(cell).text(colour);
                         instance.jexcel.options.data[row][col] = colour;
                ...
                if(col == 6)
                {
                    ...
                    //     let spec = dd[filterValue].spec;
                    //     $(cell).text(spec);
                    //      instance.jexcel.options.data[row][col] = spec;
             ...
             if(col == 8)
                {
                     //let amount_source = bom1_requirement_details.data[row][6].source;
                       let amount_source = bom1_requirement_details.columns[6].source; 
                        
                     let itemdesc = bom1_requirement_details.data[row][7];
                     //let item_name = instance.jexcel.getValueFromCoords(c - 1, r);
                     if(component != '' && itemdesc != '') {
                        
                        let items = bom1_requirement_details.data[row][7]; // this is the selected item_name
                        let itemSource = bom1_requirement_details.columns[8].source; 
                        let match = itemSource.find(obj => obj.item_name === items);
                      if (match) {
                           let item_nameee = match.name;  // or match.idValue or whatever field you need
                          $(cell).text(item_nameee);
                         instance.jexcel.options.data[row][col] = item_nameee;
                        } 
             ...
             if(col == 9)
                {
                     ...
                     if(component != '' && itemdesc != '') {
                        ...
                        let items = bom1_requirement_details.data[row][7]; // this is the selected item_name
                        let blendSource = bom1_requirement_details.columns[9].source; 
                       
                        let match = blendSource.find(obj => obj.item_name === items);
                      if (match) {
                           let content_name = match.name;
                         
                          $(cell).text(content_name);
                         instance.jexcel.options.data[row][col] = content_name;
                        } 
            ...
            if(col == 10)
                {
                     ...
                     if(component != '' && itemdesc != '') {
                        ...
                        let items = bom1_requirement_details.data[row][7]; // this is the selected item_name
                        let materialSource = bom1_requirement_details.columns[10].source; 
                        
                        let match = materialSource.find(obj => obj.item_name === items);
                      if (match) {
                           let material_name = match.name;
                         
                          $(cell).text(material_name);
                         instance.jexcel.options.data[row][col] = material_name;
                        } 
             ...
             if(col == 12)
                {
                     let garment_size_name = bom1_requirement_details.data[row][11];
                     //let item_name = instance.jexcel.getValueFromCoords(c - 1, r);
                     if(component != '' && garment_size_name != '') {
                       
                        let garment_size = bom1_requirement_details.data[row][11]; // this is the selected item_name
                        let garment_size_code_source = bom1_requirement_details.columns[12].source; 
                        let item_name = bom1_requirement_details.data[row][7];
                          
                        let match = garment_size_code_source.find(obj => obj.garment_size_name === garment_size && obj.item_name === item_name);
                      if (match) {
                           let garment_size_code = match.name;
                         
                          $(cell).text(garment_size_code);
                         instance.jexcel.options.data[row][col] = garment_size_code;
                        } 
            ...
            if(col == 13)
                {
                     ...
                     if(component != '' && garment_size_name != '') {
                        ...
                        let garment_size = bom1_requirement_details.data[row][11]; // this is the selected item_name
                        let item_name = bom1_requirement_details.data[row][7];
                        let colour_code_source = bom1_requirement_details.columns[13].source; 
                       let match = colour_code_source.find(obj => obj.garment_size_name === garment_size && obj.item_name === item_name );
                      if (match) {
                           let colour_code = match.name;
                         
                          $(cell).text(colour_code);
                         instance.jexcel.options.data[row][col] = colour_code;
                        } 
            ...
            if(col == 14)
                {
                     ...
                     if(component != '' && garment_size_name != '') {
                        ...
                        let garment_size = bom1_requirement_details.data[row][11]; // this is the selected item_name
                        let size_dimen_source = bom1_requirement_details.columns[14].source; 
                        let item_name = bom1_requirement_details.data[row][7];
                       let match = size_dimen_source.find(obj => obj.garment_size_name === garment_size && obj.item_name === item_name);
                      if (match) {
                           let size_dimen = match.name;
                         
                          $(cell).text(size_dimen);
                         instance.jexcel.options.data[row][col] = size_dimen;
                        } 
            ...
            if(col == 15)
                {
                     ...
                     if(component != '' && garment_size_name != '') {
                        ...
                        let size_dim = bom1_requirement_details.data[row][14]; // this is the selected item_name
                        let size_dimen_source = bom1_requirement_details.columns[15].source; 

                        let garment_size = bom1_requirement_details.data[row][11]; // this is the selected item_name
                       
                        let item_name = bom1_requirement_details.data[row][7];

                     let match = size_dimen_source.find(obj => obj.size_dim === size_dim && obj.garment_size_name === garment_size && obj.item_name === item_name);
                      if (match) {
                           let uomm = match.name;
                          
                          $(cell).text(uomm);
                         instance.jexcel.options.data[row][col] = uomm;
                        } 
                ...
                if(col == 16) 
                {
                   ...
                   let garment_size_name = bom1_requirement_details.data[row][11];
                     //let item_name = instance.jexcel.getValueFromCoords(c - 1, r);        
                     if(component != '' && garment_size_name != '') {
                        let size_dimen_source = bom1_requirement_details.data[row][11]; 
                        let po_ref = bom1_requirement_details.data[row][2];
                        let combo = bom1_requirement_details.data[row][3];
                        let component = bom1_requirement_details.data[row][4];
                        ...
                        // let size_dimen_source = bom1_requirement_details.data[row][11];
                        // console.log(size_dimen_source);
                        if(po_ref == 'All'){
                            if(size_dimen_source=='All'){
                                 let uniquePoData = Array.from(
                            new Map(dd.map(item => [item.po_ref, item])).values());
                          
                             itemQty = getSizeSum(uniquePoData, 'amount');
                             $(cell).text(itemQty);
                            ...
                            new Map(dd.map(item => [item.po_ref, item])).values());
                             ...
                             itemQty = getSizeSum(uniquePoData, size_dimen_source);
                             $(cell).text(itemQty);
                              instance.jexcel.options.data[row][col] = itemQty; 
                            }
    ...
    let filterValue1 = dd.findIndex(function (item) {
  
    return item.po_ref.trim().toLowerCase() == po_ref.trim().toLowerCase() &&
           item.combo.trim().toLowerCase() == combo.trim().toLowerCase() &&
           item.component.trim().toLowerCase() == component.trim().toLowerCase() &&
    ...
    if (filterValue1 !== -1) {
        if(size_dimen_source == 'All'){
            itemQty = dd[filterValue1].amount; // Use filterValue1 here
            $(cell).text(itemQty);
            instance.jexcel.options.data[row][col] = itemQty;
        } else {
            itemQty = dd[filterValue1][size_dimen_source]; // Use filterValue1 here
            $(cell).text(itemQty);
            instance.jexcel.options.data[row][col] = itemQty;
        }
    } else {
        console.log('No matching item found for the given filters.');
    }
        ...
        $('#bom1RequirementSubmit').click(function () {
            swalWithBootstrapButtons.fire({
                title: 'Are you sure want to \n save the details ?', 
                //text: "If you save You won't be able to revert this!",
                 type: 'warning', showCancelButton: true, scrollbarPadding: false, confirmButtonText: 'Yes, do it!', cancelButtonText: 'No, cancel!', reverseButtons: true, customClass: { 'confirmButton': 'btn btn-green mx-2 px-3', 'cancelButton': 'btn btn-red mx-2 px-3' }
            }).then(function (result) {
                if (result.value) {
                    ...
                    if (result.isConfirmed) {
                        // Get active tab
                        let $active = $('.nav-tabs.bom-article li.active');
                        let $next = $active.next('li'); // Get next tab

                        // If the next tab exists, show it
                        if ($next.length) {
                            $next.find('a[data-toggle="tab"]').tab('show');
                        }
                           setTimeout(function () {
    ...
    function append_bom1_sourcing_details(data) {
        ...
        let bom1_sourcing_details_wise = {
            ...
            columns: [
                ...
                { title: 'Item description', width: '6%', align: 'left', readOnly: true },
                { type: 'dropdown', title: 'Sourcing Advice', width: '6%', align: 'left', source: data.sourceData, },
                { type: 'dropdown', title: 'Vendor Location', width: '6%', align: 'left', source: ['Local', 'Within State', 'Within Country', 'Overseas'], },
                { type: 'dropdown', title: 'Vendor Name & \n Address', width: '9%', align: 'left', source:data.bomVendor },
                { title: 'Contact Person / e-mail ID / Phone / Mobile', width: '9%', align: 'left', readOnly: true },
            ...
            updateTable: function(instance, cell, col, row, val, label, cellName, des_city) {
                ...
                if(col == 6) {
                    if(vendor_name != '') {
                        ...
                        const displayText = obj.name.replace(/\n/g, '<br>');
    ...
    function append_bom2_requirement_details(data) {
        ...
        let bom2_requirement_details = {
            ...
            columns: [
                ...
                { title: 'Material', width: 120, align: 'left', type: 'dropdown', source: data.materialSource, filter: materialFilter,readOnly: true},
                { title: 'Garment\n Size', width: 110, align: 'center', type: 'dropdown', source: data.garmentsizeSource, filter: sizeFilter },
                { title: 'Approved\n Item Code', width: 130, align: 'left', type: 'dropdown', source: data.apprItemCodeSource, filter: appr_item_Filter,readOnly: true },
                { title: 'Approved Item\n Colour Code', width: 130, align: 'left', type: 'dropdown', source: data.apprItemColourCodeSource, filter: appr_item_color_Filter,readOnly: true },
                { title: 'Size / Dim.\n (L*W*H)', width: 120, align: 'center', type: 'dropdown', source: data.sizeDimSource, filter: size_dim_Filter,readOnly: true },
                { title: 'UOM', width: 110, align: 'left', type: 'dropdown', source: data.uomSource, filter: uom_Filter,readOnly: true },
            ...
            updateTable: function(instance, cell, col, row, val, label, cellName) {
                ...
                {
                     //let amount_source = bom1_requirement_details.data[row][6].source;
                       let amount_source = bom2_requirement_details.columns[6].source; 
                        
                     let itemdesc = bom2_requirement_details.data[row][7];
                     //let item_name = instance.jexcel.getValueFromCoords(c - 1, r);
                     if(component != '' && itemdesc != '') {
                        
                        let items = bom2_requirement_details.data[row][7]; // this is the selected item_name
                        let itemSource = bom2_requirement_details.columns[8].source; 
                        let match = itemSource.find(obj => obj.item_name === items);
                      if (match) {
                           let item_nameee = match.name;  // or match.idValue or whatever field you need
                          $(cell).text(item_nameee);
             ...
             if(col == 9)
                {
                     ...
                     if(component != '' && itemdesc != '') {
                        ...
                        let items = bom2_requirement_details.data[row][7]; // this is the selected item_name
                        let blendSource = bom2_requirement_details.columns[9].source; 
            ...
            if(col == 10)
                {
                     ...
                     if(component != '' && itemdesc != '') {
                        ...
                        let items = bom2_requirement_details.data[row][7]; // this is the selected item_name
                        let materialSource = bom2_requirement_details.columns[10].source; 
             ...
             if (col == 12) {
    let garment_size_name = bom2_requirement_details.data[row][11];
    if (component != '' && garment_size_name != '') {
        let garment_size = bom2_requirement_details.data[row][11];
        let garment_size_code_source = bom2_requirement_details.columns[12].source;
        let item_name = bom2_requirement_details.data[row][7];
...
if (col == 13) {
    ...
    if (component != '' && garment_size_name != '') {
        let garment_size = bom2_requirement_details.data[row][11];
        let item_name = bom2_requirement_details.data[row][7];
        let colour_code_source = bom2_requirement_details.columns[13].source;
        let match = colour_code_source.find(obj => obj.garment_size_name === garment_size && obj.item_name === item_name);
        if (match) {
            let colour_code = match.name;
            $(cell).text(colour_code);
...
if (col == 14) {
    ...
    if (component != '' && garment_size_name != '') {
        let garment_size = bom2_requirement_details.data[row][11];
        let size_dimen_source = bom2_requirement_details.columns[14].source;
        let item_name = bom2_requirement_details.data[row][7];
        let match = size_dimen_source.find(obj => obj.garment_size_name === garment_size && obj.item_name === item_name);
        if (match) {
            let size_dimen = match.name;
...
if (col == 15) {
    let garment_size_name = bom2_requirement_details.data[row][11];
    if (component != '' && garment_size_name != '') {
        let size_dim = bom2_requirement_details.data[row][14];
        let size_dimen_source = bom2_requirement_details.columns[15].source;
...
if (col == 16) {
    let garment_size_name = bom2_requirement_details.data[row][11];
    if (component != '' && garment_size_name != '') {
        let size_dimen_source = bom2_requirement_details.data[row][11];
        let po_ref = bom2_requirement_details.data[row][2];
        let combo = bom2_requirement_details.data[row][3];
        let component = bom2_requirement_details.data[row][4];
    ...
    return item.po_ref.trim().toLowerCase() == po_ref.trim().toLowerCase() &&
           ...
           item.colour.trim().toLowerCase() == colour.trim().toLowerCase() 
            ...
            if (size_dimen_source == 'All') {
                itemQty = dd[filterValue1].amount;
                $(cell).text(itemQty);
                instance.jexcel.options.data[row][col] = itemQty;
            } else {
                itemQty = dd[filterValue1][size_dimen_source];
                $(cell).text(itemQty);
                instance.jexcel.options.data[row][col] = itemQty;
            }
        }
    ...
    function append_bom2_sourcing_details(data) {
        ...
        let bom1_sourcing_details_wise = {
            ...
            columns: [
                { title:'mode', width:'0%',align:'left',type:'hidden'},
                { title:'id', width:'0%',align:'left',type:'hidden'},
                { title: 'Item Description', width: '5%', align: 'left', readOnly: true },
                { type: 'dropdown', title: 'Sourcing Advice', width: '6%', align: 'left', source: data.sourceData, },
                { type: 'dropdown', title: 'Vendor Location', width: '6%', align: 'left', source: ['Local', 'Within State', 'Within Country', 'Overseas'], },
                { type: 'dropdown', title: 'Vendor Name & \n Address', width: '7%', align: 'left', source: data.bomVendor, },
                { title: 'Contact Person / e-mail ID / Phone / Mobile', width: '11%', align: 'left', readOnly: true },
                { title: 'GST / IE Code Details', width: '7%', align: 'left', readOnly: true },
                { title: 'If On-line Ordering System\n Website / User ID / Password', width: '10%', align: 'left' },
                { type: 'calendar', title: 'Password Expiry \n Date & Time', width: '7%', align: 'center' }
            ],
    ...
    function append_bom_1_pi_details(data) {
        ...
        let bom1_pi_details = {
            ...
            columns: [
                ...
                { title: 'P.I. Ref. No.', width: '9%', align: 'right', readOnly: true },
                { title: 'P.I. Qty.', width: '5%', align: 'right', readOnly: true },
                { title: 'UOM', width: '5%', align: 'center', readOnly: true },
                { title: 'P.I. Approval\n Status', width: '6%', type:'text', align: 'right', readOnly: true, },
                { title: 'P.I. Approved\n Date & Time', width: '6%', align: 'right', readOnly: true },
                { title: 'Expected\n Date of Delivery', width: '6%', align: 'right', readOnly: true },
        ...
        let approvalStatusData = [
         ...
         let bom1_item_accept_status = {
             ...
             columns: [
                 ...
                 { type: 'text', title: 'Approved\n Item Code', width: '6%', align: 'left', readOnly: true },
                 { type: 'text', title: 'Approved Item\n Colour Code', width: '6%', align: 'left', readOnly: true },
                 { title: 'D.C. No.', width: '10%', align: 'right', readOnly: true },
                 ...
                 { title: 'Merchant Status\n Update Date & Time', width: '6%', align: 'center', type: 'text', readOnly: true, options: { format:'DD/MM/YYYY HH12:MI AM/PM', time:1 } },
            ...
            tableOverflow: true,
             ...
             updateTable: function(instance, cell, col, row, val, label) {
                 ...
                 if(col == 9)
                {
                     meStatus1 = data.itemacceptstatus[row][9];
                 ...
                 if(col == 11)
                {
                     qaStatus1 = data.itemacceptstatus[row][11];
                  ...
                  if(col == 13)
                {
                     maStatus1 = data.itemacceptstatus[row][11];
        ...
        let bom1_in_house_consolidated_qty_details = {
            ...
            updateTable: function(instance, cell, col, row, val, label) {
                ...
                if(col == 9) {
                 ...
                 if(col == 11) {
                   suplaystatus= data.inhouseconsolidatedqtydetails[row][11];
                
                    if(suplaystatus == 0 ) {
                        $(cell).css('background-color', '#FFA519');
                    }
                    else if(suplaystatus == 1) {
                        $(cell).css('background-color', '#fc0303ff');
                    } else if(suplaystatus == 2) {
                        $(cell).css('background-color', '#fc0303ff');
                    } else if(suplaystatus == 3) {
                        $(cell).css('background-color', '#5DE684');
                    } else if(suplaystatus == 4) {
                        $(cell).css('background-color', '#fc0303ff');
                    } 
                    // else {
                    //      $(cell).text('-');
        ...
        let bom1_in_house_status_details = {
            ...
            tableOverflow: true,
             ...
             updateTable: function (instance, cell, col, row, val, label) {
                 ...
                 if (col === 6) { // Q.A. Status column index
                  setTimeout(() => {
                    const statusId_1 = val;

                  
                    let backgroundColor = '';
                    let textColor = 'black';
                    //const group1 = ['0', '1', '2', '3'];
                     const group1 = ['IN QUEUE', 'SCHEDULED', 'RE-SCHEDULED', 'Q.A.IN PROGRESS'];
                      //const group2 = ['5', '6'];
                    const group2 = ['PASS', 'PASS COND.'];
                      //const group2 = ['4', '7'];
                    const group3 = ['DISCREPANCY','FAIL','IN-QUEUE RR'];

                    if (group1.includes(statusId_1)) {
                        backgroundColor = '#FFA519'; // light yellow
                        } else if (group2.includes(statusId_1)) {
                        backgroundColor = '#5DE684'; // light green
                       
                    } else if (group3.includes(statusId_1)) {
            ...
            if (col === 8) { // Q.A. Status column index
                setTimeout(() => {
                    ...
                    const group1 = ['IN-QUEUE', 'SCHEDULED', 'RESCHEDULED', 'Q.A. REQ. SENT.','WORK IN PROG.'];
                     //const group2 = ['5', '6','7'];
                    const group2 = ['Q.A. RR SENT', 'RE-WORK PEND.','RE-WORK IN PROG.'];
                      //const group3 = ['4'];
                    const group3 = ['COMPLETED',];
    ...
    function append_sam_qa_audit_details(data) {
        ...
        let sam_qa_audit_details = {
            ...
            updateTable: function (instance, cell, col, row, val, label) {
                 ...
                 if (col === 7) { // Q.A. Status column index
                  setTimeout(() => {
                    const statusId_1 = val;
                    ...
                    const group2 = ['PASS', 'PASS COND.'];
                      //const group2 = ['4', '7'];
                    const group3 = ['NEED ALTERATION','FAIL','IN-QUEUE RR'];
            ...
            if (col === 9) { // Q.A. Status column index
                setTimeout(() => {
                    ...
                    const group1 = ['IN-QUEUE', 'SCHEDULED', 'RESCHEDULED', 'Q.A. REQ. SENT','JOB IN PROG.'];
                     //const group2 = ['5', '6','7','10'];
                    const group2 = ['Q.A. RR SENT', 'ALT. PEND.','ALT. IN PROG.','REWORK'];
                      //const group3 = ['4','9'];
                    const group3 = ['COMPLETED','GAR. ISSUED'];
    ...
    function getSizeSum(data, sizeKey) {
    return data.reduce((sum, item) => {
        return sum + (parseFloat(item[sizeKey]) || 0);
    }, 0);
}
    ...
    document.getElementById('fullscreenButton').addEventListener('click', function() {
        _proto.toggleFullscreen.call(this); // Call the toggleFullscreen method
    });
 });
 ...
 $(document).on('click', '.jexcel_date', function (e) {
    alert('clicked');
    var datepicker = $('.js-datepicker');
    var cellOffset = $(this).offset();
    
    // Adjust the date picker position manually
    datepicker.css({
        top: cellOffset.top - datepicker.height() - 10, // position above the cell
        left: cellOffset.left // keep it aligned horizontally
    });

    datepicker.show();
});

function placeCalendarAtCell(cell, force) {
    const cal = document.querySelector('.jcalendar-container') || document.querySelector('.jcalendar');
    if (!cal || !cell) return;

    // Move calendar to <body> so it isn't clipped by grid wrappers
    if (cal.parentElement !== document.body) document.body.appendChild(cal);
    ...
    let top;
    if (force === 'above') {
      top = window.scrollY + r.top - calH - 4;
    } else if (force === 'below') {
      top = window.scrollY + r.bottom + 4;
    } else {
      // auto choose side with more room
      top = (spaceBelow >= calH || spaceBelow >= spaceAbove)
        ? window.scrollY + r.bottom + 4
        : window.scrollY + r.top - calH - 4;
    }

    cal.style.position = 'absolute';
    cal.style.left = Math.round(window.scrollX + r.left) + 'px';
    cal.style.top  = Math.max(0, Math.round(top)) + 'px';

    // If space is tight, constrain height
    const room = Math.max(spaceAbove, spaceBelow) - 8;
    cal.style.maxHeight = Math.max(160, Math.min(calH, room)) + 'px';
