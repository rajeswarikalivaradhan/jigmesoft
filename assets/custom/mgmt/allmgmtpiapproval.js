...
$(document).ready(function() {
	...
	{
		return $.ajax({
			url: base_path+'management/getAllMgmtPIApprovalList',
			type:'POST',
			...
			"aoColumns": [	
				...
				{ 
					"mDataProp": function ( data, type, full, meta) {
						...
						return '<a class="bold" href="' + base_path +'request/Bomrequest/managementpurchaseindentapprovaldetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqId/' + encodeURIComponent(btoa(data.request_id)) + '/' +encodeURIComponent(btoa(data.purchase_indent_id)) + '">' + data.isriorcode + '</a>';
					}
				},		
				{ "mDataProp": "brandname" },	
				{ 
					"mDataProp": function ( data, type, full, meta) {
						//return '<a class="bold" href="' + base_path +'request/Fabricrequest/yarnmgmtpiapprovaldetails' + '/' + encodeURIComponent(btoa(data.enquiry_id)) + '/reqYarnId/' + encodeURIComponent(btoa(data.request_yarn_id)) + '"> P.I.APPROVAL </a>';
...
$('#btnChangeStatus').on('click', function () {
        ...
        if(checkBoxLength == 0) {
         ...
         $('#btn-active').on('click', function () {
            ...
            inactiveBtn.classList.remove('active');;
            const enquiryJSON1 = reqRequestJSON.filter(item => item.flag === satusval);
